...
object ContextRDD {
  ...
  def textFilesLines(
    ...
    ContextRDD.weaken(
      SparkBackend
        .sparkContext("ContxtRDD.textFilesLines").textFilesLines(files, nPartitions)
  ...
  def czipNPartitions[T: ClassTag, U: ClassTag](
    ...
    def inCtx(f: RVDContext => Iterator[U]): Iterator[RVDContext => Iterator[U]] =
      Iterator.single(f)
    new ContextRDD(MultiWayZipPartitionsRDD(crdds.map(_.rdd)) { its =>
...
class ContextRDD[T: ClassTag](
  ...
  def cmapPartitions[U: ClassTag](
    ...
    rdd.mapPartitions(part => inCtx(ctx => f(ctx, part.flatMap(_(ctx)))), preservesPartitioning)
  )
  ...
  def cmapPartitionsWithContext[U: ClassTag](
    ...
    new ContextRDD(
      rdd.mapPartitions(part => part.flatMap(x => inCtx(consumerCtx => f(consumerCtx, x))))
    )
  ...
  def cmapPartitionsWithContextAndIndex[U: ClassTag](
    ...
    new ContextRDD(
      rdd.mapPartitionsWithIndex((i, part) =>
        part.flatMap(x => inCtx(consumerCtx => f(i, consumerCtx, x)))
      )
    )
  ...
  def cmapPartitionsAndContext[U: ClassTag](
    ...
    onRDD(_.mapPartitions(part => inCtx(ctx => f(ctx, part)), preservesPartitioning))
  ...
  def cmapPartitionsAndContextWithIndex[U: ClassTag](
    ...
    onRDD(
      _.mapPartitionsWithIndex((i, part) => inCtx(ctx => f(i, ctx, part)), preservesPartitioning)
    )
  ...
  def czipPartitionsWithIndex[U: ClassTag, V: ClassTag](
    ...
    rdd
      .zipPartitions(that.rdd, preservesPartitioning)((l, r) =>
        Iterator.single(l -> r)
