...
object OrderedDependency {
  def generate[T](oldPartitioner: RVDPartitioner, newIntervals: IndexedSeq[Interval], rdd: RDD[T])
    : OrderedDependency[T] =
    new OrderedDependency(newIntervals.map(oldPartitioner.queryInterval).toArray, rdd)
}
...
class RepartitionedOrderedRDD2 private (
  ...
  val dependency: OrderedDependency[_] =
    OrderedDependency.generate(prev.partitioner, newRangeBounds, prevCRDD.rdd)
