  ...
  def exportType(t: Type): DataType = (t: @unchecked) match {
    ...
    case tbs: TBaseStruct =>
      if (tbs.fields.isEmpty)
        ...
        StructType(
          tbs.fields
            .map(f => StructField(escapeColumnName(f.name), f.typ.schema, nullable = true))
        )
  }
}
      ...
      (t: @unchecked) match {
        ...
        case TNDArray(elementType, _) =>
          ...
          JObject(
            ...
            "data" -> JArray(
              jnd.getRowMajorElements().map(a => exportAnnotation(a, elementType)).toList
            ),
          )
      }
    }
  ...
  private def importAnnotationInternal(
    ...
    (jv, t) match {
      ...
      case (JArray(a), TArray(elementType)) =>
        a.iterator
          .map(jv2 => imp(jv2, elementType, parent + "[element]")).toArray[Any]: IndexedSeq[Any]
