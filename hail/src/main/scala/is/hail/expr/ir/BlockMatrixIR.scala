...
class BlockMatrixNativeReader(
  ...
  override def lower(ctx: ExecuteContext, evalCtx: IRBuilder): BlockMatrixStage2 = {
    ...
    BlockMatrixStage2(FastSeq(), fullType, contexts, blockIR)
  }
...
case class BlockMatrixDot(left: BlockMatrixIR, right: BlockMatrixIR) extends BlockMatrixIR {
  override lazy val typ: BlockMatrixType = {
    ...
    val sparsity =
      if (left.typ.isSparse || right.typ.isSparse)
        BlockMatrixSparsity.constructFromShapeAndFunction(
          ...
          Array
            .tabulate(BlockMatrixType.numBlocks(rCols, blockSize)) { k =>
      ...
      if (child.typ.isSparse)
        inIndexExpr match {
          ...
          case IndexedSeq(1, 0) => // transpose
            ...
            BlockMatrixSparsity(
              child.typ.sparsity.definedBlocks.map(seq => seq.map { case (i, j) => (j, i) })
            )
          case IndexedSeq(0, 1) =>
            assert(child.typ.blockSize == blockSize)
...
case class BlockMatrixAgg(
  ...
  override lazy val typ: BlockMatrixType = {
    ...
    val shape = IndexedSeq(0, 1)
      .filter(i => !axesToSumOut.contains(i)).map({ i: Int => matrixShapeArr(i) }).toFastSeq
...
case class RowIntervalSparsifier(
  ...
  def pretty(): String =
    s"(RowIntervalSparsifier ${Pretty.prettyBooleanLiteral(blocksOnly)} ${starts.mkString("(", " ", ")")} ${stops
        .mkString("(", " ", ")")})"
}
