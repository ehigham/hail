...
class AppendOnlyBTree(
    ...
    val insertAt = kb.getOrGenEmitMethod(
      ...
      def promote(cb: EmitCodeBuilder, idx: Int): Unit = {
        ...
        key.copy(
          cb,
          loadKey(cb, node, idx),
          cb.invokeCode(insertAt, parent(cb), upperBound, key.loadCompKey(cb, nikey), newNode),
        )
      ...
      def splitAndInsert(cb: EmitCodeBuilder): Code[Long] = {
        ...
        cb.if_(
          insertIdx > splitIdx, {
            ...
            cb.assign(
              out,
              cb.invokeCode(insertAt, newNode, cb.memoize(insertIdx - splitIdx - 1), k, child),
            )
          }, {
  ...
  def bulkStore(
    ...
    val f = kb.genEmitMethod(
      "btree_bulkStore",
      FastSeq[ParamType](typeInfo[Long], typeInfo[OutputBuffer]),
      typeInfo[Unit],
    )
    ...
    f.voidWithBuilder { cb =>
      ...
      cb.if_(!isLeaf(cb, node), cb.invokeVoid(f, loadChild(cb, node, -1), ob))
      val Lexit = CodeLabel()
      (0 until maxElements).foreach { i =>
        cb.if_(
          hasKey(cb, node, i), {
            ...
            cb.if_(!isLeaf(cb, node), cb.invokeVoid(f, loadChild(cb, node, i), ob))
          }, {
