...
class TableStage(
      ...
      case Some(k) =>
        if (!partitioner.kType.fieldNames.startsWith(k))
          throw new RuntimeException(
            s"cannot map partitions to new key!" +
  ...
  def collectWithGlobals(staticID: String, dynamicID: IR = NA(TString)): IR =
    mapCollectWithGlobals(staticID, dynamicID)(ToArray) { (parts, globals) =>
      MakeStruct(
        FastSeq("rows" -> ToArray(flatMapIR(ToStream(parts))(ToStream(_))), "global" -> globals)
      )
    }
    ...
    val newStage = if (LowerTableIR.isRepartitioningCheap(partitioner, newPartitioner)) {
      ...
      if (
        startAndEnd.forall { case ((start, end), i) =>
          start + 1 == end &&
          newPartitioner
            .rangeBounds(start).includes(newPartitioner.kord, partitioner.rangeBounds(i))
        }
      ) {
      ...
      val newContexts = Let(
        ...
        StreamMap(
          ToStream(Literal(TArray(partitionMappingType), partitionMapping)),
          mappingUID,
          MakeStruct(
            FastSeq(
      ...
      TableStage(
        ...
        newContexts,
        (ctxRef: Ref) => {
          val body = self.partition(
            Ref(prevContextUIDPartition, self.contexts.typ.asInstanceOf[TStream].elementType)
          )
  ...
  def intervalAlignAndZipPartitions(
    ...
    val rightWithPartNums = right.mapPartition(None) { partStream =>
      flatMapIR(partStream) { row =>
        val interval = bindIR(GetField(row, right.key.head)) { interval =>
          invoke(
            ...
            MakeTuple.ordered(
              FastSeq(
                MakeStruct(
                  FastSeq(kType.fieldNames.head -> invoke("start", kType.types.head, interval))
                ),
                I32(1),
              )
            ),
            MakeTuple.ordered(
              FastSeq(
                MakeStruct(
                  FastSeq(kType.fieldNames.head -> invoke("end", kType.types.head, interval))
                ),
                I32(1),
              )
            ),
        ...
        bindIR(
          invoke("partitionerFindIntervalRange", TTuple(TInt32, TInt32), irPartitioner, interval)
        ) { range =>
    ...
    val rightRowRTypeWithPartNum = IndexedSeq(
      "__partNum" -> TypeWithRequiredness(TInt32)
    ) ++ rightRowRType.fields.map(rField => rField.name -> rField.typ)
      ...
      case TableToValueApply(child, TableCalculateNewPartitions(nPartitions)) =>
        ...
        bindIR(flatten(stage.mapCollect("table_calculate_new_partitions") { rows =>
          streamAggIR(mapIR(rows)(row => SelectFields(row, keyType.fieldNames))) { elt =>
            ToArray(
              flatMapIR(
                ToStream(
                  MakeArray(
                    ApplyAggOp(FastSeq(I32(samplesPerPartition)), FastSeq(elt), samplekey),
                    ApplyAggOp(FastSeq(I32(1)), FastSeq(elt, elt), minkey),
                    ApplyAggOp(FastSeq(I32(1)), FastSeq(elt, elt), maxkey),
                  )
                )
          ...
          bindIR(
            ToArray(
              flatMapIR(
                StreamGroupByKey(ToStream(sorted), keyType.fieldNames, missingEqual = true)
          ...
          lcWithInitBinding.mapCollectWithGlobals("table_aggregate")({ part: IR =>
              ...
              Begin(
                FastSeq(
                  if (useInitStates) {
                    ...
                    bindIR(
                      ReadValue(ArrayRef(partArrayRef, 0), reader, reader.spec.encodedVirtualType)
                    ) { serializedTuple =>
            ...
            bindIR(
              TailLoop(
                treeAggFunction,
                FastSeq[(String, IR)](
                  currentAggStates.name -> collected,
                  iterNumber.name -> I32(0),
                ),
                loopBody.typ,
                loopBody,
              )
    ...
    val lowered: TableStage = tir match {
          ...
          bindIR(
            ...
            bindIR(GetField(loweredRowsAndGlobalRef, "rows")) { rows =>
              mapIR(rangeIR(nPartitionsAdj)) { partIdx =>
                ToArray(
                  mapIR(
                    rangeIR(ArrayRef(indicesArray, partIdx), ArrayRef(indicesArray, partIdx + 1))
          ...
          body = in =>
            lowerIR {
              ...
              if (partitioner.kType.fields.isEmpty) rows
                ...
                bindIR(GetTupleElement(in, 1)) { interval =>
                  mapIR(rows) { row =>
              ...
              mapIR(StreamGroupByKey(partition, child.typ.key, missingEqual = true)) { groupRef =>
                StreamAgg(
                  ...
                  bindIRs(
                    ...
                    MakeStruct(
                      child.typ.key.map(k => (k, GetField(key, k))) ++ expr.typ
                        .asInstanceOf[TStruct].fieldNames.map(f => (f, GetField(value, f)))
                    )
                  },
                )
              },
            ...
            part.rangeBounds.zipWithIndex.flatMap { case (interval, i) =>
              ...
              if (
                (lowerBound until upperBound).map(filterPartitioner.rangeBounds).exists {
                  filterInterval =>
                    iord.compareNonnull(filterInterval.left, interval.left) <= 0 && iord
                      .compareNonnull(filterInterval.right, interval.right) >= 0
                }
              )
                None
        ...
        def answerTuple(partitionSizeArrayRef: Ref): IR =
          bindIR(ArrayLen(partitionSizeArrayRef)) { numPartitions =>
            ...
            val loopBody = If(
              ...
              Recur(
                howManyPartsToKeep,
                FastSeq(i + 1, numLeft - Cast(ArrayRef(partitionSizeArrayRef, i), TInt64)),
                  ...
                  bindIR(WriteValue(initState, Str(tmpDir) + UUID4(), writer)) { freshState =>
                    val loopBody = If(
                      ...
                      bindIR(ArrayRef(aggStack, level)) { aggsArray =>
                        val groups = mapIR(
                          zipWithIndex(
                            mapIR(StreamGrouped(ToStream(aggsArray), I32(branchFactor)))(x =>
                              ToArray(x)
                            )
                          )
                        ) { eltAndIdx =>
                        ...
                        Recur(
                          downPassLoopName,
                          IndexedSeq(level - 1, ToArray(flatten(ToStream(results))), iteration + 1),
                          TArray(TString),
                        )
                      },
                    )
      ...
      case TableIntervalJoin(left, right, root, product) =>
        ...
        def partitionJoiner(lPart: IR, rPart: IR): IR = {
          ...
          StreamJoinRightDistinct(
            ...
            InsertFields(
              lEltRef,
              FastSeq(root -> SelectFields(rEltRef, right.typ.valueType.fieldNames)),
            ),
            "left",
          )
        }

        loweredLeft.intervalAlignAndZipPartitions(
          ctx,
          loweredRight,
      ...
      case x @ TableMultiWayZipJoin(children, fieldName, globalName) =>
        ...
        val newGlobals = MakeStruct(
          FastSeq(globalName -> MakeArray(lowered.map(_.globals), TArray(lowered.head.globalType)))
        )
        val globalsRef = Ref(genUID(), newGlobals.typ)
      ...
      case TableToTableApply(child, TableFilterPartitions(seq, keep)) =>
        ...
        if (keep) {
          ...
          lc.copy(
            ...
            contexts = mapIR(filterIR(zipWithIndex(lc.contexts)) { t =>
          ...
          lc.copy(
            partitioner = lc.partitioner.copy(rangeBounds =
              lc.partitioner.rangeBounds.zipWithIndex
                .filter { case (_, idx) => !keptSet.contains(idx) }.map(_._1)
            ),
            contexts = mapIR(filterIR(zipWithIndex(lc.contexts)) { t =>
              !invoke("contains", TBoolean, lit, GetField(t, "idx"))
            })(t => GetField(t, "elt")),
  ...
  def isRepartitioningCheap(original: RVDPartitioner, planned: RVDPartitioner): Boolean = {
    val cost =
      if (original.numPartitions == 0)
        ...
        (0.0167 / original.numPartitions) * planned.rangeBounds.map { intrvl =>
