...
object EmitStreamDistribute {
  ...
  def emit(
    ...
    val treeHeight = cb.memoize[Int](
      Code.invokeScalaObject1[Int, Int](MathFunctions.getClass, "log2", filledInTreeSize)
    )
    ...
    def cleanupSplitters() = {
      ...
      cb.for_(
        ...
        cb.assign(requestedSplittersIdx, requestedSplittersIdx + 1), {
          val currentSplitter = requestedSplittersVal
            .loadElement(cb, requestedSplittersIdx).memoize(
              cb,
              "stream_distribute_current_splitter",
            )
          cb.if_(
            requestedSplittersIdx ceq 0, {
              ...
              cb.if_(
                !equal(cb, lastKeySeen, currentSplitter), {
                  ...
                  paddedSplittersPType.elementType.storeAtAddress(
                    cb,
                    paddedSplittersPType
                      .loadElement(paddedSplittersAddr, paddedSplittersSize, uniqueSplittersIdx),
                    region,
                    currentSplitter.get(cb),
                    false,
                  )
      ...
      cb.for_(
        ...
        cb.assign(uniqueSplittersIdx, uniqueSplittersIdx + 1), {
          ...
          paddedSplittersPType.elementType
            .storeAtAddress(cb, loaded, region, lastKeySeen.get(cb), false)
        },
      )
        ...
        cb.assign(currentHeight, currentHeight - 1), {
          ...
          cb.for_(
            ...
            cb.assign(inner, inner + 1), {
              val elementLoaded = paddedSplitters
                .loadElement(cb, startingPoint + inner * (const(1) << (currentHeight + 1))).get(cb)
    ...
    def createFileMapping(
      ...
      def destFileSCode(cb: EmitCodeBuilder) = new SInt32Value(
        cb.memoize(
          (currentFileToMapTo >= numFilesToWrite).mux(numFilesToWrite - 1, currentFileToMapTo)
        )
      )
        ...
        cb.assign(bucketIdx, bucketIdx + indexIncrement), {
          ...
          cb.if_(
            shouldUseIdentityBuckets, {
              cb.assign(
                currentFileToMapTo,
                currentFileToMapTo + splitterWasDuplicated
                  .loadElement(cb, bucketIdx / 2).get(cb).asBoolean.value.toI,
              )
              fileMappingType.elementType.storeAtAddress(
                cb,
    ...
    cb.for_(
      ...
      cb.assign(
        numFilesToWrite,
        numFilesToWrite + 1 + splitterWasDuplicated
          .loadElement(cb, uniqueSplittersIdx).get(cb).asBoolean.value.toI,
      ),
    )
      ...
      cb.if_(
        ...
        cb.assign(
          ...
          const(2) * b + 1 - lessThan(
            ...
            current,
            paddedSplitters
              .loadElement(cb, b - numberOfBuckets / 2).memoize(cb, "stream_dist_splitter_compare"),
          ).toI,
        ),
      )
    ...
    val max = requestedSplittersAndEndsVal
      .loadElement(cb, requestedSplittersAndEndsVal.loadLength() - 1).memoize(cb, "stream_dist_min")
    val lastSplitter = paddedSplitters
      .loadElement(cb, paddedSplitters.loadLength() - 1).memoize(cb, "stream_dist_last_splitter")

    val skipMinInterval = cb.memoize(
      equal(cb, min, firstSplitter) && splitterWasDuplicated
        .loadElement(cb, 0).get(cb).asBoolean.value
    )
      ...
      !skipMaxInterval, {
        val lastInterval = intervalType.constructFromCodes(
          ...
          EmitCode.fromI(cb.emb)(cb =>
            requestedSplittersAndEndsVal
              .loadElement(cb, requestedSplittersAndEndsVal.loadLength() - 1)
          ),
          false,
          true,
        )
