...
case class BlockMatrixNativeWriter(
  ...
  override def lower(
    ...
    RelationalWriter.scoped(path, overwrite, None)(
      WriteMetadata(paths, BlockMatrixNativeMetadataWriter(path, stageLocally, s.typ))
    )
  }
}
...
case class BlockMatrixNativeMetadataWriter(
  ...
  def writeMetadata(
    ...
    cb += cb.emb
      .getObject(metaHelper).invoke[FS, Array[String], Unit]("write", cb.emb.getFS, partFiles)
  }
