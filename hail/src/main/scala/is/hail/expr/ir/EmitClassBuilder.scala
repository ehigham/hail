...
class EmitModuleBuilder(val ctx: ExecuteContext, val modb: ModuleBuilder) {
  ...
  def referenceGenomeMap: Value[Map[String, ReferenceGenome]] = {
    if (_rgMapField == null) {
      ...
      _rgMapField = new StaticFieldRef(
        cls.newStaticField("reference_genome_map", Code._null[Map[String, ReferenceGenome]])
      )
    }
    _rgMapField
  }
...
trait WrappedEmitClassBuilder[C] extends WrappedEmitModuleBuilder {
  ...
  def addModule(
    name: String,
    mod: (HailClassLoader, FS, HailTaskContext, Region) => AsmFunction3[Region, Array[Byte], Array[
      Byte
    ], Array[Byte]],
  ): Unit =
  ...
  ): EmitMethodBuilder[C] = ecb.getOrGenEmitMethod(baseName, key, argsInfo, returnInfo)(body)
...
class EmitClassBuilder[C](
    ...
    mb2.voidWithBuilder { cb =>
      ...
      preEncodedLiterals.zipWithIndex.foreach { case ((encLit, pt, arrIdx), index) =>
        ...
        cb.assign(
          ib,
          spec.buildCodeInputBuffer(
            Code.newInstance[ArrayOfByteArrayInputStream, Array[Array[Byte]]](
              Code.checkcast[Array[Array[Byte]]](allEncodedFields(index + 1))
            )
          ),
        )
        val decodedValue = encLit.codec.encodedType
          .buildDecoder(encLit.typ, this)
          .apply(cb, partitionRegion, ib)
  ...
  def addAggStates(aggSigs: Array[agg.AggStateSig]): agg.TupleAggregatorState = {
    ...
    val (nfcode, states) = EmitCodeBuilder.scoped(newF) { cb =>
      ...
      cb += (_aggOff := _aggRegion
        .load().allocate(states.storageType.alignment, states.storageType.byteSize))
    ...
    setSer.emit(
      _aggSerialized.load().update(setSer.getCodeParam[Int](1), setSer.getCodeParam[Array[Byte]](2))
    )
  ...
  def runMethodWithHailExceptionHandler(mname: String): Code[(String, java.lang.Integer)] =
    Code.invokeScalaObject2[AnyRef, String, (String, java.lang.Integer)](
      ...
      "handleUserException",
      cb._this.get.asInstanceOf[Code[AnyRef]],
      mname,
    )
  ...
  def getStructOrderingFunction(
    ...
    op: CodeOrdering.Op,
  ): CodeOrdering.F[op.ReturnType] = { (cb: EmitCodeBuilder, v1: EmitValue, v2: EmitValue) =>
  ...
  def getThreefryRNG(): Value[ThreefryRandomEngine] = {
    threefryRNG match {
      ...
      case None =>
        ...
        val rngInit =
          Code.invokeScalaObject0[ThreefryRandomEngine](ThreefryRandomEngine.getClass, "apply")
  ...
  def resultWithIndex(print: Option[PrintWriter] = None)
    ...
    val references: Array[ReferenceGenome] =
      if (hasReferences)
...
class EmitMethodBuilder[C](
  ...
  if (nCodeArgs > 255)
    throw new RuntimeException(
      s"invalid method ${mb.methodName}: $nCodeArgs code arguments:" +
  ...
  def emitWithBuilder[T](f: (EmitCodeBuilder) => Code[T]): Unit = emit(
    EmitCodeBuilder.scopedCode[T](this)(f)
  )

  def voidWithBuilder(f: (EmitCodeBuilder) => Unit): Unit = emit(
    EmitCodeBuilder.scopedVoid(this)(f)
  )
...
class EmitFunctionBuilder[F](val apply_method: EmitMethodBuilder[F])
