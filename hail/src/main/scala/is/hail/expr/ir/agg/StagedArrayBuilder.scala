...
class StagedArrayBuilder(
  ...
  def serialize(codec: BufferSpec): (EmitCodeBuilder, Value[OutputBuffer]) => Unit = {
    ...
    (cb: EmitCodeBuilder, ob: Value[OutputBuffer]) => {
      ...
      codecSpec.encodedType
        .buildEncoder(eltArray.sType, kb)
  ...
  def deserialize(codec: BufferSpec): (EmitCodeBuilder, Value[InputBuffer]) => Unit = {
    ...
    (cb: EmitCodeBuilder, ib: Value[InputBuffer]) => {
      ...
      val decValue = codecSpec.encodedType
        .buildDecoder(eltArray.virtualType, kb)
        .apply(cb, region, ib)
  ...
  def setMissing(cb: EmitCodeBuilder): Unit = incrementSize(
    cb
  ) // all elements set to missing on initialization
