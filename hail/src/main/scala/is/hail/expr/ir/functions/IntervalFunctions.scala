...
object IntervalFunctions extends RegistryFunctions {
  ...
  def _partitionIntervalEndpointCompare(
    ...
    (0 until (lStruct.st.size min rStruct.st.size)).foreach { idx =>
      ...
      cb.assign(
        result,
        cb.emb.ecb
          .getOrderingFunction(lField.st, rField.st, CodeOrdering.Compare())
  ...
  def registerAll(): Unit = {
    ...
    registerSCode1("includesStart", TInterval(tv("T")), TBoolean, (_: Type, x: SType) => SBoolean) {
      case (r, cb, rt, interval: SIntervalValue, _) => primitive(interval.includesStart)
    }

    registerSCode1("includesEnd", TInterval(tv("T")), TBoolean, (_: Type, x: SType) => SBoolean) {
    ...
    registerSCode2(
      ...
      new SStackStructValue(
        ...
        FastSeq(EmitValue.present(primitive(start)), EmitValue.present(primitive(end))),
      )
    }
