...
object Simplify {
  ...
  private[this] def hasMissingStrictChild(x: IR): Boolean = {
    x match {
      case _: Apply | _: ApplySeeded | _: ApplyUnaryPrimOp | _: ApplyBinaryPrimOp | _: ArrayRef |
          _: ArrayLen | _: GetField | _: GetTupleElement =>
        x.children.exists(_.isInstanceOf[NA])
  ...
  private[this] def isDefinitelyDefined(x: IR): Boolean = {
    x match {
      case _: MakeArray | _: MakeStruct | _: MakeTuple | _: IsNA |
          ApplyComparisonOp(EQWithNA(_, _), _, _) | ApplyComparisonOp(NEQWithNA(_, _), _, _) |
  ...
  private[this] def valueRules(ctx: ExecuteContext): PartialFunction[IR, IR] = {
    ...
    case Let(Let.Insert(before, (name, x @ InsertFields(old, newFields, _)) +: after), body)
      ...
      def rewrite(ir1: IR): IR = ir1 match {
        case GetField(Ref(`name`, _), fd) =>
          newFieldRefs.get(fd) match {
    ...
    case x @ InsertFields(SelectFields(struct, selectFields), insertFields, _)
        if insertFields.exists { case (name, f) => f == GetField(struct, name) } =>
  ...
  private[this] def tableRules(ctx: ExecuteContext): PartialFunction[TableIR, TableIR] = {
        ...
        case _ =>
          ...
          Let(
            ...
            MakeStruct(
              FastSeq(
                "rows" -> ToArray(
                  StreamFilter(ToStream(GetField(Ref(uid, rowsAndGlobal.typ), "rows")), "row", pred)
                ),
                "global" -> Ref(
                  "global",
    ...
    case MatrixRowsTable(MatrixFilterRows(child, pred)) =>
      ...
      TableFilter(mrt, Subst(pred, BindingEnv(Env("va" -> Ref("row", mrt.typ.rowType)))))
    ...
    case MatrixColsTable(MatrixMapCols(child, newRow, newKey))
      ...
      TableMapRows(mct, Subst(newRow, BindingEnv(Env("sa" -> Ref("row", mct.typ.rowType)))))
      ...
      val te =
        TableExplode(
          TableKeyByAndAggregate(
            child,
            MakeStruct(
              FastSeq("row" -> ApplyAggOp(FastSeq(I32(n.toInt)), Array(row, keyStruct), aggSig))
            ),
    ...
    case TableKeyByAndAggregate(child, expr, newKey, _, _)
        if (newKey == MakeStruct(
          child.typ.key.map(k => k -> GetField(Ref("row", child.typ.rowType), k))
        ) ||
    ...
    case MatrixMapCols(MatrixMapCols(child, newCol1, nk1), newCol2, nk2)
      ...
      MatrixMapCols(
        ...
        bindIR(newCol1) { uid =>
          Subst(
            newCol2,
            BindingEnv[IR](Env("sa" -> uid), agg = Some(Env.empty[IR]), scan = Some(Env.empty[IR])),
          )
        },
        nk2.orElse(nk1),
      )
