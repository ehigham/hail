...
class CallStatsState(val kb: EmitClassBuilder[_]) extends PointerBasedRVAState {
  ...
  def serialize(codec: BufferSpec): (EmitCodeBuilder, Value[OutputBuffer]) => Unit = { (cb, ob) =>
    ...
    codecSpec.encodedType
      .buildEncoder(CallStatsState.stateType.sType, kb)
  ...
  def deserialize(codec: BufferSpec): (EmitCodeBuilder, Value[InputBuffer]) => Unit = {
    (cb: EmitCodeBuilder, ib: Value[InputBuffer]) =>
      ...
      val decValue = codecSpec.encodedType
        .buildDecoder(CallStatsState.stateType.virtualType, kb)
        .apply(cb, region, ib)
  ...
  def copyFromAddress(cb: EmitCodeBuilder, src: Value[Long]): Unit = {
    cb.assign(
      off,
      CallStatsState.stateType
        .store(cb, region, CallStatsState.stateType.loadCheapSCode(cb, src), deepCopy = true),
    )
    loadNAlleles(cb)
  }
}
...
class CallStatsAggregator extends StagedAggregator {
      ...
      .consume(
        ...
        { sc =>
          ...
          cb.assign(
            state.off,
            state.region
              .allocate(CallStatsState.stateType.alignment, CallStatsState.stateType.byteSize),
          )
        ...
        { case call: SCallValue =>
          ...
          call.forEachAllele(cb) { allele: Value[Int] =>
            cb.if_(
              ...
              cb._fatal(
                const("hl.agg.call_stats: found allele outside of expected range [0, ")
  ...
  protected def _result(cb: EmitCodeBuilder, state: State, region: Value[Region]): IEmitCode = {
    ...
    val homCount = homCountType.constructFromElements(cb, region, state.nAlleles, deepCopy = true) {
      (cb, i) =>
