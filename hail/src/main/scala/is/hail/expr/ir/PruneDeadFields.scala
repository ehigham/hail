          ...
          t match {
            ...
            case _ =>
              if (!children.forall(_.asInstanceOf[Type] == t)) {
                val badChildren = children
                  .filter(c => c.asInstanceOf[Type] != t)
  ...
  def memoizeTableIR(
    ...
    tir match {
      ...
      case TableJoin(left, right, _, joinKey) =>
        val lk = unifyKey(
          FastSeq(requestedType.key.take(left.typ.key.length), left.typ.key.take(joinKey))
        )
        val lkSet = lk.toSet
        val leftDep = TableType(
          key = lk,
          rowType = TStruct(
            left.typ.rowType.fieldNames.flatMap(f =>
              if (lkSet.contains(f))
          ...
          globalType = TStruct(
            left.typ.globalType.fieldNames.flatMap(f =>
        ...
        val rightDep = TableType(
          key = rk,
          rowType = TStruct(
            right.typ.rowType.fieldNames.flatMap(f =>
          ...
          globalType = TStruct(
            right.typ.globalType.fieldNames.flatMap(f =>
              requestedType.globalType.selfField(f).map(reqF => f -> reqF.typ)
            ): _*
          ),
        )
      ...
      case TableMultiWayZipJoin(children, fieldName, globalName) =>
        val gType = requestedType.globalType
          .selfField(globalName)
          .map(_.typ.asInstanceOf[TArray].elementType)
          .getOrElse(TStruct.empty).asInstanceOf[TStruct]
        val rType = requestedType.rowType
          .selfField(fieldName)
          .map(_.typ.asInstanceOf[TArray].elementType)
          .getOrElse(TStruct.empty).asInstanceOf[TStruct]
        ...
        val dep = TableType(
          key = child1.typ.key,
          rowType = TStruct(
            child1.typ.rowType.fieldNames.flatMap(f =>
      ...
      case CastMatrixToTable(child, entriesFieldName, colsFieldName) =>
        val childDep = MatrixType(
          ...
          colKey = FastSeq(),
          globalType =
            if (requestedType.globalType.hasField(colsFieldName))
          ...
          colType =
            if (requestedType.globalType.hasField(colsFieldName))
              requestedType.globalType
                .field(colsFieldName).typ.asInstanceOf[TArray].elementType.asInstanceOf[TStruct]
            else
              TStruct.empty,
          entryType =
            if (requestedType.rowType.hasField(entriesFieldName))
              requestedType.rowType
                .field(entriesFieldName).typ.asInstanceOf[TArray].elementType.asInstanceOf[TStruct]
            else
              TStruct.empty,
          rowType =
            if (requestedType.rowType.hasField(entriesFieldName))
    ...
    mir match {
      ...
      case MatrixCollectColsByKey(child) =>
        ...
        val explodedDep = requestedType.copy(
          ...
          entryType = TStruct(
            requestedType.entryType.fields.map(f =>
      ...
      case MatrixAnnotateColsTable(child, table, uid) =>
        ...
        fieldDep match {
          case Some(struct) =>
            ...
            val mk = unifyKey(
              FastSeq(child.typ.colKey.take(table.typ.key.length), requestedType.colKey)
            )
            val matDep = requestedType.copy(
              colKey = mk,
              colType = unify(
      ...
      case MatrixUnionRows(children) =>
        ...
        children.tail.foreach(
          memoizeMatrixIR(ctx, _, requestedType.copy(colType = requestedType.colKeyStruct), memo)
        )
  ...
  def memoizeValueIR(
    ...
    ir match {
      ...
      case StreamWhiten(a, newChunk, prevWindow, _, _, _, _, _) =>
        ...
        unifyEnvs(memoizeValueIR(ctx, a, TStream(unifiedStructType), memo))
      ...
      case StreamZip(as, names, body, behavior, _) =>
        ...
        if (behavior == ArrayZipBehavior.AssumeSameLength && valueTypes.forall(_.isEmpty)) {
          ...
          unifyEnvs((as, valueTypes).zipped.map { (a, vtOption) =>
            val at = tcoerce[TStream](a.typ)
            if (behavior == ArrayZipBehavior.AssumeSameLength) {
              vtOption
                .map(vt => memoizeValueIR(ctx, a, TStream(vt), memo)).getOrElse(BindingEnv.empty)
            } else
      ...
      case MakeTuple(fields) =>
        ...
        unifyEnvsSeq(fields.flatMap { case (i, value) =>
          // ignore unreachable fields, these are eliminated on the upwards pass
          tType.fieldIndex
            .get(i)
    ...
    tir match {
      ...
      case TableMapPartitions(child, gName, pName, body, requestedKey, allowedOverlap) =>
        ...
        val body2 = rebuildIR(
          ctx,
          body,
          BindingEnv(Env(gName -> child2.typ.globalType, pName -> TStream(child2.typ.rowType))),
          memo,
        )
        val body2ElementType = TIterable.elementType(body2.typ).asInstanceOf[TStruct]
        val child2Keyed =
          if (child2.typ.key.exists(k => !body2ElementType.hasField(k)))
  ...
  def rebuild(
    ...
    mir match {
      ...
      case MatrixMapRows(child, newRow) =>
        ...
        val newRowType = newRow2.typ.asInstanceOf[TStruct]
        val child2Keyed =
          if (child2.typ.rowKey.exists(k => !newRowType.hasField(k)))
      ...
      case MatrixMapCols(child, newCol, newKey) =>
        ...
        val newKey2 = newKey match {
          ...
          case None =>
            if (child2.typ.colKey.exists(k => !newColType.hasField(k)))
  ...
  def rebuildIR(
    ...
    ir match {
      ...
      case StreamZip(as, names, body, b, errorID) =>
        val (newAs, newNames) = (as, names).zipped.flatMap { case (a, name) =>
      ...
      case StreamMultiMerge(as, key) =>
        ...
        val newAs = as.map(a => rebuildIR(ctx, a, env, memo))
        val newAs2 =
          if (newAs.forall(_.typ == newAs(0).typ))
            newAs
          else
      ...
      case NDArrayMap2(left, right, leftName, rightName, body, errorID) =>
        ...
        val body2 = rebuildIR(
          ...
          body,
          env
            .bindEval(leftName, left2.typ.asInstanceOf[TNDArray].elementType).bindEval(
              rightName,
      ...
      case InsertFields(old, fields, fieldOrder) =>
        ...
        val wrappedChild = if (insertOverwritesUnrequestedButPreservedField) {
          ...
          SelectFields(
            rebuiltChild,
            rebuiltChild.typ
              .asInstanceOf[TStruct].fieldNames.filter(selectedChildFields.contains(_)),
          )
        } else {
          rebuiltChild
        }
      ...
      case ApplyAggOp(initOpArgs, seqOpArgs, aggSig) =>
        ...
        ApplyAggOp(
          initOpArgs2,
          seqOpArgs2,
          aggSig.copy(initOpArgs = initOpArgs2.map(_.typ), seqOpArgs = seqOpArgs2.map(_.typ)),
        )
