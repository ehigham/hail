...
abstract class NDArrayProducer {
  ...
  def toSCode(
    ...
    initAll(cb)
    val idxGenerator =
      if (rowMajor) SNDArray.forEachIndexWithInitAndIncRowMajor _
...
object EmitNDArray {
        ...
        x match {
            ...
            deforestRecur(lChild, cb).flatMap(cb) { leftProducer =>
              deforestRecur(rChild, cb).map(cb) { rightProducer =>
                ...
                new NDArrayProducer {
                  ...
                  override val initAxis: IndexedSeq[EmitCodeBuilder => Unit] = shape.indices.map {
                    idx => cb: EmitCodeBuilder =>
          ...
          case x @ NDArrayReshape(childND, shape, errorID) =>
            emitI(childND, cb).flatMap(cb) { case childND: SNDArrayValue =>
              ...
              val requestedShapeValues = Array
                .tabulate(outputNDims)(i =>
                        ...
                        (cb: EmitCodeBuilder, step: Value[Long]) => {
                          ...
                          if (idx == axis) {
                            ...
                            val shouldLoop = cb.newLocal[Boolean](
                              "should_loop",
                              curIdxVar >= stagedArrayOfSizes
                                .loadElement(cb, currentNDArrayIdx).get(cb).asInt64.value,
                            )
                            cb.while_(
                              shouldLoop, {
                                cb.assign(
                      ...
                      cb.if_(
                        filterWasMissing(idx), {
                          ...
                          val startPoint = cb.newLocal[Long](
                            ...
                            filtPValues(idx)
                              .loadElement(cb, idxVars(idx).toI).get(
                                cb,
                  ...
                  shape.indices.map { idx => (cb: EmitCodeBuilder, step: Value[Long]) =>
                    {
                      cb.if_(
                        filterWasMissing(idx), {
                          ...
                          val currentPos = filtPValues(idx)
                            .loadElement(cb, idxVars(idx).toI).get(
                              cb,
                              s"NDArrayFilter: can't filter on missing index (axis=$idx)",
                            ).asLong.value
                          cb.assign(idxVars(idx), idxVars(idx) + step)
                          val newPos = filtPValues(idx)
                            .loadElement(cb, idxVars(idx).toI).get(
                              cb,
                              s"NDArrayFilter: can't filter on missing index (axis=$idx)",
                            ).asLong.value
  ...
  def fromShapeStridesFirstAddress(
    ...
    new NDArrayProducer {
      ...
      override val initAxis: IndexedSeq[EmitCodeBuilder => Unit] =
        shape.indices.map(i => (cb: EmitCodeBuilder) => cb.assign(counters(i), 0L))
      override val stepAxis: IndexedSeq[(EmitCodeBuilder, Value[Long]) => Unit] =
