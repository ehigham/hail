...
class ExtractIntervalFilters(ctx: ExecuteContext, keyType: TStruct) {
  ...
  object AbstractLattice extends Lattice {
    ...
    def join(l: Value, r: Value): Value = (l, r) match {
      ...
      case (ConcreteStruct(l), ConcreteStruct(r)) =>
        StructValue(l.keySet.intersect(r.keySet).view.map(f => f -> join(l(f), r(f))).toMap)
    ...
    def meet(l: Value, r: Value): Value = (l, r) match {
      ...
      case (l: ConcreteStruct, r: ConcreteStruct) =>
        StructValue(
          l.fields.keySet.union(r.fields.keySet).view.map(f => f -> meet(l(f), r(f))).toMap
        )
      case (l: BoolValue, r: BoolValue) =>
        BoolValue(
          KeySetLattice.meet(l.trueBound, r.trueBound),
  ...
  private def intervalsFromCollection(
    ...
    KeySet.reduce(
      lit.toArray.distinct
        .filter(x => wrapped || x != null).sorted(ordering)
  ...
  private def intervalsFromLiteralContigs(contigs: Any, rg: ReferenceGenome): KeySet = {
    KeySet((contigs: @unchecked) match {
      case x: Map[_, _] =>
        x.keys
          .asInstanceOf[Iterable[String]].toFastSeq
      ...
      case x: Traversable[_] =>
        x.asInstanceOf[Traversable[String]].toArray.toFastSeq
          .sortBy(rg.contigsIndex.get(_))(
            TInt32.ordering(null).toOrdering.asInstanceOf[Ordering[Integer]]
          )
          .flatMap(getIntervalFromContig(_, rg))
  ...
  def analyze(
    ...
    val env = Env
      .empty[AbstractValue].bind(
        rowName,
        StructValue(Map(keyType.fieldNames.zipWithIndex.map(t => t._1 -> KeyField(t._2)): _*)),
      )
      ...
      if (collectionVal == null) {
        ...
        queryVal match {
          case Contig(rgStr) =>
            ...
            BoolValue(intervals, KeySetLattice.complement(intervals), KeySetLattice.bottom)
              .restrict(keySet)
          case KeyField(0) =>
  ...
  private def _analyze(x: IR, env: AbstractEnv, rewrites: Option[Rewrites]): AbstractValue = {
    ...
    var res: AbstractLattice.Value =
      if (env.keySet == KeySetLattice.bottom)
        AbstractLattice.bottom
      else
        x match {
          ...
          case MakeStruct(fields) =>
            StructValue(fields.view.map { case (name, field) =>
              name -> recur(field)
            })
          ...
          case Switch(y_, default_, cases_) =>
            recur(y_) match {
              ...
              case _ =>
                val res =
                  cases_.foldLeft(recur(default_))((e, c) => AbstractLattice.join(e, recur(c)))
                if (x.typ == TBoolean)
                  AbstractLattice.join(
                    res,
