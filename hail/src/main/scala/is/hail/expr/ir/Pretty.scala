...
//    case Ref(name, t) if t != null => FastSeq(prettyIdentifier(name), t.parsableString())  // For debug purposes
    ...
    case ApplyIR(function, typeArgs, _, _, errorID) =>
      FastSeq(
        s"$errorID",
    ...
    case MatrixWrite(_, writer) =>
      single(
        '"' + StringEscapeUtils.escapeString(
    ...
    case BlockMatrixFilter(_, indicesToKeepPerDim) =>
      single(
        fillList(indicesToKeepPerDim.toSeq.view.map(indices => prettyLongs(indices, elideLiterals)))
      )
    ...
    case TableMapPartitions(_, g, p, _, requestedKey, allowedOverlap) =>
      FastSeq(
        prettyIdentifier(g),
        prettyIdentifier(p),
    ...
    case MatrixFilterIntervals(child, intervals, keep) =>
      FastSeq(
        ...
        prettyStringLiteral(
          Serialization.write(
            JSONAnnotationImpex
              .exportAnnotation(intervals, TArray(TInterval(child.typ.rowKeyStruct)))
          )(RelationalSpec.formats)
        ),
        Pretty.prettyBooleanLiteral(keep),
      )
  ...
  def ssaStyle(ir: BaseIR): String = {
    ...
    def blockArgs(ir: BaseIR, i: Int): Option[IndexedSeq[(String, String)]] = ir match {
      ...
      case _: Switch =>
        if (i > 0) Some(FastSeq()) else None
      case TailLoop(name, args, _, body) =>
        if (i == args.length)
          Some(
            args.map { case (name, ir) => name -> "loopvar" } :+
              name -> "loop"
          )
        else None
    ...
    def pretty(ir: BaseIR, bindings: Env[String]): (Doc, Doc) = ir match {
      ...
      case _ =>
        ...
        val attributes =
          if (attsIterable.isEmpty) Iterable.empty
          else
        ...
        val head = ir match {
          case MakeStruct(fields) =>
            val args = (fields.map(_._1), strictChildIdents).zipped
              .map((field, value) => s"$field: $value").mkString("(", ", ", ")")
            hsep(text(Pretty.prettyClass(ir) + args) +: (attributes ++ nestedBlocks))
          case InsertFields(_, fields, _) =>
            val newFields = (fields.map(_._1), strictChildIdents.tail).zipped
              .map((field, value) => s"$field: $value").mkString("(", ", ", ")")
