...
object LinearRegressionAggregator {
  ...
  def computeResult(region: Region, xtyPtr: Long, xtxPtr: Long, k0: Int): Long = {
    ...
    try {
      ...
      case _: breeze.linalg.MatrixSingularException | _: breeze.linalg.NotConvergedException =>
        rvb.setMissing()
        rvb.setMissing()
        rvb.setMissing()
        rvb.setMissing()
...
class LinearRegressionAggregator() extends StagedAggregator {
  ...
  def initOpF(state: State)(cb: EmitCodeBuilder, kc: Code[Int], k0c: Code[Int]): Unit = {
    ...
    cb.if_(
      (k0 < 0) || (k0 > k),
      cb += Code._fatal[Unit](
        const("linreg: `nested_dim` must be between 0 and the number (")
          .concat(k.toS)
                  ...
                  j < k, {
                    ...
                    cb += Region.storeDouble(
                      ...
                      Region.loadDouble(sptr) +
                        (x.loadElement(cb, i).get(cb).asDouble.value * x
                          .loadElement(cb, j).get(cb).asDouble.value),
                    )
                    cb.assign(j, j + 1)
                    cb.assign(sptr, sptr + scalar.byteSize)
                  },
  ...
  protected def _seqOp(cb: EmitCodeBuilder, state: State, seq: Array[EmitCode]): Unit = {
    ...
    y.toI(cb)
      .consume(
        ...
        yCode =>
          x.toI(cb)
            .consume(cb, {}, xCode => seqOpF(state)(cb, yCode.asDouble.value, xCode.asIndexable)),
      )
  }
