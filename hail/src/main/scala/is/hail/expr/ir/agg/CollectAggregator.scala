...
class CollectAggState(val elemVType: VirtualTypeWithReq, val kb: EmitClassBuilder[_])
  ...
  def serialize(codec: BufferSpec): (EmitCodeBuilder, Value[OutputBuffer]) => Unit = { (cb, ib) =>
    bll.serialize(cb, region, ib)
  }

  def deserialize(codec: BufferSpec): (EmitCodeBuilder, Value[InputBuffer]) => Unit = { (cb, ib) =>
    bll.init(cb, region)
