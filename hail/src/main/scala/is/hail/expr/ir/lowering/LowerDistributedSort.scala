...
object LowerDistributedSort {
  def localSort(
    ...
    val (Some(PTypeReferenceSingleCodeType(resultPType: PStruct)), f) =
      ctx.timer.time("LowerDistributedSort.localSort.compile")(
    ...
    val rowsAndGlobal = ctx.scopedExecution { (hcl, fs, htc, r) =>
      ...
      ctx.timer
        .time("LowerDistributedSort.localSort.toJavaObject")(
          SafeRow.read(resultPType, resultAddress)
        ).asInstanceOf[Row]
    }
    ...
    val partitionerKeyType = TStruct(
      sortFields
        .takeWhile(_.sortOrder == Ascending).map(f =>
    ...
    val partitioner = new RVDPartitioner(
      ...
      partitionerKeyType,
      sortedRows
        .grouped(itemsPerPartition).map { group =>
  ...
  case class LocalSortReader(
    ...
    override def lower(ctx: ExecuteContext, requestedType: TableType): TableStage = {
      TableStage(
        ...
        contexts = mapIR(
          StreamGrouped(ToStream(Literal(TArray(rowType), sortedRows)), I32(itemsPerPartition))
        )(ToArray(_)),
  ...
  def distributedSort(
    ...
    while (!loopState.largeSegments.isEmpty) {
      ...
      val perPartStatsCDAContexts = ToStream(
        Literal(
          TArray(
            TStruct(
              "segmentIdx" -> TInt32,
      ...
      val pivotsPerSegmentAndSortedCheck = ToArray(bindIR(perPartStatsIR) { perPartStats =>
        mapIR(
          StreamGroupByKey(ToStream(perPartStats), IndexedSeq("segmentIdx"), missingEqual = true)
        ) { oneGroup =>
          ...
          bindIR(
                  ...
                  AggLet(
                    ...
                    MakeStruct(
                      FastSeq(
                        ...
                        (
                          "min",
                          AggFold.min(GetField(dataRef, "min"), sortFields),
                        ), // Min of the mins
                        (
                          "max",
                          AggFold.max(GetField(dataRef, "max"), sortFields),
                        ), // Max of the maxes
                        (
                          "perPartMins",
                        ...
                        (
                          ...
                          ApplyAggOp(Collect())(
                            MakeTuple.ordered(
                              FastSeq(GetField(dataRef, "min"), GetField(dataRef, "max"))
                            )
                          ),
                        ),
                      )
            ...
            bindIR(sortedOversampling) { sortedOversampling =>
              bindIR(ArrayLen(sortedOversampling)) { numSamples =>
                ...
                MakeStruct(
                  FastSeq(
                    "pivotsWithEndpoints" -> ArrayFunctions
                      .extend(ArrayFunctions.extend(minArray, sortedSampling), maxArray),
                    ...
                    "intervalTuple" -> MakeTuple.ordered(
                      FastSeq(GetField(aggResults, "min"), GetField(aggResults, "max"))
                    ),
      ...
      val pivotsWithEndpointsAndInfoGroupedBySegmentNumber =
        CompileAndEvaluate[Annotation](ctx, pivotsPerSegmentAndSortedCheck)
      ...
      val outputPartitions = sortedSegmentsTuples.flatMap {
        case ((_, _, _, partMins, partMaxes), originalSegmentIdx) =>
      ...
      val (newBigUnsortedSegments, newSmallSegments) =
        if (unsortedPivotsWithEndpointsAndInfoGroupedBySegmentNumber.size > 0) {
          ...
          val distributeContexts = ToStream(
            Literal(
              TArray(
                TStruct(
                  "segmentIdx" -> TInt32,
          ...
          val distributeGlobals = MakeStruct(
            IndexedSeq(
          ...
          val distributeResult = CompileAndEvaluate[Annotation](ctx, distribute)
            .asInstanceOf[IndexedSeq[Row]].map(row =>
              (
                ...
                row(1)
                  .asInstanceOf[IndexedSeq[Row]].map(innerRow =>
                    (
          ...
          dataPerSegment.partition { sr =>
            ...
            isBig && (sr.interval.left.point != sr.interval.right.point) && (sr.chunks
              .map(_.size).sum > 1)
          }
    ...
    val sortedFilenames = CompileAndEvaluate[Annotation](ctx, sortedFilenamesIR)
      .asInstanceOf[IndexedSeq[Row]].map(_(0).asInstanceOf[String])
  ...
  def samplePartition(dataStream: IR, sampleIndices: IR, sortFields: IndexedSeq[SortField]): IR = {
    ...
    val aggFoldSortedZero = MakeStruct(
      FastSeq("lastKeySeen" -> NA(eltType), "sortedSoFar" -> true, "haveSeenAny" -> false)
    )
    ...
    val isSortedSeq =
      bindIR(GetField(aggFoldSortedAccumRef1, "lastKeySeen")) { lastKeySeenRef =>
        If(
          ...
          MakeStruct(
            FastSeq("lastKeySeen" -> eltRef, "sortedSoFar" -> true, "haveSeenAny" -> true)
          ),
          If(
            ...
            MakeStruct(
              FastSeq("lastKeySeen" -> eltRef, "sortedSoFar" -> false, "haveSeenAny" -> true)
            ),
          ),
        )
      }
...
case class DistributionSortReader(
  ...
  override def lower(ctx: ExecuteContext, requestedType: TableType): TableStage = {
    ...
    val contexts = ToStream(
      Literal(
        TArray(
          TStruct(
            "files" -> TArray(TStruct("partitionIndex" -> TInt64, "partitionPath" -> TString))
          )
        ),
        contextData,
      )
