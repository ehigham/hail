  ...
  def registerAll() {
    ...
    registerSCode5(
      ...
      case (
        ...
        result.coiterateMutate(cb, r.region) { _ =>
          primitive(
            cb.memoize(
              rng.invoke[Double, Double, Double]("runif", min.asDouble.value, max.asDouble.value)
            )
          )
        }
        result
    ...
    registerSCode5(
      ...
      case (
        ...
        result.coiterateMutate(cb, r.region) { _ =>
          primitive(
            cb.memoize(
              rng.invoke[Double, Double, Double]("rnorm", mean.asDouble.value, sd.asDouble.value)
            )
          )
        }
        result
    ...
    registerSCode5(
      ...
      case (
        ...
        rngState.copyIntoEngine(cb, rng)
        val value = cb
          .newLocal[Double]("value", rng.invoke[Double, Double, Double]("rbeta", a.value, b.value))
        cb.while_(
    ...
    registerSCode3(
      ...
      case (
        ...
        val (push, finish) = arrayRt.pType
          .asInstanceOf[PCanonicalArray].constructFromFunctions(cb, r.region, resultSize, false)

        cb.for_(
          cb.assign(i, 0),
          i < resultSize,
          cb.assign(i, i + 1), {
            val numSuccesses = cb.memoize(
              rng
            ...
            cb.assign(
              ...
              failureStatesRemaining - (partitionCounts
                .loadElement(cb, i).get(cb).asInt32.value - numSuccesses),
            )
