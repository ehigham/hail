  ...
  def apply(x: IR, newChildren: IndexedSeq[BaseIR]): IR = {
    x match {
      ...
      case Let(bindings, _) =>
        ...
        val newBindings =
          (bindings, newChildren.init).zipped
      ...
      case x @ ApplyScanOp(initOpArgs, _, aggSig) =>
        val args = newChildren.map(_.asInstanceOf[IR])
        assert(args.length == x.nInitArgs + x.nSeqOpArgs)
        ApplyScanOp(args.take(x.nInitArgs), args.drop(x.nInitArgs), aggSig)
      ...
      case MakeTuple(fields) =>
        assert(fields.length == newChildren.length)
        MakeTuple(
          fields.zip(newChildren).map { case ((i, _), newValue) => (i, newValue.asInstanceOf[IR]) }
        )
