...
class NDArraySumAggregator(ndVTyp: VirtualTypeWithReq) extends StagedAggregator {
  ...
  override protected def _initOp(cb: EmitCodeBuilder, state: State, init: Array[EmitCode]): Unit = {
    ...
    initMethod.voidWithBuilder(cb => state.storeMissing(cb))
      ...
      nextNDInput
        .toI(cb).consume(
          ...
          { case nextNDPV: SNDArrayValue =>
            ...
            statePV
              .loadField(cb, ndarrayFieldNumber).consume(
                cb, {
...
object NDArraySumAggregator {
  ...
  def addValues(
    ...
    leftNdValue.coiterateMutate(cb, region, (rightNdValue, "right")) { case Seq(l, r) =>
