...
object IRFunctionRegistry {
  ...
  def pyRegisterIRForServiceBackend(
    ...
    val body = IRParser.parse_value_ir(bodyStr, IRParserEnvironment(ctx, Map()), refMap)
  ...
  def lookupFunctionOrFail(
    ...
    jvmRegistry.lift(name) match {
      ...
      case Some(functions) =>
        functions
          .filter(t => t.unify(typeParameters, valueParameterTypes, returnType)).toSeq match {
            case Seq() =>
  ...
  def lookupIR(
    ...
    irRegistry
      .getOrElse(name, Map.empty).filter {
  ...
  def lookupUnseeded(
    ...
    val validIR: Option[IRFunctionImplementation] =
      lookupIR(name, returnType, typeParameters, arguments).map {
        case ((_, _, _, inline), conversion) =>
          (typeParametersPassed, args, errorID) =>
  ...
  Seq(
    ArrayFunctions, NDArrayFunctions, CallFunctions, DictFunctions, GenotypeFunctions,
    IntervalFunctions, LocusFunctions, MathFunctions, RandomSeededFunctions, SetFunctions,
    StringFunctions, UtilFunctions, ExperimentalFunctions, ReferenceGenomeFunctions, BGENFunctions,
    ApproxCDFFunctions,
  ...
  def dumpFunctions(): Unit = {
    ...
    jvmRegistry.foreach { case (name, fns) =>
      fns.foreach { f =>
        println(s"""register_function("${StringEscapeUtils.escapeString(name)}", (${f.typeParameters
            .map(dtype).mkString(",")}), (${f.valueParameterTypes
            .map(dtype).mkString(",")}), ${dtype(f.returnType)})""")
      }
    }
  }
}
...
abstract class RegistryFunctions {
  ...
  def unwrapReturn(cb: EmitCodeBuilder, r: Value[Region], st: SType, value: Code[_]): SValue =
    st.virtualType match {
      ...
      case _: TBaseStruct =>
        ...
        val addr = cb.memoize(
          Code.invokeScalaObject4[Map[String, ReferenceGenome], Region, Row, PType, Long](
      ...
      case TArray(_: TBaseStruct) =>
        val ast = st.asInstanceOf[SIndexablePointer]
        val pca = ast.pType.asInstanceOf[PCanonicalArray]
        val array = cb.memoize(
          Code.invokeScalaObject4[Map[String, ReferenceGenome], Region, IndexedSeq[
            Annotation
          ], PType, Long](
            RegistryHelpers.getClass,
            "stupidUnwrapArray",
  ...
  def registerCode1(
    ...
    registerCode(name, Array(mt1), rt, unwrappedApply(pt)) { case (r, cb, rt, _, Array(a1)) =>
      impl(cb, r, rt, a1)
    }
