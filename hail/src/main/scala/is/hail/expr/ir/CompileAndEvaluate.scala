...
object CompileAndEvaluate {
  ...
  def _apply(
    ...
    val (Some(PTypeReferenceSingleCodeType(resType: PTuple)), f) = ctx.timer.time("Compile")(
    ...
    val fRunnable = ctx.timer.time("InitializeCompiledFunction")(
      f(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, ctx.r)
    )
