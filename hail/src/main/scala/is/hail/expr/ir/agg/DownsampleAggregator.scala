...
class DownsampleBTreeKey(
  binType: PBaseStruct,
  pointType: PBaseStruct,
...
class DownsampleState(
    ...
    val mb = kb.getOrGenEmitMethod(name, (this, name), FastSeq[ParamType](), UnitInfo) { mb =>
      ...
      mb.voidWithBuilder { cb =>
        ...
        cb.while_(
          i < buffer.size, {
            buffer
              .loadElement(cb, i).toI(cb).consume(
                cb,
                {},
        ...
        l.toI(cb)
          .consume(
            ...
            { sc =>
              ...
              pointType
                .fieldType("label").storeAtAddress(
                  cb,
  ...
  def deepCopyAndInsertPoint(cb: EmitCodeBuilder, point: Value[Long]): Unit = {
    ...
    val mb = kb.getOrGenEmitMethod(name, (this, name), IndexedSeq[ParamType](LongInfo), UnitInfo) {
      mb =>
...
class DownsampleAggregator(arrayType: VirtualTypeWithReq) extends StagedAggregator {
  ...
  val resultPType: PCanonicalArray = PCanonicalArray(
    PCanonicalTuple(required = true, PFloat64(true), PFloat64(true), arrayType.canonicalPType)
  )
