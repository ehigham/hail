...
object StringFunctions extends RegistryFunctions {
      ...
      separator match {
        ...
        case Right(regex) =>
          val m = cb.newLocal[Matcher](
            "matcher",
            Code
              .invokeStatic1[Pattern, String, Pattern]("compile", regex)
  ...
  def strftime(fmtStr: String, epochSeconds: Long, zoneId: String): String =
    DateFormatUtils
      .parseDateFormat(fmtStr, locale).withZone(ZoneId.of(zoneId))
  ...
  def registerAll(): Unit = {
    ...
    registerIEmitCode2(
      ...
      trunc.toI(cb).map(cb) { trunc =>
        ...
        val str = cb.emb
          .getType(a.st.virtualType).invoke[Any, Int, String]("showStr", jObj, trunc.asInt.value)
        st.construct(cb, str)
      }
    }
            ...
            IEmitCode(
              ...
              m, {
                cb.while_(
                  i < l1, {
                    cb.if_(
                      v1.invoke[Int, Char]("charAt", i).toI.cne(
                        v2.invoke[Int, Char]("charAt", i).toI
                      ),
                      cb.assign(n, n + 1),
                    )
                    cb.assign(i, i + 1)
