...
class Requiredness(val usesAndDefs: UsesAndDefs, ctx: ExecuteContext) {
    ...
    node match {
      ...
      case TailLoop(loopName, params, _, body) =>
        ...
        while (i < params.length) {
          ...
          s(i) = lookup(
            refMap
              .get(name).flatMap(refs => refs.headOption.map(_.t.asInstanceOf[IR])).getOrElse(init)
          )
      ...
      case StreamFold2(a, accums, valueName, seq, _) =>
        ...
        while (i < accums.length) {
          ...
          s(i) = lookup(
            refMap.get(n).flatMap(refs => refs.headOption.map(_.t.asInstanceOf[IR])).getOrElse(z)
          )
          i += 1
        }
        states.bind(node, s)
  ...
  def analyzeTable(node: TableIR): Boolean = {
    ...
    node match {
      ...
      case TableExplode(child, path) =>
        ...
        while (i < path.length) {
          val explode = path(i)
          tcoerce[RStruct](newFields).fields
            .filter(f => f.name != explode)
      ...
      case TableJoin(left, right, joinType, joinKey) =>
        ...
        if (joinType == "inner")
          requiredness.key.take(joinKey).zipWithIndex.foreach { case (k, i) =>
            requiredness
              .field(k).unionWithIntersection(
                FastSeq(leftReq.field(leftReq.key(i)), rightReq.field(rightReq.key(i)))
              )
          }
      ...
      case TableIntervalJoin(left, right, root, product) =>
        ...
        requiredness.valueFields
          .filter(_ != root)
          .foreach(n => requiredness.field(n).unionFrom(lReq.field(n)))
        val joinField =
          if (product)
            requiredness.field(root).asInstanceOf[RIterable].elementType.asInstanceOf[RStruct]
  ...
  def analyzeIR(node: IR): Boolean = {
    ...
    node match {
      // union all
      case _: Cast | _: CastRename | _: ToSet | _: CastToArray | _: ToArray | _: ToStream |
          _: NDArrayReindex | _: NDArrayAgg =>
        node.children.foreach { case c: IR => requiredness.unionFrom(lookup(c)) }

      // union top-level
      case _: ApplyBinaryPrimOp | _: ApplyUnaryPrimOp | _: Consume | _: ArrayLen | _: StreamLen |
          _: ArrayZeros | _: StreamRange | _: StreamIota | _: SeqSample | _: StreamDistribute |
          _: WriteValue =>
      ...
      case _: I32 | _: I64 | _: F32 | _: F64 | _: Str | True() | False() | _: IsNA | _: Die |
          _: UUID4 | _: RNGStateLiteral | _: RNGSplit =>
      ...
      case In(_, t) =>
        t match {
          ...
          case SingleCodeEmitParamType(required, StreamSingleCodeType(_, eltType, eltRequired)) =>
            requiredness
              .asInstanceOf[RIterable].elementType.fromPType(eltType.setRequired(eltRequired))
      ...
      case TableCollect(c) =>
        tcoerce[RIterable](tcoerce[RStruct](requiredness).field("rows")).elementType
          .unionFrom(lookup(c).rowType)
