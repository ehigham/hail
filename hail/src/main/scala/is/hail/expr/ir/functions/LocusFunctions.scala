...
object LocusFunctions extends RegistryFunctions {
  ...
  def emitLocus(cb: EmitCodeBuilder, r: Value[Region], locus: Code[Locus], rt: PCanonicalLocus)
    ...
    rt.constructFromPositionAndString(
      cb,
      r,
  ...
  def registerAll() {
      ...
      case (
        ...
        cb.if_(
          ...
          cb.assign(
            newPos,
            (basePos + bps).min(
              cb.emb.getReferenceGenome(rt.rg).invoke[String, Int]("contigLength", contig)
            ),
          ),
        )
    ...
    registerSCode2(
      ...
      case (
        ...
        def forAllContigs(
          ...
          cb.while_(
            iContig < ncontigs, {
              val coordPerContig = grouped
                .loadElement(cb, iContig).get(cb, "locus_windows group cannot be missing")
                .asIndexable
    ...
    registerSCode2(
      ...
      cb += rgCode(r.mb, rt.rg)
        .invoke[String, Int, Unit]("checkLocus", contig.asString.loadString(cb), pos.asInt.value)
      rt.constructFromPositionAndString(
        cb,
        r.region,
        ...
        minMatch.toI(cb).flatMap(cb) { minMatch =>
          ...
          val negativeStrandCode = EmitCode.present(
            ...
            primitive(
              cb.memoize(
                Code
                  .checkcast[java.lang.Boolean](lifted.getField[java.lang.Object]("_2"))
          ...
          val structCode =
            rt.constructFromFields(cb, r, FastSeq(locusCode, negativeStrandCode), deepCopy = false)
    ...
    registerIEmitCode2(
      ...
      { (returnType: Type, _: EmitType, _: EmitType) =>
        val lTyp =
          returnType
            .asInstanceOf[TStruct].field("result").typ.asInstanceOf[
              TInterval
      ...
      interval.toI(cb).flatMap(cb) { interval =>
        minMatch.toI(cb).flatMap(cb) { minMatch =>
          ...
          val structCode = rt
            .constructFromFields(cb, r, FastSeq(intervalCode, negativeStrandCode), deepCopy = false)
