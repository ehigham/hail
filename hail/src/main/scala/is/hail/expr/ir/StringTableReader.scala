      ...
      SStreamValue(new StreamProducer {
        ...
        override def initialize(cb: EmitCodeBuilder, partitionRegion: Value[Region]): Unit = {
          ...
          cb.assign(
            iter,
            cb.emb
              .getObject[(FS, Any) => CloseableIterator[GenericLine]](lines.body)
...
case class StringTableReader(
  ...
  override def fullTypeWithoutUIDs: TableType =
    TableType(TStruct("file" -> TString, "text" -> TString), FastSeq.empty, TStruct())
