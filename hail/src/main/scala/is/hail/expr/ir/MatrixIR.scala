...
trait MatrixReader {
  ...
  lazy val fullMatrixType: MatrixType = {
    ...
    val newRowType =
      if (rowType.hasField(rowUIDFieldName))
        rowType
      else
    ...
    if (dropCols) {
      ...
      tr = TableMapRows(
        ...
        InsertFields(
          ...
          FastSeq(
            LowerMatrixIR.entriesFieldName -> MakeArray(FastSeq(), TArray(requestedType.entryType))
          ),
        ),
      )
      tr = TableMapGlobals(
        ...
        InsertFields(
          ...
          FastSeq(
            LowerMatrixIR.colsFieldName -> MakeArray(FastSeq(), TArray(requestedType.colType))
          ),
        ),
      )
    }
...
object MatrixNativeReader {
  ...
  def apply(fs: FS, params: MatrixNativeReaderParameters): MatrixNativeReader = {
    ...
    if (intervals.nonEmpty && !spec.indexed)
      fatal(
        """`intervals` specified on an unindexed matrix table.
...
class MatrixNativeReader(
  ...
  lazy val columnCount: Option[Int] = Some(spec.colsSpec.partitionCounts.sum.toInt)
...
case class MatrixRangeReader(
  ...
  lazy val partitionCounts: Option[IndexedSeq[Long]] = Some(
    partition(params.nRows, nPartitionsAdj).map(_.toLong)
  )
      ...
      ht = ht.mapGlobals(
        makeStruct(
          LowerMatrixIR.colsField ->
            irRange(0, nColsAdj).map(
              'i ~> makeStruct('col_idx -> 'i, Symbol(colUIDFieldName) -> 'i.toL)
            )
        )
      )
    else
...
object MatrixRead {
  def apply(
    ...
    assert(
      !reader.fullMatrixTypeWithoutUIDs.rowType.hasField(MatrixReader.rowUIDFieldName) &&
...
case class MatrixCollectColsByKey(child: MatrixIR) extends MatrixIR {
  ...
  lazy val typ: MatrixType = {
    val newColValueType = TStruct(
      child.typ.colValueStruct.fields.map(f => f.copy(typ = TArray(f.typ)))
    )
...
case class MatrixAnnotateColsTable(
  ...
  lazy val typ: MatrixType =
    child.typ.copy(colType = child.typ.colType.structInsert(table.typ.valueType, FastSeq(root)))
...
case class CastTableToMatrix(
  ...
  def copy(newChildren: IndexedSeq[BaseIR]): CastTableToMatrix = {
    assert(newChildren.length == 1)
    CastTableToMatrix(newChildren(0).asInstanceOf[TableIR], entriesFieldName, colsFieldName, colKey)
  }
