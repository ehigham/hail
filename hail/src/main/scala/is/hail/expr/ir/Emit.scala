...
case class EmitEnv(bindings: Env[EmitValue], inputValues: IndexedSeq[EmitValue]) {
  ...
  def asParams(freeVariables: Env[Unit])
    ...
    val recreateFromMB = { (cb: EmitCodeBuilder, startIdx: Int) =>
      val emb = cb.emb
      EmitEnv(
  ...
  def apply[C](
    ...
    val returnTypeOption: Option[SingleCodeType] = if (ir.typ == TVoid) {
      ...
      fb.emitWithBuilder { cb =>
        ...
        val sc = emitter
          .emitI(ir, cb, region, env, container, None).handle(
            cb,
...
object AggContainer {
  ...
  def fromVars(
    ...
    val (setup, aggState) = EmitCodeBuilder.scoped(mb) { cb =>
      ...
      cb += (off := region
        .load().allocate(aggState.storageType.alignment, aggState.storageType.byteSize))
...
object EmitValue {
  def apply(missing: Option[Value[Boolean]], v: SValue): EmitValue =
    new EmitValue(missing.filterNot(m => Code.constBoolValue(m).contains(false)), v)
...
class Emit[C](
      ...
      case x @ Begin(xs) =>
        if (
          ...
          xs.grouped(16).zipWithIndex.foreach { case (group, idx) =>
            val mb = cb.emb
              .genEmitMethod(s"begin_group_$idx", FastSeq[ParamType](classInfo[Region]), UnitInfo)
            mb.voidWithBuilder { cb =>
              group
                .foreach(x => emitVoid(x, cb, mb.getCodeParam[Region](1), env, container, loopEnv))
            }
      ...
      case DeserializeAggs(start, sIdx, spec, sigs) =>
        ...
        cb.assign(
          ...
          spec.buildCodeInputBuffer(
            Code.newInstance[ByteArrayInputStream, Array[Byte]](mb.getSerializedAgg(sIdx))
          ),
        )
  ...
  private[ir] def emitI(
    ...
    val result: IEmitCode = (ir: @unchecked) match {
        ...
        presentPC(
          pt.loadCheapSCode(
            ...
            pt.allocateAndStoreString(
              cb,
              region,
              Code.invokeScalaObject0[String](Class.forName("is.hail.expr.ir.package$"), "uuid4"),
            ),
          )
        )
      case Literal(_, v) =>
      ...
      case ArrayRef(a, i, errorID) =>
        def boundsCheck(cb: EmitCodeBuilder, index: Value[Int], len: Value[Int]): Unit = {
          val bcMb = mb.getOrGenEmitMethod(
            "arrayref_bounds_check",
            "arrayref_bounds_check",
            IndexedSeq[ParamType](IntInfo, IntInfo, IntInfo),
            UnitInfo,
          )({ mb =>
            mb.voidWithBuilder { cb =>
      ...
      case ArrayMaximalIndependentSet(edges, tieBreaker) =>
        emitI(edges).map(cb) { edgesCode =>
          val jEdges: Value[UnsafeIndexedSeq] = cb.memoize(
            Code.checkcast[UnsafeIndexedSeq](
      ...
      case x @ ToSet(a) =>
        emitStream(a, cb, region).map(cb) { case stream: SStreamValue =>
          ...
          def lessThan(cb: EmitCodeBuilder, region: Value[Region], l: Value[_], r: Value[_])
            : Value[Boolean] =
            cb.emb.ecb
              .getOrdering(sct.loadedSType, sct.loadedSType)
      ...
      case x @ ToDict(a) =>
        emitStream(a, cb, region).map(cb) { case stream: SStreamValue =>
          ...
          def skipNext(cb: EmitCodeBuilder, region: Value[Region], l: EmitCode, r: EmitCode)
            ...
            val lk = cb.memoize(l.toI(cb).flatMap(cb)(x => x.asBaseStruct.loadField(cb, 0)))

            val rk = cb.memoize(r.toI(cb).flatMap(cb)(x => x.asBaseStruct.loadField(cb, 0)))
      ...
      case GroupByKey(collection) =>
        emitStream(collection, cb, region).map(cb) { case stream: SStreamValue =>
          ...
          def sameKeyAtIndices(
            ...
            val lk = cb.memoize(sortedElts.loadFromIndex(cb, region, idx1).flatMap(cb) { x =>
              x.asBaseStruct.loadField(cb, 0)
            })

            val rk = cb.memoize(sortedElts.loadFromIndex(cb, region, idx2).flatMap(cb) { x =>
              x.asBaseStruct.loadField(cb, 0)
            })
        ...
        emitI(rowMajorIR).flatMap(cb) { isRowMajorCode =>
          emitI(shapeIR).flatMap(cb) { case shapeTupleValue: SBaseStructValue =>
            dataIR.typ match {
              case _: TArray =>
                emitI(dataIR).map(cb) { case dataValue: SIndexableValue =>
                  ...
                  cb.if_(
                    ...
                    cb._throw(
                      Code.newInstance[HailException, String, Int](
              ...
              case _: TStream =>
                EmitStream
                  .produce(this, dataIR, cb, cb.emb, region, env, container)
      ...
      case NDArrayReindex(child, indexMap) =>
        ...
        childEC.map(cb) { case sndVal: SNDArrayPointerValue =>
          ...
          newPType.constructByCopyingDataPointer(newShape, newStrides, dataPtr, cb, region)
        }
          ...
          case (leftPVal: SNDArrayValue, leftIsColumnMajor: Value[Boolean]) =>
            emitNDArrayStandardStriding(rChild).map(cb) {
              case (rightPVal: SNDArrayValue, rightIsColumnMajor: Value[Boolean]) =>
                ...
                val leftBroadcastMask =
                  if (lSType.nDims > 2) NDArrayEmitter.broadcastMask(lShape)
                  else IndexedSeq[Value[Long]]()
                val rightBroadcastMask =
                  if (rSType.nDims > 2) NDArrayEmitter.broadcastMask(rShape)
                  else IndexedSeq[Value[Long]]()
            ...
            if (mode == "r") {
              ...
              cb.if_(
                qCondition, {
                  ...
                  cb.append(
                    Region.copyFrom(hFirstElement, aAddressDORGQRFirstElement, aNumElements * 8L)
                  )
                },
      ...
      case ToArray(a) =>
        EmitStream
          .produce(this, a, cb, cb.emb, region, env, container)
          .map(cb) { case stream: SStreamValue =>
      ...
      case x @ StreamFold2(a, acc, valueName, seq, res) =>
        emitStream(a, cb, region)
          .flatMap(cb) { case stream: SStreamValue =>
            ...
            val accTypes = ctx.req
              .lookupState(x).zip(acc.map(_._2.typ))
              .map { case (btwr, t) =>
                VirtualTypeWithReq(t, btwr.asInstanceOf[TypeWithRequiredness]).canonicalEmitType
              }
      ...
      case x @ TailLoop(name, args, _, body) =>
        ...
        val accTypes = ctx.req
          .lookupState(x).zip(args.map(_._2.typ))
          .map { case (btwr, t) =>
            VirtualTypeWithReq(t, btwr.asInstanceOf[TypeWithRequiredness]).canonicalEmitType
          }
      ...
      case Recur(name, args, _) =>
        ...
        (loopRef.tmpLoopArgs, loopRef.loopTypes, args).zipped.map { case (tmpLoopArg, et, arg) =>
          tmpLoopArg.store(
            cb,
            emitI(arg, loopEnv = None, region = loopRef.r1)
      ...
      case CollectDistributedArray(contexts, globals, cname, gname, body, dynamicID, staticID,
        ...
        emitStream(contexts, cb, region).map(cb) { case ctxStream: SStreamValue =>
          ...
          bodyFB.emitWithBuilder { cb =>
            ...
            val decodedContext = contextSpec.encodedType
            ...
            val decodedGlobal = globalSpec.encodedType
            ...
            val env =
              EmitEnv(Env[EmitValue]((cname, decodedContext), (gname, decodedGlobal)), FastSeq())
            ...
            bodySpec.encodedType
              .buildEncoder(bodyResult.st, cb.emb.ecb)
          ...
          def addContexts(cb: EmitCodeBuilder, ctxStream: StreamProducer): Unit = {
            ctxStream.memoryManagedConsume(
              ...
              contextSpec.encodedType
                .buildEncoder(ctxTuple.st, parentCB)
          ...
          def addGlobals(cb: EmitCodeBuilder): Unit = {
            ...
            globalSpec.encodedType
              .buildEncoder(wrapped.st, parentCB)
          ...
          val pt = PCanonicalArray(
            bodySpec.encodedType
              .decodedSType(bodySpec.encodedVirtualType).asInstanceOf[SBaseStruct].fieldEmitTypes(
                0
              ).storageType
          )
          val resultArray = pt.constructFromElements(cb, region, len, deepCopy = false) { (cb, i) =>
            val ib = cb.memoize(
              bodySpec.buildCodeInputBuffer(
                Code.newInstance[ByteArrayInputStream, Array[Byte]](encRes(i))
              )
            )
            val eltTupled = bodySpec.encodedType
              .buildDecoder(bodySpec.encodedVirtualType, parentCB)
              .apply(cb, region, ib)
              .asBaseStruct
    ...
    if (result.st.virtualType != ir.typ)
      throw new RuntimeException(
        s"type mismatch:\n  EC=${result.st.virtualType}\n  IR=${ir.typ}\n  node: ${Pretty(ctx.executeContext, ir)
            .take(50)}"
      )
      ...
      case ir @ Apply(fn, typeArgs, args, rt, errorID) =>
        ...
        EmitCode.fromI(mb) { cb =>
          ...
          IEmitCode.multiMapEmitCodes(cb, emitArgs) { codeArgs =>
            cb.invokeSCode(
              meth,
              FastSeq[Param](CodeParam(region), CodeParam(errorID)) ++ codeArgs.map(pc =>
                pc: Param
              ): _*
            )
          }
  ...
  private def makeDependentSortingFunction(
    ...
    sort.emitWithBuilder[Boolean] { cb =>
      ...
      if (leftRightComparatorNames.nonEmpty) {
        ...
        newEnv =
          newEnv.bind((leftRightComparatorNames(0), leftEC), (leftRightComparatorNames(1), rightEC))
      }
    ...
    (cb: EmitCodeBuilder, region: Value[Region], l: Value[_], r: Value[_]) =>
      cb.memoize(cb.invokeCode[Boolean](sort, region, l, r))
  }
}
...
object NDArrayEmitter {
  ...
  def matmulShape(
    ...
    val rightShapeString = const("(")
      .concat(rightShape.map(_.toS).reduce((a, b) => a.concat(", ").concat(b))).concat(")")
...
abstract class NDArrayEmitter(val outputShape: IndexedSeq[Value[Long]], val elementType: SType) {
  ...
  def emit(cb: EmitCodeBuilder, targetType: PCanonicalNDArray, region: Value[Region]): SValue = {
    ...
    SNDArray.forEachIndexColMajor(cb, shapeArray, "ndarrayemitter_emitloops") {
      case (cb, idxVars) =>
        IEmitCode
          .present(cb, outputElement(cb, idxVars)).consume(
            cb,
