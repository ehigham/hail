...
object Bindings {
  ...
  def apply(x: BaseIR, i: Int): Iterable[(String, Type)] = x match {
    ...
    case StreamFold(a, zero, accumName, valueName, _) =>
      if (i == 2) Array(accumName -> zero.typ, valueName -> tcoerce[TStream](a.typ).elementType)
      else empty
    ...
    case CollectDistributedArray(contexts, globals, cname, gname, _, _, _, _) =>
      if (i == 2) Array(cname -> tcoerce[TStream](contexts.typ).elementType, gname -> globals.typ)
      else empty
...
object AggBindings {
  ...
  def apply(x: BaseIR, i: Int, parent: BindingEnv[_]): Option[Iterable[(String, Type)]] = {
    ...
    x match {
      ...
      case AggArrayPerElement(a, elementName, indexName, _, _, false) =>
        if (i == 1)
          wrapped(
            FastSeq(elementName -> a.typ.asInstanceOf[TIterable].elementType, indexName -> TInt32)
          )
        else if (i == 2) base
        else None
  ...
  def apply(x: BaseIR, i: Int, parent: BindingEnv[_]): Option[Iterable[(String, Type)]] = {
    ...
    x match {
      ...
      case AggExplode(a, name, _, true) =>
        if (i == 1) wrapped(FastSeq(name -> a.typ.asInstanceOf[TIterable].elementType)) else None
      case AggArrayPerElement(a, elementName, indexName, _, _, true) =>
        if (i == 1)
          wrapped(
            FastSeq(elementName -> a.typ.asInstanceOf[TIterable].elementType, indexName -> TInt32)
          )
...
object ChildEnvWithoutBindings {
  def apply[T](ir: BaseIR, i: Int, env: BindingEnv[T]): BindingEnv[T] = {
    ir match {
      ...
      case AggFold(zero, seqOp, combOp, elementName, accumName, isScan) =>
        (isScan, i) match {
