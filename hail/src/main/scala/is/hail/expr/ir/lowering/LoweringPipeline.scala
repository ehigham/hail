...
case class LoweringPipeline(lowerings: LoweringPass*) {
  ...
  def noOptimization(): LoweringPipeline = LoweringPipeline(
    lowerings.filter(l => !l.isInstanceOf[OptimizePass]): _*
  )

  def +(suffix: LoweringPipeline): LoweringPipeline = LoweringPipeline(
    (lowerings ++ suffix.lowerings): _*
  )
}
...
object LoweringPipeline {
  ...
  def fullLoweringPipeline(context: String, baseTransformer: LoweringPass): LoweringPipeline = {
    ...
    val base =
      LoweringPipeline(baseTransformer, OptimizePass(s"$context, after ${baseTransformer.context}"))
  ...
  private val _dArrayLowerers =
    Array(DArrayLowering.All, DArrayLowering.TableOnly, DArrayLowering.BMOnly).map { lv =>
