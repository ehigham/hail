    ...
    ir.mapChildren {
      ...
      case mir: MatrixIR =>
        throw new RuntimeException(
          s"expect specialized lowering rule for " +
    ...
    val lowered = mir match {
      ...
      case MatrixRename(child, globalMap, colMap, rowMap, entryMap) =>
        ...
        if (colMap.nonEmpty) {
          ...
          t = t.mapGlobals(
            'global.castRename(t.typ.globalType.insertFields(FastSeq((colsFieldName, newColsType))))
          )
        }
        ...
        if (entryMap.nonEmpty) {
          ...
          t = t.mapRows(
            'row.castRename(t.typ.rowType.insertFields(FastSeq((entriesFieldName, newEntriesType))))
          )
        }
      ...
      case MatrixFilterCols(child, pred) =>
        lower(ctx, child, ab)
          ...
          .mapRows(
            'row.insertFields(entriesField -> 'global('newColIdx).map('i ~> 'row(entriesField)('i)))
          )
          .mapGlobals(
            'global
              .insertFields(
              ...
              'global.insertFields(
                colsField ->
                  'global(colsField).map(
                    col ~> col.insertFields(
                      Symbol(root) -> '__dictfield.invoke("get", table.typ.valueType, colKey)
                    )
                  )
              )
            }
        ...
        lower(ctx, child, ab)
          .mapGlobals(
            subst(
              ...
              BindingEnv(
                Env[IRProxy]("global" -> 'global.selectFields(child.typ.globalType.fieldNames: _*))
              ),
            )
      ...
      case MatrixMapCols(child, newCol, _) =>
        ...
        val (
          ...
          if (aggs.isEmpty)
            ...
            val bindResult: IRProxy => IRProxy = let
              .applyDynamicNamed("apply")(
                (aggResultRef.name, irToProxy(RelationalRef(ident, aggResult.typ)))
              ).apply(_)
                  ...
                  let(
                    ...
                    let.applyDynamicNamed("apply")((aggResultElementRef.name, 'array_aggs(idx))) {
                      aggs.foldLeft[IRProxy](x) { case (acc, (name, _)) =>
                        let.applyDynamicNamed("apply")((name, GetField(aggResultElementRef, name)))(
                          acc
                        )
                      }
                    }
          ...
          if (scans.isEmpty)
            ...
            val bodyResult: IRProxy => IRProxy = (x: IRProxy) =>
              let.applyDynamicNamed("apply")(
                (scanResultElementRef.name, ArrayRef(scanResultRef, idx))
            ...
            irIf('row(entries).isNA) {
              irRange(0, 'global(cols).len)
                .map(
                  'a ~> irToProxy(
                    MakeStruct(
                      right.typ.entryType.fieldNames.map(f =>
      ...
      case MatrixColsHead(child, n) =>
        lower(ctx, child, ab)
          .mapGlobals(
            'global.insertFields(colsField -> 'global(colsField).arraySlice(0, Some(n), 1))
          )
              ...
              entriesField ->
                irRange(0, 'row(entriesField).len, 1)
                  .flatMap(
                    colIdx ~>
                      irRange(0, 'global(lengths)(colIdx), 1)
                        .map(Symbol(genUID()) ~> 'row(entriesField)(colIdx))
                  )
            )
          )
      ...
      case MatrixAggregateRowsByKey(child, entryExpr, rowExpr) =>
        ...
        lower(ctx, child, ab)
          .aggregateByKey(
            reSub.insertFields(
              entriesField -> irRange(0, 'global(colsField).len)
                .aggElements('__element_idx, '__result_idx, Some('global(colsField).len))(
                  let(sa = 'global(colsField)('__result_idx)) {
                    aggLet(
                      sa = 'global(colsField)('__element_idx),
            ...
            'global
              .insertFields(
                colsField ->
                  'global('newColIdx).map {
                    'kv ~>
                      'kv('key).insertFields(child.typ.colValueStruct.fieldNames.map { s =>
              ...
              keyMap ->
                let(__cols_field = 'global(colsField)) {
                  irRange(0, '__cols_field.len)
                    .map(
                      originalColIdx ~> let(__cols_field_element = '__cols_field(originalColIdx)) {
                        makeStruct(
              ...
              entriesField ->
                let(__entries = 'row(entriesField), __key_map = 'global(keyMap)) {
                  irRange(0, '__key_map.len)
                    .map(
                      newColIdx1 ~> '__key_map(newColIdx1)
                        .apply('value)
                        .streamAgg(
                          aggElementIdx ~>
                            aggLet(
                              g = '__entries(aggElementIdx),
                              sa = 'global(colsField)(aggElementIdx),
                            ) {
                              aggFilter(!'g.isNA, eeSub)
                            }
                        )
    ...
    val lowered = tir match {
      case CastMatrixToTable(child, entries, cols) =>
        lower(ctx, child, ab)
          ...
          .mapGlobals(
            'global.selectFields(child.typ.globalType.fieldNames ++ Array(colsFieldName): _*)
          )
      ...
      case x @ MatrixEntriesTable(child) =>
        ...
        if (child.typ.rowKey.nonEmpty && child.typ.colKey.nonEmpty) {
          ...
          lc
            ...
            .mapRows(
              'row
                .dropFields(values).insertFields(
                  toExplode ->
                    'global(oldColIdx)
                      .flatMap(
                        lambdaIdx1 ~> 'row(values)
                          ...
                          .map(
                            lambdaIdx3 ~> let(
                              ...
                              makeStruct(
                                child.typ.rowValueStruct.fieldNames
                                  .map(Symbol(_)).map(f => f -> lambdaIdx3(f)) ++
                                  child.typ.colType.fieldNames
                                    .map(Symbol(_)).map(f => f -> '__col(f)) ++
                                  child.typ.entryType.fieldNames
                                    .map(Symbol(_)).map(f => f -> '__entry(f)): _*
                              )
                            }
                          )
                      )
          ...
          val result = lc
            .mapRows(
              'row.insertFields(
                colIdx -> irRange(0, 'global(colsField).len)
            ...
            .mapRows(
              let(
                ...
                'row
                  .dropFields(entriesField, colIdx).insertFieldsList(
                    newFields,
