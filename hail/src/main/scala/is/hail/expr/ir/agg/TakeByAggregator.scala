...
class TakeByRVAS(
  ...
  private val garbageFields: IndexedSeq[(String, PType)] =
    if (canHaveGarbage)
    ...
    value
      ...
      .consume(
        ...
        { v =>
          ...
          valueType
            .storeAtAddress(cb, eltTuple.fieldOffset(staging, 1), region, v, deepCopy = false)
        },
      )
  }
  ...
  def result(cb: EmitCodeBuilder, _r: Value[Region], resultType: PCanonicalArray)
    ...
    mb.emitWithBuilder[Long] { cb =>
      ...
      resultType
        .constructFromElements(cb, r, ab.size, deepCopy = true) { case (cb, idx) =>
