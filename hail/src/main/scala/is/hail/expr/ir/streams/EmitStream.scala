    ...
    streamIR match {
        ...
        produce(streamChild, cb)
          .map(cb) { case childStream: SStreamValue =>
            ...
            val serializedAggSType = SStackStruct(
              TTuple(tupleFields),
              tupleFieldTypes
                .map(_ => EmitType(SBinaryPointer(PCanonicalBinary()), true)).toIndexedSeq,
            )
            val keyAndAggFields = newKeyVType.canonicalPType
              .asInstanceOf[PCanonicalStruct].sType.fieldEmitTypes :+ EmitType(
                serializedAggSType,
                true,
              )
            val returnElemSType =
            ...
            val producer: StreamProducer = new StreamProducer {
              ...
              override val LproduceElement: CodeLabel = mb.defineAndImplementLabel { cb =>
                ...
                val key = dictState.keyed.storageType
                  .loadCheapSCode(cb, addrOfKeyInRightRegion).loadField(cb, "kt").memoize(
                    cb,
                    "stream_buff_agg_key_right_region",
                  )
                ...
                val sStructToReturn = keyValue.insert(
                  ...
                  (
                    "agg",
                    EmitCode
                      .present(mb, serializedAggTupleSValue)
            ...
            override val LproduceElement: CodeLabel = mb.defineAndImplementLabel { cb =>
              ...
              if (requiresMemoryManagementPerElement) {
                ...
                cb.assign(
                  ...
                  accValueAccRegion
                    .toI(cb).map(cb)(
                      _.castTo(cb, childProducer.elementRegion, accEmitType.st, deepCopy = true)
                    ),
                )
        ...
        produce(stream, cb).map(cb) { case blocks: SStreamValue =>
          ...
          val producer: StreamProducer = new StreamProducer {
            ...
            override val LproduceElement: CodeLabel = mb.defineAndImplementLabel { cb =>
              ...
              row
                .loadField(cb, prevWindowName).consume(
                  cb,
                  {},
                      ...
                      { _ =>
                        ...
                        if (innerProducer.requiresMemoryManagementPerElement)
                          cb.assign(
                            ...
                            Region
                              .stagedCreate(Region.REGULAR, outerProducer.elementRegion.getPool()),
                          )
                        else
      ...
      case StreamGroupByKey(a, key, missingEqual) =>
        produce(a, cb).map(cb) { case childStream: SStreamValue =>
          ...
          def equiv(cb: EmitCodeBuilder, l: SBaseStructValue, r: SBaseStructValue): Value[Boolean] =
            StructOrdering
              .make(l.st, r.st, cb.emb.ecb, missingFieldsEqual = missingEqual).equivNonnull(
                cb,
                l,
                r,
              )
        ...
        IEmitCode.multiMapEmitCodes(cb, as.map(a => EmitCode.fromI(cb.emb)(cb => produce(a, cb)))) {
          childStreams =>
            ...
            val producer: StreamProducer = behavior match {
              case behavior @ (ArrayZipBehavior.TakeMinLength |
            ...
            val producer = new StreamProducer {
              ...
              override val LproduceElement: CodeLabel = mb.defineAndImplementLabel { cb =>
                ...
                cb.assign(
                  curKey,
                  eltType
                    .loadCheapSCode(cb, heads(winner)).subset(key: _*)
      ...
      case x @ StreamZipJoinProducers(contexts, ctxName, makeProducer, key, keyRef, valsRef,
        ...
        emit(contexts, cb).map(cb) { case contextsArray: SIndexableValue =>
          ...
          val regionArray: Settable[Array[Region]] =
      ...
      case x @ StreamMultiMerge(as, key) =>
        IEmitCode.multiMapEmitCodes(cb, as.map(a => EmitCode.fromI(mb)(cb => emit(a, cb)))) {
          children =>
            ...
            val unifiedType = VirtualTypeWithReq
              .union(as.map(a => typeWithReqx(a))).canonicalEmitType
              .st
              .asInstanceOf[SStream]
              .elementEmitType
                ...
                childProducer.element
                  .toI(cb).consume(
                    ...
                    { case sc: SBaseStructValue =>
                      ...
                      val bpv = cb.memoize(
                        builder.invoke[Locus, Array[String], BitPackedVector](
                          "finish",
                          locusObj,
                      ...
                      val keepVariant = Code.invokeScalaObject5[util.ArrayDeque[
                        BitPackedVector
                      ], BitPackedVector, Double, Int, Int, Boolean](
                      ...
                      cb.assign(
                        elementField,
                        EmitCode
                          .present(mb, elt.castTo(cb, elementRegion, elementField.emitType.st)),
                      )
                    },
                  )
