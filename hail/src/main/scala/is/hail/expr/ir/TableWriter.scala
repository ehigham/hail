      ...
      if (stageLocally)
        Some(
          FileSystems.getDefault
            .getPath(ctx.localTmpdir, s"hail_staging_tmp_${UUID.randomUUID()}", "rows", "parts")
        )
      else None,
    )
...
case class PartitionNativeWriter(
  ...
  class StreamConsumer(
    ...
    def consumeElement(cb: EmitCodeBuilder, codeRow: SValue, elementRegion: Settable[Region])
      ...
      spec.encodedType
        .buildEncoder(row.st, cb.emb.ecb)
        .apply(cb, row, ob)
...
class TableSpecHelper(
  ...
  def write(fs: FS, partCounts: Array[Long], distinctlyKeyed: Boolean): Unit = {
    ...
    if (log)
      info(
        s"wrote table with $nRows ${plural(nRows, "row")} " +
...
case class TableSpecWriter(
  ...
  def annotationType: Type = TArray(
    TStruct(
      "partitionCounts" -> TInt64,
      "distinctlyKeyed" -> TBoolean,
  ...
  def writeMetadata(
    ...
    cb.while_(
      i < n, {
        ...
        val count = curElement.asBaseStruct
          .loadField(cb, "partitionCounts").get(cb, "part count can't be missing!").asLong.value

        if (hasKey) {
          // Only nonempty partitions affect first, last, and distinctlyKeyed.
          cb.if_(
            count cne 0L, {
              val curFirst = curElement
                .loadField(cb, "firstKey").get(
                  cb,
                  const("firstKey of curElement can't be missing, part size was ") concat count.toS,
                )

              val comparator = NEQ(lastSeenSettable.emitType.virtualType)
                .codeOrdering(cb.emb.ecb, lastSeenSettable.st, curFirst.st)
              val notEqualToLast = comparator(cb, lastSeenSettable, EmitValue.present(curFirst))
    ...
    cb += cb.emb
      .getObject(new TableSpecHelper(path, rowRelPath, globalRelPath, refRelPath, typ, log))
  ...
  def scoped(path: String, overwrite: Boolean, refs: Option[TableType])(write: IR): IR =
    WriteMetadata(
      ...
      RelationalWriter(
        ...
        overwrite,
        refs.map(typ =>
          "references" -> (ReferenceGenome.getReferences(typ.rowType) ++ ReferenceGenome
            .getReferences(typ.globalType))
        ),
      ),
    )
}
...
case class WrappedMatrixNativeMultiWriter(
  ...
  def apply(ctx: ExecuteContext, mvs: IndexedSeq[TableValue]): Unit =
    writer.apply(ctx, mvs.map(_.toMatrixValue(colKey)))
}
