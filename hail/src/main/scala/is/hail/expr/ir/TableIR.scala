  ...
  def makeCoercer(
    ...
    coercerCache.get(cacheKeyWithInfo) match {
      ...
      case None =>
        ...
        val xType = TStruct("key" -> keyType, "token" -> TFloat64, "prevkey" -> keyType)
        ...
        val sortedPartDataIR = sortIR(bindIR(scanResult) { scanResult =>
          mapIR(
            filterIR(mapIR(rangeIR(I32(0), ArrayLen(scanResult))) { i =>
              InsertFields(ArrayRef(scanResult, i), FastSeq("i" -> i))
...
trait TableReaderWithExtraUID extends TableReader {
  ...
  lazy val fullType: TableType = {
    ...
    fullTypeWithoutUIDs.copy(
      rowType = fullTypeWithoutUIDs.rowType.insertFields(Array((uidFieldName, uidType)))
    )
  }
...
case class PartitionRVDReader(rvd: RVD, uidFieldName: String) extends PartitionReader {
  ...
  def emitStream(
    ...
    val upcastCode = mb.getObject[
      Function4[HailClassLoader, FS, HailTaskContext, Region, AsmFunction2RegionLongLong]
    ](upcast)
    ...
    context.toI(cb).map(cb) { _partIdx =>
      ...
      val producer = new StreamProducer {
        ...
        override def initialize(cb: EmitCodeBuilder, partitionRegion: Value[Region]): Unit = {
          ...
          cb.assign(
            upcastF,
            Code.checkcast[AsmFunction2RegionLongLong](
              upcastCode.invoke[AnyRef, AnyRef, AnyRef, AnyRef, AnyRef](
                "apply",
  ...
  def toJValue: JValue = JString(
    "<PartitionRVDReader>"
  ) // cannot be parsed, but need a printout for Pretty
}
...
trait AbstractNativeReader extends PartitionReader {
  ...
  def fullRowType: TStruct = spec.encodedVirtualType
...
case class PartitionNativeReader(spec: AbstractTypedCodecSpec, uidFieldName: String)
  ...
  def emitStream(
    ...
    val insertUID: Boolean = requestedType.hasField(uidFieldName) && !spec.encodedVirtualType
      .asInstanceOf[TStruct].hasField(uidFieldName)
    val concreteType: TStruct =
      if (insertUID)
    ...
    val uidSType: SStackStruct =
      SStackStruct(TTuple(TInt64, TInt64), Array(EmitType(SInt64, true), EmitType(SInt64, true)))
    val elementSType =
      if (insertUID)
        SInsertFieldsStruct(
          requestedType,
    ...
    context.toI(cb).map(cb) { case ctxStruct: SBaseStructValue =>
      ...
      val producer = new StreamProducer {
        ...
        override val LproduceElement: CodeLabel = mb.defineAndImplementLabel { cb =>
          ...
          val base = spec.encodedType
            .buildDecoder(concreteType, cb.emb.ecb).apply(cb, region, xRowBuf).asBaseStruct
          if (insertUID) {
            cb.assign(rowIdx, rowIdx + 1)
            val uid = EmitValue.present(
              new SStackStructValue(
                uidSType,
                Array(EmitValue.present(partIdx), EmitValue.present(new SInt64Value(rowIdx))),
              )
            )
...
case class PartitionNativeIntervalReader(
  ...
  def emitStream(
    ...
    context.toI(cb).map(cb) { case _ctx: SIntervalValue =>
      ...
      val partitionerRuntime = cb.emb
        .addLiteral(cb, partitionerLit.value, VirtualTypeWithReq.fullyOptional(partitionerLit.typ))
        .asIndexable

      val pathsType = VirtualTypeWithReq.fullyRequired(TArray(TString))
      val rowsPath = tableSpec.rowsComponent.absolutePath(tablePath)
      val partitionPathsRuntime = cb
        .memoizeField(
          mb.addLiteral(cb, rowsSpec.absolutePartPaths(rowsPath).toFastSeq, pathsType),
          "partitionPathsRuntime",
        )
        .asIndexable
      val indexPathsRuntime = cb
        .memoizeField(
          mb.addLiteral(
            cb,
            rowsSpec.partFiles
              .map(partPath => s"$rowsPath/${indexSpec.relPath}/$partPath.idx").toFastSeq,
            pathsType,
          ),
          "indexPathsRuntime",
        )
            ...
            currIdxInPartition >= stopIdxInPartition, {
              ...
              cb.if_(
                requiresIndexInit, {
                  ...
                  val partPath = partitionPathsRuntime
                    .loadElement(cb, currPartitionIdx).get(cb).asString.loadString(cb)
                  val idxPath = indexPathsRuntime
                    .loadElement(cb, currPartitionIdx).get(cb).asString.loadString(cb)
                ...
                currPartitionIdx ceq lastIncludedPartitionIdx, {
                  cb.if_(
                    currPartitionIdx ceq startPartitionIndex, {
                      ...
                      val startIdx = indexResult
                        .loadField(cb, 0)
                        .get(cb)
                        .asInt64
                        .value
                      cb.assign(currIdxInPartition, startIdx)
                      val endIdx = indexResult
                        .loadField(cb, 1)
                        .get(cb)
                        .asInt64
                        .value
                  ...
                  cb.if_(
                    currPartitionIdx ceq startPartitionIndex, {
                      ...
                      cb.if_(
                        currIdxInPartition < stopIdxInPartition, {
                          val firstOffset = indexResult
                            .loadField(cb, 1).get(cb).asBaseStruct
          ...
          cb.assign(
            ...
            if (insertUID)
              decRow.insert(
                ...
                uidFieldName -> EmitValue.present(
                  uidSType.fromEmitCodes(
                    cb,
                    FastSeq(
    ...
    context.toI(cb).map(cb) { case ctxStruct: SBaseStructValue =>
      ...
      val producer = new StreamProducer {
        ...
        override def initialize(cb: EmitCodeBuilder, outerRegion: Value[Region]): Unit = {
          ...
          val startIndex = indexResult
            .loadField(cb, 0)
            .get(cb)
            .asInt64
            .value
          val endIndex = indexResult
            .loadField(cb, 1)
            .get(cb)
            .asInt64
            .value
              ...
              new SInsertFieldsStructValue(
                ...
                Array(
                  EmitValue.present(
                    new SStackStructValue(
                      uidSType,
                      Array(EmitValue.present(partIdx), EmitValue.present(primitive(curIdx))),
                    )
                  )
                ),
              ),
...
case class PartitionZippedNativeReader(left: PartitionReader, right: PartitionReader)
  ...
  def contextType: Type =
    TStruct("leftContext" -> left.contextType, "rightContext" -> right.contextType)
  ...
  override def rowRequiredness(requestedType: TStruct): RStruct = {
    ...
    RStruct.fromNamesAndTypes(
      requestedType.fieldNames.map(f =>
...
case class PartitionZippedIndexedNativeReader(
  ...
  def rowRequiredness(requestedType: TStruct): RStruct = {
    ...
    val pt = specLeft
      .decodedPType(leftStruct).asInstanceOf[PStruct].insertFields(
  ...
  def fullRowType: TStruct =
    (specLeft.encodedVirtualType.asInstanceOf[TStruct] ++ specRight.encodedVirtualType
      .asInstanceOf[TStruct])
      ...
      val producer = new StreamProducer {
          ...
          cb.assign(
            ...
            specLeft.buildCodeInputBuffer(
              Code.newInstance[ByteTrackingInputStream, InputStream](
                mb.openUnbuffered(
                  ctxStruct
                    .loadField(cb, "leftPartitionPath")
                    .get(cb)
                    .asString
                    .loadString(cb),
                  true,
          ...
          cb.assign(
            ...
            specRight.buildCodeInputBuffer(
              Code.newInstance[ByteTrackingInputStream, InputStream](
                mb.openUnbuffered(
                  ctxStruct
                    .loadField(cb, "rightPartitionPath")
                    .get(cb)
                    .asString
                    .loadString(cb),
                  true,
        ...
        override val element: EmitCode = EmitCode.fromI(mb) { cb =>
          if (insertUID) {
            ...
            IEmitCode
              .present(cb, merged._insert(requestedType, uidFieldName -> EmitValue.present(uid)))
          } else {
            IEmitCode
...
class TableNativeReader(
  ...
  override def lower(ctx: ExecuteContext, requestedType: TableType): TableStage = {
    ...
    val partitioner =
      if (filterIntervals)
        params.options.map(opts =>
          RVDPartitioner
            .union(ctx.stateManager, specPart.kType, opts.intervals, specPart.kType.size - 1)
        )
      else
        params.options.map(opts =>
    ...
    val requestedUIDFieldName =
      if (spec.table_type.rowType.hasField(uidFieldName))
        "__dummy_uid"
      else
        uidFieldName
...
case class TableNativeZippedReader(
  ...
  require(
    (specLeft.table_type.rowType.fieldNames ++ specRight.table_type.rowType.fieldNames)
      .areDistinct()
  )
  ...
  override def fullTypeWithoutUIDs: TableType =
    specLeft.table_type.copy(rowType = specLeft.table_type.rowType ++ specRight.table_type.rowType)
    ...
    val (Some(PTypeReferenceSingleCodeType(t: PStruct)), mk) =
      ir.Compile[AsmFunction3RegionLongLongLong](
        ...
        InsertFields(
          ...
          pRight.fieldNames.map(f => f -> GetField(Ref("right", pRight.virtualType), f)),
        ),
      )
    (t, mk)
  }
  ...
  def native(fs: FS, path: String, uidField: Boolean = false): TableRead = {
    ...
    val requestedType =
      if (uidField)
        ...
        tr.fullType.copy(rowType = tr.fullType.rowType.deleteKey(TableReader.uidFieldName))
...
case class TableParallelize(rowsAndGlobal: IR, nPartitions: Option[Int] = None) extends TableIR {
  ...
  lazy val typ: TableType = {
    ...
    TableType(rowsType.elementType.asInstanceOf[TStruct], FastSeq(), globalsType)
  }
...
case class TableGen(
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    : TableExecuteIntermediate =
    new TableStageIntermediate(
      LowerTableIR.applyTable(this, DArrayLowering.All, ctx, LoweringAnalyses(this, ctx))
    )
}
...
case class TableRange(n: Int, nPartitions: Int) extends TableIR {
  ...
  val typ: TableType = TableType(TStruct("idx" -> TInt32), Array("idx"), TStruct.empty)
    ...
    new TableValueIntermediate(
      TableValue(
        ...
        new RVD(
          ...
          ContextRDD
            .parallelize(Range(0, nPartitionsAdj), nPartitionsAdj)
            .cmapPartitionsWithIndex { case (i, ctx, _) =>
              val region = ctx.region

              val start = partStarts(i)
              Iterator
                .range(start, start + localPartCounts(i))
                .map { j =>
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    ...
    val localNewRowType = PCanonicalStruct(
      ...
      keyFields ++ Array(
        (
          fieldName,
          PCanonicalArray(PCanonicalStruct(required = false, valueFields: _*), required = true),
        )
      ): _*
    )
    ...
    val rvd = RVD(
      ...
      crdd = ContextRDD
        .czipNPartitions(repartitionedRVDs.map(_.crdd.toCRDDRegionValue)) { (ctx, its) =>
...
case class TableLeftJoinRightDistinct(left: TableIR, right: TableIR, root: String) extends TableIR {
  override def typecheck(): Unit =
    assert(right.typ.keyType isPrefixOf left.typ.keyType, s"\n  L: ${left.typ}\n  R: ${right.typ}")
  ...
  lazy val typ: TableType =
    left.typ.copy(rowType = left.typ.rowType.structInsert(right.typ.valueType, FastSeq(root)))
...
case class TableMapPartitions(
  ...
  lazy val typ: TableType =
    child.typ.copy(rowType = body.typ.asInstanceOf[TStream].elementType.asInstanceOf[TStruct])
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    ...
    val itF = { (idx: Int, consumerCtx: RVDContext, partition: (RVDContext) => Iterator[Long]) =>
      ...
      makeIterator(
        ...
        consumerCtx,
        globalsBc.value
          .readRegionValue(consumerCtx.partitionRegion, theHailClassLoaderForSparkWorkers),
        boxedPartition,
      ).map(l => l.longValue())
    }
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    ...
    if (ctx.getFlag("distributed_scan_comb_op") != null && extracted.shouldTreeAggregate) {
        ...
        .mapPartitionsWithIndex { (i, ctx, it) =>
          ...
          ctx.r.pool.scopedSmallRegion { aggRegion =>
            ...
            using(new DataOutputStream(fsBc.value.create(path))) { os =>
              val bytes =
                write(theHailClassLoaderForSparkWorkers, tc, aggRegion, seq.getAggOffset())
      ...
      while (filesToMerge.length > 1) {
        ...
        filesToMerge = ContextRDD
          .weaken(
            SparkBackend
              .sparkContext("TableMapRows.execute").parallelize(0 until nToMerge, nToMerge)
          )
          .cmapPartitions { (ctx, it) =>
            val i = it.next()
            assert(it.isEmpty)
    ...
    val scanPartitionAggs = SpillingCollectIterator(
      ...
      tv.rvd.mapPartitionsWithIndex { (i, ctx, it) =>
        ...
        SparkTaskContext
          .get().getRegionPool().scopedSmallRegion { aggRegion =>
            val hcl = theHailClassLoaderForSparkWorkers
            val tc = SparkTaskContext.get()
            val seq = eltSeqF(hcl, fsBc.value, tc, globalRegion)
    ...
    val partAggs = scanPartitionAggs.scanLeft(initAgg)(
      combOpFNeedsPool(() => (ctx.r.pool, ctx.theHailClassLoader, ctx.taskContext))
    )
...
case class TableExplode(child: TableIR, path: IndexedSeq[String]) extends TableIR {
  ...
  private[this] lazy val newRow: InsertFields = {
    ...
    path
      .zip(refs).zipWithIndex.foldRight[IR](idx) { case (((field, ref), i), arg) =>
        InsertFields(
          ref,
          FastSeq(
            field ->
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    ...
    val length: IR =
      Coalesce(
        FastSeq(
          ArrayLen(CastToArray(path.foldLeft[IR](Ref("row", childRowType)) { (struct, field) =>
...
case class TableUnion(childrenSeq: IndexedSeq[TableIR]) extends TableIR {
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    ...
    TableValueIntermediate(
      tvs(0).copy(rvd = RVD.union(RVD.unify(ctx, tvs.map(_.rvd)), tvs(0).typ.key.length, ctx))
    )
  }
}
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    ...
    val (Some(PTypeReferenceSingleCodeType(localKeyPType: PStruct)), makeKeyF) =
      ir.Compile[AsmFunction3RegionLongLongLong](
        ...
        LongInfo,
        Coalesce(
          FastSeq(newKey, Die("Internal error: TableKeyByAndAggregate: newKey missing", newKey.typ))
        ),
      )
    ...
    val rdd = prev.rvd.boundary
      .mapPartitionsWithIndex { (i, ctx, it) =>
        ...
        new BufferedAggregatorIterator[Long, RegionValue, Array[Byte], Row](it, makeAgg, makeKey,
          seqOp, serializeAndCleanupAggs, localBufferSize)
      }.aggregateByKey(initAggs, nPartitions.getOrElse(prev.rvd.getNumPartitions))(combOp, combOp)

    val crdd = ContextRDD
      .weaken(rdd).cmapPartitionsWithIndex({ (i, ctx, it) =>
        val region = ctx.region
    ...
    new TableValueIntermediate(
      prev.copy(typ = typ, rvd = RVD.coerce(ctx, RVDType(newRowType, keyType.fieldNames), crdd))
    )
  }
}
...
case class TableRename(child: TableIR, rowMap: Map[String, String], globalMap: Map[String, String])
  ...
  override protected[ir] def execute(ctx: ExecuteContext, r: LoweringAnalyses)
    : TableExecuteIntermediate =
    TableValueIntermediate(child.execute(ctx, r).asTableValue(ctx).rename(globalMap, rowMap))
}
