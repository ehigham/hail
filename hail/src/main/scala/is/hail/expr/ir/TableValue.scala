...
case class TableValue(ctx: ExecuteContext, typ: TableType, globals: BroadcastRow, rvd: RVD) {
  ...
  def toDF(): DataFrame =
    HailContext
      .sparkBackend("toDF").sparkSession.createDataFrame(
        rvd.toRows,
  ...
  def toMatrixValue(
    ...
    val mType: MatrixType = MatrixType(
      ...
      typ.rowType
        .field(MatrixType.entriesIdentifier).typ.asInstanceOf[TArray].elementType.asInstanceOf[
          TStruct
        ],
    )
    ...
    val globals2 =
      if (colsT.required && colsT.elementType.required)
        ...
        globals.cast(
          globalsT.insertFields(
            FastSeq(colsFieldName -> PCanonicalArray(colsT.elementType.setRequired(true), true))
          )
        )
