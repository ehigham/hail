...
object DistinctlyKeyed {
  ...
  def apply(node: BaseIR): DistinctKeyedAnalysis = {
    ...
    IRTraversal.postOrder(node).foreach {
      ...
      case t @ (_: TableFilter | _: TableIntervalJoin |
          _: TableLeftJoinRightDistinct | _: TableMapRows | _: TableMapGlobals) =>
        memo.bindIf(memo.contains(t.children.head), t, ())

      case t @ RelationalLetTable(_, _, body) =>
        memo.bindIf(memo.contains(body), t, ())

      case t @ (_: TableHead | _: TableTail | _: TableRepartition |
          _: TableJoin | _: TableMultiWayZipJoin | _: TableRename | _: TableFilterIntervals) =>
        memo.bindIf(t.children.forall(memo.contains), t, ())

      case t @ (_: TableRange | _: TableDistinct |
          _: TableKeyByAndAggregate | _: TableAggregateByKey) =>
        memo.bind(t, ())
