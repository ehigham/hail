            ...
            if (insertUID) {
              val uid = EmitValue.present(
                new SStackStructValue(
                  uidSType,
                  Array(EmitValue.present(primitive(partIdx)), EmitValue.present(primitive(rowIdx))),
                )
              )
...
class GenericTableValue(
  ...
  def toTableStage(ctx: ExecuteContext, requestedType: TableType, context: String, cacheKey: Any)
    ...
    if (p != null) {
      ...
      getLTVCoercer(ctx, context, cacheKey).coerce(ctx, globalsIR, contextType, contexts,
        requestedBody)
    }
  }
}
