  ...
  def generateComponentFunctions(
    ...
    val lowered =
      if (partitions != null) {
        ...
        val rangeBounds = JSONAnnotationImpex
          .importAnnotation(jv, partitionsType)
...
case class MatrixNativeWriter(
  ...
  override def lower(
    ...
    val components = MatrixNativeWriter.generateComponentFunctions(colsFieldName, entriesFieldName,
      colKey, ctx, tablestage, r, path, overwrite, stageLocally, codecSpecJSONStr, partitions,
      partitionsTypeStr)
  ...
  def consumeStream(
    ...
    context.toI(cb).map(cb) { pctx =>
          ...
          writer.add(
            cb, {
              IEmitCode.present(
                cb,
                keyType
                  .asInstanceOf[PCanonicalBaseStruct]
                  .constructFromFields(
                    cb,
            ...
            distinctlyKeyed, {
              lastSeenSettable
                .loadI(cb).consume(
                  ...
                  { lastSeen =>
                    val comparator = EQ(lastSeenSettable.emitType.virtualType)
                      .codeOrdering(cb.emb.ecb, lastSeenSettable.st, key.st)
      ...
      stages.flatMap(_.toIterable).zip(filenames).foreach { case (source, destination) =>
        cb += mb.getFS
          .invoke[String, String, Boolean, Unit]("copy", source, destination, const(true))
      }
...
case class MatrixSpecWriter(
  ...
  def writeMetadata(
    ...
    cb += cb.emb
      .getObject(
        new MatrixSpecHelper(path, rowRelPath, globalRelPath, colRelPath, entryRelPath, refRelPath,
          typ, log)
      )
...
case class VCFPartitionWriter(
  ...
  final def consumeStream(
    ...
    context.toI(cb).map(cb) { case ctx: SBaseStructValue =>
      ...
      if (writeHeader) {
        ...
        val headerStr = Code.invokeScalaObject6[TStruct, TStruct, ReferenceGenome, Option[
          String
        ], Option[VCFMetadata], Array[String], String](
          ExportVCF.getClass,
          "makeHeader",
      ...
      if (tabix) {
        cb += Code
          .invokeScalaObject2[FS, String, Unit](TabixVCF.getClass, "apply", cb.emb.getFS, filename)
      }
  ...
  def consumeElement(
    ...
    if (filtersExists)
      elt
        .loadField(cb, filtersIdx).consume(
          cb,
          writeC('.'),
...
case class VCFExportFinalizer(
  ...
  private def header(cb: EmitCodeBuilder, annotations: SBaseStructValue): Code[String] = {
    ...
    Code.invokeScalaObject6[TStruct, TStruct, ReferenceGenome, Option[String], Option[
      VCFMetadata
    ], Array[String], String](
      ExportVCF.getClass,
      "makeHeader",
  ...
  def writeMetadata(writeAnnotations: => IEmitCode, cb: EmitCodeBuilder, region: Value[Region])
    ...
    val partFiles = partPaths
      .castTo(cb, region, SJavaArrayString(true), false).asInstanceOf[SJavaArrayStringValue].array
    cb.if_(
    ...
    exportType match {
      case ExportType.CONCATENATED =>
        ...
        cb += cb.emb.getFS
          .invoke[Array[String], String, Unit]("concatenateFiles", jFiles, const(outputPath))
...
case class MatrixBGENWriter(
  ...
  override def lower(
    ...
    ts.mapContexts { oldCtx =>
      ...
      val numVariants =
        if (writeHeader) ToStream(ts.countPerPartition())
...
case class BGENPartitionWriter(
  ...
  final def consumeStream(
    ...
    context.toI(cb).map(cb) { case ctx: SBaseStructValue =>
      ...
      val slowCount =
        if (writeHeader || stream.length.isDefined) None
        else Some(cb.newLocal[Long]("num_variants", 0))
      val fastCount =
        if (writeHeader) Some(ctx.loadField(cb, "numVariants").get(cb).asInt64.value)
  ...
  private def consumeElement(
    ...
    def stringToBytesWithShortLength(
      ...
      cb += Code.toUnit(
        Code.invokeScalaObject2[ByteArrayBuilder, String, Int](
          BgenWriter.getClass,
    ...
    def intToBytesLE(cb: EmitCodeBuilder, bb: Value[ByteArrayBuilder], i: Value[Int]) = cb += Code
      .invokeScalaObject2[ByteArrayBuilder, Int, Unit](BgenWriter.getClass, "intToBytesLE", bb, i)
    def shortToBytesLE(cb: EmitCodeBuilder, bb: Value[ByteArrayBuilder], i: Value[Int]) = cb += Code
      .invokeScalaObject2[ByteArrayBuilder, Int, Unit](BgenWriter.getClass, "shortToBytesLE", bb, i)
            ...
            { gp =>
              ...
              cb.if_(
                gpSum >= 0.999 && gpSum <= 1.001, {
                  cb += uncompBuf
                    .invoke[Int, Byte, Unit]("update", samplePloidyStart + j, BgenWriter.ploidy)
                  cb += Code.invokeScalaObject6[Array[Double], Array[Double], Array[Int], Array[
                    Int
                  ], ByteArrayBuilder, Long, Unit](
                    BgenWriter.getClass,
    ...
    val compLen = cb.memoize(
      Code.invokeScalaObject2[ByteArrayBuilder, Array[Byte], Int](
...
case class BGENExportFinalizer(typ: MatrixType, path: String, exportType: String, compression: Int)
  ...
  def writeMetadata(writeAnnotations: => IEmitCode, cb: EmitCodeBuilder, region: Value[Region])
    ...
    if (exportType != ExportType.PARALLEL_HEADER_IN_SHARD) {
      results.forEachDefined(cb) { (cb, i, res) =>
        res.asBaseStruct
          .loadField(cb, "numVariants").consume(
            cb,
            { /* do nothing */ },
    ...
    if (exportType == ExportType.CONCATENATED) {
      ...
      annotations.loadField(cb, "results").get(cb).asIndexable.forEachDefined(cb) { (cb, i, res) =>
        res.asBaseStruct
          .loadField(cb, "partFile").consume(
            cb,
            { /* do nothing */ },
            { case pf: SStringValue =>
              val f = cb.memoize(cb.emb.open(pf.loadString(cb), false))
              cb += Code
                .invokeStatic3[org.apache.hadoop.io.IOUtils, InputStream, OutputStream, Int, Unit](
                  "copyBytes",
                  f,
                  os,
                  4096,
  ...
  override def lower(
    ...
    ts.mapContexts { oldCtx =>
      ...
      zip2(oldCtx, ToStream(files), ArrayZipBehavior.AssertSameLength) { (ctxElt, pf) =>
        MakeStruct(FastSeq("oldCtx" -> ctxElt, "file" -> pf))
      }
  ...
  private def consumeElement(
    ...
    val (contig, position) = elt.loadField(cb, locusIdx).get(cb) match {
      ...
      case locus: SBaseStructValue =>
        locus.loadField(cb, 0).get(cb).asString.loadString(cb) -> locus
          .loadField(cb, 1).get(cb).asInt.value
    }
    ...
    elt
      .loadField(cb, entriesFieldName).get(cb).asIndexable.forEachDefinedOrMissing(cb)(
        (cb, i) => cb += bp.invoke[Int, Unit]("add", 1),
        { (cb, i, va) =>
          va.asBaseStruct
            .loadField(cb, "GT").consume(
              cb,
              cb += bp.invoke[Int, Unit]("add", 1),
              { case call: SCallValue =>
                val gtIx = cb.memoize(
                  Code.invokeScalaObject1[Call, Int](
                    Call.getClass,
...
case class MatrixBlockMatrixWriter(
  ...
  override def lower(
    ...
    val desiredRowIntervals =
      desiredRowStarts.zip(desiredRowStops).map { case (intervalStart, intervalEnd) =>
    ...
    val tableOfNDArrays = rowsInBlockSizeGroups
      .mapContexts(createBlockMakingContexts)(ir =>
        ...
        bindIR(GetField(ctxRef, "blockStart")) { blockStartRef =>
          ...
          val arrayOfSlicesAndIndices = ToArray(mapIR(partIr) { singleRow =>
            val mappedSlice = ToArray(
              ...
              )(entriesStructRef => GetField(entriesStructRef, entryField))
            )
            MakeStruct(
              FastSeq(
          ...
          bindIR(arrayOfSlicesAndIndices) { arrayOfSlicesAndIndicesRef =>
            ...
            val ndarrayData = ToArray(flatMapIR(ToStream(arrayOfSlicesAndIndicesRef)) {
              idxAndSlice => ToStream(GetField(idxAndSlice, "rowOfData"))
            })
    ...
    val pathsWithColMajorIndices = tableOfNDArrays.mapCollect("matrix_block_matrix_writer") {
      partition =>
        ToArray(mapIR(partition) { singleNDArrayTuple =>
          bindIR(
            GetField(singleNDArrayTuple, "blockRowIdx") + (GetField(
              singleNDArrayTuple,
              "blockColIdx",
            ) * numBlockRows)
          ) { colMajorIndex =>
    ...
    RelationalWriter.scoped(path, overwrite, None)(
      WriteMetadata(flatPaths, BlockMatrixNativeMetadataWriter(path, false, bmt))
    )
  }
}
  ...
  def lower(
    ...
    val components =
      paths.zip(tables).map { case (path, (colsFieldName, entriesFieldName, colKey, ts, rt)) =>
        MatrixNativeWriter.generateComponentFunctions(colsFieldName, entriesFieldName, colKey, ctx,
            ...
            case (ctx, globals) =>
              bindIR(GetField(ctx, "options")) { options =>
                Switch(
                  ...
                  default =
                    Die("MatrixId exceeds matrix count", components.head.writePartitionType),
