        ...
        MakeArray(rowBlocks(i).map { ii =>
          MakeArray(colBlocks(j).map { jj =>
            ...
            if (typ.hasBlock(idx2))
              ...
              MakeTuple.ordered(
                FastSeq(MakeTuple.ordered(FastSeq(nRows, nCols)), NA(outer.ctxType))
              )
            }
          }: _*)
        }: _*)
...
object BMSContexts {
  ...
  def tabulate(typ: BlockMatrixType, ib: IRBuilder)(f: (IR, IR) => IR): BMSContexts = {
    val contexts = ib.memoize(
      ToArray(mapIR(typ.sparsity.allBlocksColMajorIR(typ.nRowBlocks, typ.nColBlocks)) { coords =>
      ...
      If(
        ...
        Die(
          strConcat("Internal Error, tried to load missing BlockMatrix context: (row = ", row,
            ", col = ", col, ", pos = ", pos, ", rowPos = ", rowPos, ", rowIdx = ", rowIdx, ")"),
          elementType,
          ErrorIDs.NO_ERROR,
        ),
      )
...
class BlockMatrixStage2 private (
  ...
  def toTableStage(ib: IRBuilder, ctx: ExecuteContext, bmTyp: BlockMatrixType): TableStage = {
    ...
    val contextsIR = ToStream(
      contexts
        .map(ib) { (rowIdx, colIdx, pos, oldContext) =>
  ...
  def collectBlocks(
    ...
    val cdaContexts = ToStream(
      contexts.map(ib)((rowIdx, colIdx, pos, oldContext) => maketuple(pos, oldContext)).contexts
    )
  ...
  def lowerNonEmpty2(
    ...
    bmir match {
      ...
      case x @ BlockMatrixBroadcast(child, IndexedSeq(axis, axis2), _, _)
        ...
        val rowPos =
          if (child.typ.nColBlocks > diagLen)
  ...
  def lowerNonEmpty(
    ...
    bmir match {
            ...
            new BlockMatrixStage(loweredChild.broadcastVals, TArray(loweredChild.ctxType)) {
              ...
              override def blockBody(ctxRef: Ref): IR = {
                  ...
                  bindIR(NDArrayAgg(loweredChild.blockBody(singleChildCtx), axesToSumOut))(
                    aggedND =>
                      NDArrayReshape(
                        aggedND,
                        MakeTuple.ordered(
                          FastSeq(I64(1), GetTupleElement(NDArrayShape(aggedND), 0))
                        ),
                        ErrorIDs.NO_ERROR,
                      )
                  )
                ...
                StreamAgg(
                  summedChildBlocks,
                  aggVar,
                  ApplyAggOp(NDArraySum())(
                    Ref(aggVar, summedChildBlocks.typ.asInstanceOf[TStream].elementType)
                  ),
                )
              }
            }
                  ...
                  bindIR(NDArrayAgg(loweredChild.blockBody(singleChildCtx), axesToSumOut)) {
                    aggedND =>
                      NDArrayReshape(
                        aggedND,
                        MakeTuple(
                          FastSeq(0 -> GetTupleElement(NDArrayShape(aggedND), 0), 1 -> I64(1))
                        ),
                        ErrorIDs.NO_ERROR,
                      )
                  }
      ...
      case x @ BlockMatrixSlice(
        ...
        lower(child)
          .condenseBlocks(child.typ, rowDependents, colDependents)
          .addContext(TTuple(TTuple(TInt64, TInt64, TInt64), TTuple(TInt64, TInt64, TInt64))) {
            idx =>
              ...
              val rows = MakeTuple.ordered(FastSeq[IR](rowTrueStart, rowTrueEnd, rStep))
              ...
              val cols = MakeTuple.ordered(FastSeq[IR](colTrueStart, colTrueEnd, cStep))
      ...
      case x @ BlockMatrixDot(leftIR, rightIR) =>
        ...
        new BlockMatrixStage(left.broadcastVals ++ right.broadcastVals, newCtxType) {
          def blockContext(idx: (Int, Int)): IR = {
            ...
            MakeArray(
              Array
                .tabulate[Option[IR]](leftIR.typ.nColBlocks) { k =>
                  if (leftIR.typ.hasBlock(i -> k) && rightIR.typ.hasBlock(k -> j))
                    Some(
                      MakeTuple.ordered(
                        FastSeq(left.blockContext(i -> k), right.blockContext(k -> j))
                      )
                    )
                  else None
                }.flatten[IR],
