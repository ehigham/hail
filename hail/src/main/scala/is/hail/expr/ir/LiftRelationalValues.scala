...
object LiftRelationalValues {
  ...
  def apply(ir0: BaseIR): BaseIR = {
    ...
    def rewrite(ir: BaseIR, ab: BoxedArrayBuilder[(String, IR)], memo: mutable.Map[IR, String])
      ...
      case (_: TableAggregate | _: TableCount | _: TableToValueApply | _: BlockMatrixToValueApply |
          _: TableCollect | _: BlockMatrixCollect | _: TableGetGlobals) if ir.typ != TVoid =>
    ...
    rewrite(ir0, ab, memo) match {
      case rw: IR =>
        ab.result().foldRight[IR](rw) { case ((name, value), acc) =>
          RelationalLet(name, value, acc)
        }
      case rw: TableIR =>
        ab.result().foldRight[TableIR](rw) { case ((name, value), acc) =>
          RelationalLetTable(name, value, acc)
        }
      case rw: MatrixIR =>
        ab.result().foldRight[MatrixIR](rw) { case ((name, value), acc) =>
          RelationalLetMatrixTable(name, value, acc)
        }
      case rw: BlockMatrixIR =>
        ab.result().foldRight[BlockMatrixIR](rw) { case ((name, value), acc) =>
          RelationalLetBlockMatrix(name, value, acc)
        }
    }
