...
class StagedBlockLinkedList(val elemType: PType, val kb: EmitClassBuilder[_]) {
  ...
  val nodeType =
    PCanonicalStruct("buf" -> bufferType, "count" -> PInt32Required, "next" -> PInt64Required)
  ...
  def push(cb: EmitCodeBuilder, region: Value[Region], elt: EmitCode): Unit = {
    val pushF = kb.genEmitMethod(
      "blockLinkedListPush",
      FastSeq[ParamType](typeInfo[Region], elt.emitParamType),
      typeInfo[Unit],
    )
  ...
  def append(cb: EmitCodeBuilder, region: Value[Region], bll: StagedBlockLinkedList): Unit = {
    ...
    val appF = kb.genEmitMethod(
      "blockLinkedListAppend",
