...
sealed abstract class AbstractApplyNode[F <: JVMFunction] extends IR {
  ...
  lazy val implementation: F = IRFunctionRegistry
    .lookupFunctionOrFail(function, returnType, typeArgs, argTypes)
    .asInstanceOf[F]
}
    ...
    (jv \ "name").extract[String] match {
      ...
      case "GVCFPartitionReader" =>
        ...
        GVCFPartitionReader(header, callFields, entryFloatType, arrayElementsRequired, rg,
          contigRecoding, skipInvalidLoci, filterAndReplace, entriesFieldName, uidFieldName)
