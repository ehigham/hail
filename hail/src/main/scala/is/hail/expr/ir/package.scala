...
package object ir {
  ...
  def invoke(name: String, rt: Type, typeArgs: Seq[Type], errorID: Int, args: IR*): IR =
    IRFunctionRegistry.lookupUnseeded(name, rt, typeArgs, args.map(_.typ)) match {
      ...
      case None =>
        fatal(s"no conversion found for $name(${typeArgs
            .mkString(", ")}, ${args.map(_.typ).mkString(", ")}) => $rt")
    }
  ...
  def maketuple(fields: IR*): MakeTuple = MakeTuple(fields.toArray.zipWithIndex.map {
    case (field, idx) => (idx, field)
  })
