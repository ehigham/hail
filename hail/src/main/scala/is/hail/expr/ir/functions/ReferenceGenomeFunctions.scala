...
object ReferenceGenomeFunctions extends RegistryFunctions {
  ...
  def registerAll() {
    registerSCode1t(
      ...
      primitive(
        cb.memoize(
          rgCode(r.mb, tlocus.asInstanceOf[TLocus].rg)
    ...
    registerSCode2t(
      ...
      primitive(
        cb.memoize(
          rgCode(r.mb, tlocus.rg)
            .invoke[String, Int, Boolean]("isValidLocus", scontig, pos.asInt.value)
        )
      )
    }
    ...
    registerIR(
      ...
      val isValid = IRFunctionRegistry
        .lookupUnseeded("isValidLocus", TBoolean, typeParameters = tl, Seq(TString, TInt32)).get
