...
object ArrayFunctions extends RegistryFunctions {
  val arrayOps: Array[(String, Type, Type, (IR, IR, Int) => IR)] =
    Array(
      ...
      (
        ...
        tnum("T"),
        tv("T"),
        (ir1: IR, ir2: IR, _) => ApplyBinaryPrimOp(RoundToNegInfDivide(), ir1, ir2),
      ),
      ...
      (
        ...
        tnum("T"),
        TFloat64,
        (ir1: IR, ir2: IR, errorID: Int) => Apply("pow", Seq(), Seq(ir1, ir2), TFloat64, errorID),
      ),
      (
        "mod",
        tnum("T"),
        tv("T"),
        (ir1: IR, ir2: IR, errorID: Int) => Apply("mod", Seq(), Seq(ir1, ir2), ir2.typ, errorID),
      ),
    )
  ...
  def extend(a1: IR, a2: IR): IR = {
    ...
    If(
      ...
      If(
        ...
        NA(typ),
        ToArray(
          StreamFlatMap(MakeStream(FastSeq(a1, a2), TStream(typ)), uid, ToStream(Ref(uid, a1.typ)))
        ),
      ),
    )
  }
  ...
  def contains(a: IR, value: IR): IR =
    exists(a, elt => ApplyComparisonOp(EQWithNA(elt.typ, value.typ), elt, value))
      ...
      case (
        ...
        IEmitCode.multiMapEmitCodes(cb, FastSeq(array, localAlleles, nTotalAlleles)) {
          case IndexedSeq(
            ...
            cb.while_(
              i < laLen, {
                val lai = localAlleles
                  .loadElement(cb, i).get(
                    cb,
                ...
                cb.while_(
                  j <= i, {
                    val laj = localAlleles
                      .loadElement(cb, j).get(
                        cb,
                        "local_to_global: local alleles elements cannot be missing",
                        err,
                      ).asInt32.value
        ...
        IEmitCode.multiMapEmitCodes(
          ...
          case IndexedSeq(
            ...
            cb.while_(
              i < localLen, {
                val lai = localAlleles
                  .loadElement(cb, i + idxAdjustmentForOmitFirst).get(
                    cb,
                    "local_to_global: local alleles elements cannot be missing",
                    err,
                  ).asInt32.value
