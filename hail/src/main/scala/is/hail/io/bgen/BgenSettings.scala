...
object BgenSettings {
  ...
  def indexCodecSpecs(indexVersion: SemanticVersion, rg: Option[String])
    ...
    val keyEType = EBaseStruct(
      FastSeq(
        EField(
          "locus",
          EBaseStruct(
            FastSeq(EField("contig", EBinaryRequired, 0), EField("position", EInt32Required, 1))
          ),
          0,
        ),
    ...
    val internalNodeEType = EBaseStruct(
      FastSeq(
        EField(
          "children",
          EArray(
...
case class BgenSettings(
  ...
  val rowPType: PCanonicalStruct = PCanonicalStruct(
    ...
    Array(
      ...
      "offset" -> PInt64(),
      "file_idx" -> PInt32(),
      MatrixType.entriesIdentifier -> PCanonicalArray(
        PCanonicalStruct(
          Array(
            "GT" -> PCanonicalCall(),
