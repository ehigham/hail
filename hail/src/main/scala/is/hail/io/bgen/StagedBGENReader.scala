...
object StagedBGENReader {
  ...
  def decodeRow(
    ...
    cb.invokeVoid(emb, region, cbfis, nSamples, fileIdx, compression, skipInvalidLoci,
      contigRecoding)
    out
  }
  ...
  def queryIndexByPosition(
    ...
    fb.emitWithBuilder { cb =>
      ...
      cb.while_(
        i < len, {
          ...
          val r = index
            .queryIndex(cb, mb.partitionRegion, cb.memoize(indices(i))).loadField(cb, "key").get(cb)
          cb += boxed.update(
            i,
    ...
    { (path: String, indices: Array[Long]) =>
      ctx.r.pool.scopedRegion { r =>
        res
          .apply(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, r)
          .apply(path, indices)
      }
    }
  }
  ...
  override def registerAll(): Unit = {
    registerSCode(
      ...
      case (
        ...
        val header = cb.memoize(
          Code.invokeScalaObject3[HadoopFSDataBinaryReader, String, Long, BgenHeader](
            LoadBgen.getClass,
            "readState",
            cbfis,
            path,
            ...
            iterCurrIdx < iterSize, {
              cb.assign(
                ...
                rowPType.store(
                  ...
                  iterEltRegion,
                  spec.encodedType
                    .buildDecoder(rowPType.virtualType, ecb).apply(cb, iterEltRegion, ib),
                  false,
                ),
              )
        ...
        cb.while_(
          i < groupIndex, {
            cb += iters.update(
              i,
              coerce[NoBoxLongIterator](
                Code.newInstance(ecb.cb, ctor.mb, FastSeq(paths(i), fileSizes(i)))
              ),
            )
            cb.assign(i, i + 1)
          },
        ...
        iw.init(
          ...
          idxPath,
          cb.memoize(
            Code.invokeScalaObject3[String, Map[String, String], Boolean, Map[String, Any]](
