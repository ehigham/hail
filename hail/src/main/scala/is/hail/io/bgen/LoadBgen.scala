...
object LoadBgen {
  def readSamples(fs: FS, file: String): Array[String] = {
    ...
    if (bState.hasIds) {
      ...
      warn(
        s"BGEN file '$file' contains no sample ID block and no sample ID file given.\n" +
  ...
  def readState(is: HadoopFSDataBinaryReader, path: String, byteSize: Long): BgenHeader = {
    ...
    BgenHeader(
      compressType, nSamples, nVariants, headerLength, dataStart, hasIds, version, byteSize, path,
    )
  }
        ...
        if (fs.isDir(file))
          fs.listDirectory(file)
            .filter(fileListEntry =>
              ".*part-[0-9]+(-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})?".r
                .matches(fileListEntry.getPath.toString)
            )
        else
          Array(fileListEntry)
      }
  ...
  def getBgenFileMetadata(ctx: ExecuteContext, files: Array[String], indexFiles: Array[String])
    ...
    headers.zip(indexFiles).map { case (h, indexFile) =>
      ...
      val getKeys = cacheByRG
        .getOrElseUpdate(rg, StagedBGENReader.queryIndexByPosition(ctx, leafSpec, internalSpec))

      val attributes = metadata.attributes
      val skipInvalidLoci = attributes("skip_invalid_loci").asInstanceOf[Boolean]
      val contigRecoding = Option(attributes("contig_recoding"))
        .map(_.asInstanceOf[Map[String, String]]).getOrElse(Map.empty[String, String])
...
object MatrixBGENReader {
  def fullMatrixTypeWithoutUIDs(rg: Option[String]): MatrixType = {
    MatrixType(
      ...
      entryType = TStruct("GT" -> TCall, "GP" -> TArray(TFloat64), "dosage" -> TFloat64),
    )
  }
  ...
  def fullTableType(rg: Option[String]): TableType = {
    val mt = fullMatrixTypeWithoutUIDs(rg)
    val ttNoUID = mt
      .copy(mt.colType.appendKey(MatrixReader.colUIDFieldName, TInt64))
  ...
  def apply(ctx: ExecuteContext, params: MatrixBGENReaderParameters): MatrixBGENReader = {
    ...
    val sampleIds = params.sampleFile
      .map(file => LoadBgen.readSampleFile(fs, file))
...
class MatrixBGENReader(
  ...
  def getSettings(requestedType: TableType): BgenSettings = {
    if (_settings == null || _settings.requestedType != requestedType) {
      _settings = BgenSettings(nSamples, requestedType, referenceGenome, indexAnnotationType)
    }
    _settings
  }
    ...
    variants match {
      case Some(v) =>
        ...
        filePartitionInfo.zipWithIndex.foreach { case (file, fileIdx) =>
          ...
          strictShortKey.partitionBoundsIRRepresentation.value
...
case class BgenPartitionReaderWithVariantFilter(
  ...
  def emitStream(
    ...
    context.toI(cb).flatMap(cb) { case context: SBaseStructValue =>
      ...
      child
        .emitStream(ctx, cb, mb, rangeBound, child.fullRowType.deleteKey(child.uidFieldName))
        .map(cb) { case variantsStream: SStreamValue =>
          ...
          SStreamValue(new StreamProducer {
            ...
            override def initialize(cb: EmitCodeBuilder, outerRegion: Value[Region]): Unit = {
              ...
              val metadata = cb.memoize(
                mb.getObject[IndexedSeq[BgenFileMetadata]](fileMetadata.toFastSeq)
            ...
            override val LproduceElement: CodeLabel = mb.defineAndImplementLabel { cb =>
              ...
              cb.if_(
                currVariantIndex < stopVariantIndex, {
                  val addr = index
                    .queryIndex(cb, vs.elementRegion, currVariantIndex)
                    .loadField(cb, "offset")
                    .get(cb).asLong.value
                  cb += cbfis.invoke[Long, Unit]("seek", addr)

                  val reqTypeNoUID =
                    if (requestedType.hasField(uidFieldName)) requestedType.deleteKey(uidFieldName)
                    else requestedType
                  val sc = StagedBGENReader
                    .decodeRow(cb, elementRegion, cbfis, nSamples, fileIdx, compression,
                      skipInvalidLoci, contigRecoding, reqTypeNoUID, rg)
                    .toI(cb).get(cb)
                  val scUID =
              ...
              val bound = SStackStruct.constructFromArgs(
                ...
                EmitValue.present(
                  if (nextVariant.st.size == 1)
                    nextVariant.insert(
                      cb,
                      elementRegion,
                      nextVariant.st.virtualType
                        .asInstanceOf[TStruct].structInsert(TArray(TString), FastSeq("alleles")),
    ...
    context.toI(cb).map(cb) { case context: SBaseStructValue =>
      ...
      SStreamValue(new StreamProducer {
        ...
        override val LproduceElement: CodeLabel = mb.defineAndImplementLabel { cb =>
          ...
          val addr = index
            .queryIndex(cb, eltRegion, currVariantIndex)
            .loadField(cb, "offset")
            .get(cb).asLong.value
          cb += cbfis.invoke[Long, Unit]("seek", addr)
