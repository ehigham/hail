...
trait FS extends Serializable {
  ...
  final def openCachedNoCompression(filename: String): SeekableDataInputStream = openNoCompression(
    filename
  )
  ...
  final def openNoCompression(filename: String): SeekableDataInputStream = openNoCompression(
    parseUrl(filename)
  )
  ...
  final def createNoCompression(filename: String): PositionedDataOutputStream = createNoCompression(
    parseUrl(filename)
  )
  ...
  final def listDirectory(filename: String): Array[FileListEntry] = listDirectory(
    parseUrl(filename)
  )
  ...
  def deleteOnExit(url: URL): Unit =
    Runtime.getRuntime.addShutdownHook(new Thread(() => delete(url, recursive = false)))
  ...
  def copyMerge(
    ...
    info(
      s"merging ${filesToMerge.length} files totalling " +
  ...
  def copyMergeList(
    ...
    require(srcFileListEntries.forall { fileListEntry =>
      fileListEntry.getPath != destFilename && fileListEntry.isFile
    })
    ...
    using(createNoCompression(destFilename)) { os =>
      ...
      while (i < srcFileListEntries.length) {
        ...
        val lenAdjust: Long =
          if (isBGzip && i < srcFileListEntries.length - 1)
            -28
          else
            0
  ...
  def concatenateFiles(sourceNames: Array[String], destFilename: String): Unit = {
    ...
    info(
      s"merging ${fileListEntries.length} files totalling " +
