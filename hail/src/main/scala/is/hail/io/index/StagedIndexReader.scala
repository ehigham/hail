...
class StagedIndexReader(
  ...
  private[this] val leafChildType = leafPType
    .asInstanceOf[PCanonicalBaseStruct].types(1).asInstanceOf[
      PCanonicalArray
    ].elementType.sType.asInstanceOf[SBaseStruct]
  ...
  def initialize(cb: EmitCodeBuilder, indexPath: Value[String]): Unit = {
    ...
    cb.assign(
      metadata,
      Code
        .invokeScalaObject2[FS, String, IndexMetadataUntypedJSON](
    ...
    cb.assign(
      ...
      Code.newInstance[ByteTrackingInputStream, InputStream](
        cb.emb.openUnbuffered(indexPath.concat("/index"), false)
      ),
    )
  ...
  def queryInterval(cb: EmitCodeBuilder, interval: SIntervalValue): SBaseStructValue = {
    ...
    val (_startIdx, _startLeaf, _endIdx) =
      runQuery(cb, stackInterval, rootLevel, rootOffset, nKeys, startLeaf, isPointQuery = false)
    ...
    cb.if_(
      level > 0, {
        ...
        cb.if_(
          if (isPointQuery) const(true).get else start.ceq(end), {
            ...
            cb.assign(
              nodeOffset,
              children
                .loadElement(cb, start - 1).get(cb).asBaseStruct.loadField(
                  cb,
                  "index_file_offset",
                ).get(cb).asLong.value,
            )
        ...
        cb.assign(
          nodeOffset,
          children
            .loadElement(cb, end - 1).get(cb).asBaseStruct.loadField(cb, "index_file_offset").get(
              cb
            ).asLong.value,
        )
        ...
        cb.assign(
          nodeOffset,
          children
            .loadElement(cb, start - 1).get(cb).asBaseStruct.loadField(cb, "index_file_offset").get(
              cb
            ).asLong.value,
        )
          ...
          cb.invokeSCode(
            cb.emb.ecb.getOrGenEmitMethod(
              ...
              emb.emitSCode { cb =>
                ...
                val region = cb.memoize(
                  cb.emb.ecb.pool().invoke[Region.Size, Region]("getRegion", Region.TINIER)
                )
                val internalNode = internalDec.apply(cb, region, ib)
                val internalNodeAddr = internalPType.store(cb, region, internalNode, false)
                cb += cache
                  .invoke[Long, Region, Long, Unit]("put", offset, region, internalNodeAddr)
                internalNode
              }
            },
            offset,
        ...
        emb.emitSCode { cb =>
          ...
          cb.if_(
            level ceq 0, {
              ...
              val firstIdx = children
                .loadElement(cb, 0).get(cb).asBaseStruct.loadField(cb, "first_idx").get(
                  cb
                ).asInt64.value
              val nKeysPerChild = cb.memoize(
                Code
                  .invokeStatic2[java.lang.Math, Double, Double, Double](
                    "pow",
                    branchingFactor.toD,
                    level.toD,
                  ).toL
              ...
              cb.assign(
                offset,
                children
                  .loadElement(cb, localIdx.toI).get(cb).asBaseStruct.loadField(
                    cb,
                    "index_file_offset",
