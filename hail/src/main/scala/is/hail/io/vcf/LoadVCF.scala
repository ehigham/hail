...
object VCFHeaderInfo {
  ...
  def fromJSON(jv: JValue): VCFHeaderInfo = {
    ...
    def lookupFields(name: String) = (jv \ name)
      .asInstanceOf[JArray].arr.map { case elt: JArray =>
    ...
    def lookupAttrs(name: String) = (jv \ name)
...
object LoadVCF {
  def warnDuplicates(ids: Array[String]): Unit = {
    ...
    if (duplicates.nonEmpty) {
      warn(
        ...
        duplicates.toArray
          .sortBy(-_._2).map { case (id, count) => s"""($count) "$id"""" }.truncatable("\n  "),
      )
    }
  }
  ...
  def headerSignature[T <: VCFCompoundHeaderLine](
    ...
    val (fields, attrs, flags) = lines.asScala.map(line => headerField(line)).unzip3

    val flagFieldNames =
      fields.zip(flags).flatMap { case ((f, _), isFlag) => if (isFlag) Some(f) else None }.toSet
  ...
  def parseHeader(
    ...
    val header = codec
      .readHeader(new BufferedLineIterator(lines.iterator.buffered))
      .getHeaderValue
      .asInstanceOf[htsjdk.variant.vcf.VCFHeader]

    val filterAttrs: VCFAttributes = header.getFilterLines.asScala.toList
      // (ID, description)
    ...
    VCFHeaderInfo(sampleIds, infoSignature, gSignature, filterAttrs, infoAttrs, formatAttrs,
      infoFlagFields)
  }
...
class PartitionedVCFRDD(
  ...
  def compute(split: Partition, context: TaskContext): Iterator[WithContext[String]] = {
    ...
    val chromToQuery =
      if (contigRemappingBc != null) contigRemappingBc.value.getOrElse(p.chrom, p.chrom)
      else p.chrom
...
object MatrixVCFReader {
  def apply(
    ...
    MatrixVCFReader(
      ctx,
      MatrixVCFReaderParameters(files, callFields, entryFloatTypeName, headerFile, sampleIDs,
        nPartitions, blockSizeInMB, minPartitions, rg, contigRecoding, arrayElementsRequired,
        skipInvalidLoci, gzAsBGZ, forceGZ, filterAndReplace, partitionsJSON, partitionsTypeStr),
    )
        ...
        backend.parallelizeAndComputeWithIndex(
         ...
         """.stripMargin
          ...
          if (params.sampleIDs.isEmpty) {
            hd1.sampleIds.iterator.zipAll(hd.sampleIds.iterator, None, None).zipWithIndex.foreach {
              case ((s1, s2), i) =>
                if (s1 != s2) {
                  fatal(
...
class MatrixVCFReader(
  ...
  def partitioner(sm: HailStateManager): Option[RVDPartitioner] = params.partitionsJSON.map {
    partitionsJSON =>
      ...
      val rangeBounds = JSONAnnotationImpex
        .importAnnotation(jv, indexedPartitionsType)
      ...
      new RVDPartitioner(sm, Array("locus"), fullType.keyType, rangeBounds)
  }
  ...
  override def lowerGlobals(ctx: ExecuteContext, requestedGlobalsType: TStruct): IR = {
    ...
    Literal.coerce(
      ...
      fullType.globalType
        .valueSubsetter(requestedGlobalsType)
        .apply(globals),
    )
  }
...
case class GVCFPartitionReader(
  ...
  def rowRequiredness(requestedType: TStruct): RStruct =
    VirtualTypeWithReq(tcoerce[PStruct](fullRowPType.subsetTo(requestedType))).r
      .asInstanceOf[RStruct]
