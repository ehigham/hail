...
object MatrixPLINKReader {
  def fromJValue(ctx: ExecuteContext, jv: JValue): MatrixPLINKReader = {
    ...
    val locusAllelesType = TStruct("locus" -> locusType, "alleles" -> TArray(TString))
...
class MatrixPLINKReader(
  ...
  def executeGeneric(ctx: ExecuteContext): GenericTableValue = {
    ...
    val contextType =
      TStruct("bed" -> TString, "start" -> TInt32, "end" -> TInt32, "partitionIndex" -> TInt32)
    ...
    val body = { (requestedType: TStruct) =>
      ...
      val hasGT = hasEntries && (requestedType
        .fieldType(LowerMatrixIR.entriesFieldName).asInstanceOf[TArray]
      ...
      { (region: Region, theHailClassLoader: HailClassLoader, fs: FS, context: Any) =>
        ...
        table(0) =
          if (localA2Reference) Call2.fromUnphasedDiploidGtIndex(2)
          else Call2.fromUnphasedDiploidGtIndex(0)
        // 1 missing
        table(2) = Call2.fromUnphasedDiploidGtIndex(1)
        table(3) =
          if (localA2Reference) Call2.fromUnphasedDiploidGtIndex(0)
