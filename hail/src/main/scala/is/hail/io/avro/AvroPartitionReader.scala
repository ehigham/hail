    ...
    context.toI(cb).map(cb) { case ctxStruct: SBaseStructValue =>
      ...
      val producer = new StreamProducer {
        ...
        def initialize(cb: EmitCodeBuilder, outerRegion: Value[Region]): Unit = {
          ...
          val dataFileStream = Code
            .newInstance[DataFileStream[GenericRecord], InputStream, DatumReader[GenericRecord]](
              is,
              datumReader,
            )
...
object AvroReader {
  ...
  private[avro] def setRequiredness(schema: Schema, typ: TypeWithRequiredness): Unit = {
    ...
    typ match {
      case t: RStruct =>
        t.fields.foreach { case RField(name, typ, _) =>
