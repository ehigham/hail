...
object PType {
  ...
  def genFields(required: Boolean, genFieldType: Gen[PType]): Gen[Array[PField]] = {
    Gen
      .buildableOf[Array](Gen.zip(Gen.identifier, genFieldType))
      .filter(fields => fields.map(_._1).areDistinct())
      .map(fields =>
        fields.iterator.zipWithIndex.map { case ((k, t), i) => PField(k, t, i) }.toArray
      )
  }
  ...
  val genInsertableStruct: Gen[PStruct] = Gen
    .coin(defaultRequiredGenRatio).flatMap(required =>
      if (required)
...
abstract class PType extends Serializable with Requiredness {
  ...
  def genValue(sm: HailStateManager): Gen[Annotation] =
    if (required) genNonmissingValue(sm)
      ...
      Gen
        .nextCoin(0.05).flatMap(isEmpty => if (isEmpty) Gen.const(null) else genNonmissingValue(sm))
    ...
    this match {
      ...
      case PCanonicalTuple(fields, r) =>
        ...
        PCanonicalTuple(
          fields.flatMap { pf =>
            tt.fieldIndex
              .get(pf.index).map(vi => PTupleField(pf.index, pf.typ.subsetTo(tt.types(vi))))
          },
          r,
        )
