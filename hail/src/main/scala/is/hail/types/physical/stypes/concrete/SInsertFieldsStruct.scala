...
final case class SInsertFieldsStruct(
  ...
  lazy val insertedFieldIndices: Map[Int, Int] = insertedFields.zipWithIndex.map {
    case ((name, _), idx) => virtualType.fieldIdx(name) -> idx
  }.toMap
  ...
  override val fieldEmitTypes: IndexedSeq[EmitType] = virtualType.fieldNames.zipWithIndex.map {
    case (f, idx) =>
      insertedFieldIndices.get(idx) match {
...
//  def storageType(): PType = StoredSTypePType(this, false)
  ...
  override def castRename(t: Type): SType = {
    ...
    val parentCastType = TStruct(
      parentType.fieldNames.map(f =>
...
class SInsertFieldsStructValue(
  ...
  override def _insert(newType: TStruct, fields: (String, EmitValue)*): SBaseStructValue = {
    ...
    val filteredNewFields = st.insertedFields
      .map(_._1)
      .zipWithIndex
