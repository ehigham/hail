...
final case class PCanonicalArray(elementType: PType, required: Boolean = false) extends PArray {
  ...
  class Iterator(
    ...
    private[this] val firstElementOffset = PCanonicalArray.this.firstElementOffset(aoff, length)
  ...
  def constructFromIndicesUnsafe(
    ...
    val push: (EmitCodeBuilder, Value[Int], IEmitCode) => Unit = { case (cb, idx, iec) =>
      iec.consume(
        cb,
        PContainer.unsafeSetElementMissing(cb, this, addr, idx),
        { sc =>
