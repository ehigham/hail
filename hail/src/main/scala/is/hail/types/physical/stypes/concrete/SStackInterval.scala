...
final case class SStackInterval(pointEmitType: EmitType) extends SInterval {
  ...
  override def _coerceOrCopy(
    ...
    value match {
      case value: SStackIntervalValue =>
  ...
  override def castRename(t: Type): SType = SStackInterval(
    pointEmitType.copy(st = pointType.castRename(t.asInstanceOf[TInterval].pointType))
  )
