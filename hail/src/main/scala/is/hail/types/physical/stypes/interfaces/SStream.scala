...
sealed trait SStream extends SType {
  ...
  override def castRename(t: Type): SType = throw new UnsupportedOperationException(
    "rename on stream"
  )

  override def _typeWithRequiredness: TypeWithRequiredness = RIterable(
    elementEmitType.typeWithRequiredness.r
  )
}
  ...
  override def getProducer(mb: EmitMethodBuilder[_]): StreamProducer = {
    if (mb != producer.method)
      throw new RuntimeException(
        "stream used in method different from where it was generated -- " +
