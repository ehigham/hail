...
final case class PCanonicalNDArray(elementType: PType, nDims: Int, required: Boolean = false)
  ...
  def makeColumnMajorStrides(sourceShapeArray: IndexedSeq[Value[Long]], cb: EmitCodeBuilder)
    ...
    for (i <- 0 until nDims)
      if (i == 0) strides(i) = const(elementType.byteSize)
        ...
        strides(i) = cb.memoize(
          strides(i - 1) * (sourceShapeArray(i - 1) > 0L).mux(sourceShapeArray(i - 1), 1L)
        )
  ...
  private def getElementAddress(
    ...
    cb.newLocal[Long](
      ...
      indices.zipWithIndex
        .map { case (requestedElementIndex, strideIndex) =>
  ...
  def constructByCopyingArray(
          ...
          case _ =>
            ...
            cb.for_(
              ...
              elementType.storeAtAddress(
                ...
                region,
                dataValue
                  .loadElement(cb, loopCtr.toI).get(cb, "NDArray elements cannot be missing"),
                true,
              ),
            )
        }
    ...
    cb.invokeSCode(
      mb,
      FastSeq[Param](region, SCodeParam(dataCode)) ++ (newShape.map(CodeParam(_)) ++ strides.map(
        CodeParam(_)
      )): _*
    )
      .asNDArray
  ...
  def unstagedStoreAtAddress(
    ...
    if (equalModuloRequired(srcPType)) { // The situation where you can just memcpy
      Region.copyFrom(
        srcAddress,
        destAddress,
        this.representation.field("shape").typ.byteSize + this.representation
          .field("strides").typ.byteSize,
      )
