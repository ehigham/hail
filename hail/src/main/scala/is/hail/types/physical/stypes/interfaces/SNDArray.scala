...
object SNDArray {
  ...
  def gemv(
    ...
    val ldY = Y.eltStride(0).max(1)
    cb += Code
      .invokeScalaObject11[String, Int, Int, Double, Long, Int, Long, Int, Double, Long, Int, Unit](
        BLAS.getClass,
        "dgemv",
        trans,
        m.toI,
...
trait SNDArray extends SType {
  ...
  override def _typeWithRequiredness: TypeWithRequiredness = RNDArray(
    elementType.typeWithRequiredness.setRequired(true).r
  )
}
...
trait SNDArrayValue extends SValue {
  ...
  def _slice(cb: EmitCodeBuilder, indices: IndexedSeq[NDArrayIndex]): SNDArraySliceValue = {
    ...
    val newSType = SNDArraySlice(
      PCanonicalNDArray(st.pType.elementType, newShape.size, st.pType.required)
    )
  ...
  override def sizeToStoreInBytes(cb: EmitCodeBuilder): SInt64Value = {
    ...
    if (storageType.elementType.containsPointers) {
      SNDArray.coiterate(cb, (this, "A")) { case Seq(elt) =>
