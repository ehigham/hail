...
class SUnreachableLocusValue(override val st: SUnreachableLocus)
  ...
  override def structRepr(cb: EmitCodeBuilder): SBaseStructValue = SUnreachableStruct(
    TStruct("contig" -> TString, "position" -> TInt32)
...
class SUnreachableNDArrayValue(override val st: SUnreachableNDArray)
  ...
  override def shapeStruct(cb: EmitCodeBuilder): SBaseStructValue = SUnreachableStruct(
    TTuple((0 until st.nDims).map(_ => TInt64): _*)
  ).sv
