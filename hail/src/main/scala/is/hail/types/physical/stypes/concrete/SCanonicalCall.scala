...
class SCanonicalCallValue(val call: Value[Int]) extends SCallValue {
  ...
  def containsAllele(cb: EmitCodeBuilder, allele: Value[Int]): Value[Boolean] =
    cb.memoize[Boolean](
      Code.invokeScalaObject2[Int, Int, Boolean](Call.getClass, "containsAllele", call, allele),
      "contains_allele",
    )
  ...
  override def sizeToStoreInBytes(cb: EmitCodeBuilder): SInt64Value = new SInt64Value(
    this.pt.byteSize
  )
}
