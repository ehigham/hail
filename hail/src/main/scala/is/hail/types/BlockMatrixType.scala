...
object BlockMatrixSparsity {
  ...
  def apply(definedBlocks: IndexedSeq[(Int, Int)]): BlockMatrixSparsity = BlockMatrixSparsity(
    Some(definedBlocks)
  )
...
case class BlockMatrixSparsity(definedBlocks: Option[IndexedSeq[(Int, Int)]]) {
  ...
  def definedBlocksCSCIR(nCols: Int): Option[(IR, IR)] = definedBlocksCSC(nCols).map {
    case (rowPos, rowIdx) =>
      val t = TArray(TInt32)
  ...
  override def toString: String =
    definedBlocks
      .map(blocks => blocks.map { case (i, j) => s"($i,$j)" }.mkString("[", ",", "]")).getOrElse(
        "None"
      )
}
