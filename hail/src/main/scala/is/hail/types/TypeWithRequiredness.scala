    ...
    def subsetRT(vt: Type, rt: TypeWithRequiredness): TypeWithRequiredness = {
      val r = (vt, rt) match {
        ...
        case (tt: TTuple, rt: RTuple) =>
          RTuple(
            tt.fields.map(fd => RField(fd.name, subsetRT(fd.typ, rt.fieldType(fd.name)), fd.index))
          )
        case (ts: TStruct, rt: RStruct) =>
          RStruct(
            ts.fields.map(fd => RField(fd.name, subsetRT(fd.typ, rt.field(fd.name)), fd.index))
          )
  ...
  override def _unionWithIntersection(ts: IndexedSeq[BaseTypeWithRequiredness]): Unit = {
    if (eltRequired) {
      ...
      while (i < elementType.children.length) {
        elementType
          .children(i).unionWithIntersection(
            ts.map(t => tcoerce[RIterable](t).elementType.children(i))
          )
        i += 1
      }
    } else
