...
abstract class TBaseStruct extends Type {
  ...
  def isComparableAt(a: Annotation): Boolean = a match {
    case row: Row =>
      row.toSeq.zip(types).forall { case (v, t) => t.typeCheck(v) }
    case _ => false
  }
  ...
  override def _showStr(a: Annotation): String = {
    if (types.isEmpty)
      ...
      Array
        .tabulate(size)(i => types(i).showStr(a.asInstanceOf[Row].get(i)))
        .mkString("(", ",", ")")
    }
  }
  ...
  override def valuesSimilar(a1: Annotation, a2: Annotation, tolerance: Double, absolute: Boolean)
    ...
    a1 == a2 || (a1 != null && a2 != null
      && types
        .zip(a1.asInstanceOf[Row].toSeq).zip(a2.asInstanceOf[Row].toSeq)
        .forall { case ((t, x1), x2) =>
