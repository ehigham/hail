        ...
        if (!elementType.required) {
          ...
          cb.if_(
            nMissingBytes > 0, {
              ...
              cb += out.writeByte(
                (Region.loadByte(
                  array + pArray.missingBytesOffset
                    + (nMissingBytes - 1).toL
                ) & EType.lowBitMask(prefixLen)).toB
              )
            },
          )
        }
    ...
    if (elementType.required) {
      ...
      cb.if_(
        (len % 64).cne(0), {
          ...
          val lastMissingBlockOff = cb.memoize(
            UnsafeUtils.roundDownAlignment(arrayType.pastLastMissingByteOff(array, len) - 1, 8)
          )
        ...
        mbyteOffset < pastLastMissingByteOff, {
          ...
          cb.while_(
            presentBits.cne(0L), {
              ...
              val elemOff =
                cb.memoize(arrayType.incrementElementOffset(blockOff, inBlockIndexToPresentValue))
              readElemF(cb, region, elemOff, in)
