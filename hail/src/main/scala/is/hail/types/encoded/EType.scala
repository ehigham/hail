...
abstract class EType extends BaseType with Serializable with Requiredness {
  ...
  final def buildDecoder(t: Type, kb: EmitClassBuilder[_]): StagedDecoder = {
    ...
    { (cb: EmitCodeBuilder, r: Value[Region], ib: Value[InputBuffer]) => cb.invokeSCode(mb, r, ib) }
  }
...
object EType {
  ...
  def buildEncoder(ctx: ExecuteContext, et: EType, pt: PType)
    ...
    if (encoderCache.containsKey(k)) {
      ...
      log.info(
        s"encoder cache miss ($encoderCacheHits hits, $encoderCacheMisses misses, " +
  ...
  def buildDecoderToRegionValue(ctx: ExecuteContext, et: EType, t: Type)
    ...
    if (decoderCache.containsKey(k)) {
      ...
      log.info(
        s"decoder cache miss ($decoderCacheHits hits, $decoderCacheMisses misses, " +
  ...
  def fromTypeAndAnalysis(t: Type, r: TypeWithRequiredness): EType = t match {
    ...
    case TLocus(_) =>
      EBaseStruct(
        Array(EField("contig", EBinary(true), 0), EField("position", EInt32(true), 1)),
        required = r.required,
      )
  ...
  def fromPythonTypeEncoding(t: Type): EType = t match {
    ...
    case TLocus(_) =>
      EBaseStruct(
        Array(EField("contig", EBinary(false), 0), EField("position", EInt32(false), 1)),
        required = false,
      )
