...
case class MatrixType(
  ...
  lazy val entriesRVType: TStruct = TStruct(MatrixType.entriesIdentifier -> TArray(entryType))
  ...
  lazy val entriesTableType: TableType = {
    val resultStruct = TStruct(
      (rowType.fields ++ colType.fields ++ entryType.fields).map(f => f.name -> f.typ): _*
    )
  ...
  @transient lazy val globalEnv: Env[Type] = Env
    .empty[Type]
    .bind("global" -> globalType)

  @transient lazy val rowEnv: Env[Type] = Env
    .empty[Type]
    .bind("global" -> globalType)
    .bind("va" -> rowType)

  @transient lazy val colEnv: Env[Type] = Env
    .empty[Type]
    .bind("global" -> globalType)
    .bind("sa" -> colType)

  @transient lazy val entryEnv: Env[Type] = Env
    .empty[Type]
    .bind("global" -> globalType)
    .bind("sa" -> colType)
    .bind("va" -> rowType)
