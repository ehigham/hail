...
trait WrappedClassBuilder[C] extends WrappedModuleBuilder {
  ...
  def getOrGenMethod(
    baseName: String,
    key: Any,
    argsInfo: IndexedSeq[TypeInfo[_]],
    returnInfo: TypeInfo[_],
  )(
    body: MethodBuilder[C] => Unit
  ): MethodBuilder[C] =
...
class ClassBuilder[C](
    ...
    if (maybeGenericParameterTypeInfo.exists(_.isGeneric) || maybeGenericReturnTypeInfo.isGeneric) {
      ...
      generic.emitWithBuilder { cb =>
        maybeGenericReturnTypeInfo.castToGeneric(
          ...
          m.invoke(
            cb,
            maybeGenericParameterTypeInfo.zipWithIndex.map { case (ti, i) =>
  ...
  def getOrDefineLazyField[T: TypeInfo](setup: Code[T], id: Any): Value[T] =
    lazyFieldMemo
      .getOrElseUpdate(id, genLazyFieldThisRef[T](setup)).asInstanceOf[ThisLazyFieldRef[T]]
...
class MethodBuilder[C](
  ...
  def emitWithBuilder[T](f: (CodeBuilder) => Code[T]): Unit = emit(
    CodeBuilder.scopedCode[T](this)(f)
  )
  ...
  def invokeCode[T](args: Value[_]*): Code[T] = {
    ...
    if (returnTypeInfo eq UnitInfo) {
      ...
      val value = if (isStatic) {
        ...
        lir.methodInsn(INVOKEVIRTUAL, lmethod, lir.load(new lir.Parameter(null, 0, cb.ti)) +: argvs)
      }
