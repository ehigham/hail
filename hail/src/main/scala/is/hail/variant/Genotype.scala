...
object Genotype {
  ...
  def genExtreme(nAlleles: Int): Gen[Annotation] =
    Gen.frequency((100, genExtremeNonmissing(nAlleles)), (1, Gen.const(null)))
  ...
  def genRealisticNonmissing(nAlleles: Int): Gen[Annotation] = {
    ...
    val gg =
      for {
        ...
        alleleFrequencies <- Gen
          .buildableOfN[Array](nAlleles, Gen.choose(1e-6, 1d)) // avoid divison by 0
          .map { rawWeights =>
        ...
        c <- Gen.option(
          Gen
            .zip(Gen.chooseWithWeights(alleleFrequencies), Gen.chooseWithWeights(alleleFrequencies))
  ...
  def genRealistic(nAlleles: Int): Gen[Annotation] =
    Gen.frequency((100, genRealisticNonmissing(nAlleles)), (1, Gen.const(null)))
  ...
  def genGenericCallAndProbabilitiesGenotype(nAlleles: Int): Gen[Annotation] = {
    ...
    val gg = for (gp <- Gen.option(Gen.partition(nGenotypes, 32768))) yield {
      ...
      Row(c.orNull, gp.map(gpx => gpx.map(p => p.toDouble / 32768): IndexedSeq[Double]).orNull)
    }
    Gen.frequency((100, gg), (1, Gen.const(null)))
  }
}
