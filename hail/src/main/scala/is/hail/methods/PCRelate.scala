...
case class PCRelate(
  ...
  def computeResult(ctx: ExecuteContext, _blockedG: M, pcs: BDM[Double]): Result[M] = {
    ...
    val variance =
      cacheWhen(PhiK2)(ctx, mu.map(mu => if (java.lang.Double.isNaN(mu)) 0.0 else mu * (1.0 - mu)))
    ...
    if (statistics >= PhiK2) {
      val k2 = cacheWhen(PhiK2K0)(ctx, this.k2(ctx, phi, mu, variance, blockedG))
      if (statistics >= PhiK2K0) {
        val k0 =
          cacheWhen(PhiK2K0K1)(ctx, this.k0(ctx, phi, mu, k2, blockedG, ibs0(ctx, blockedG, mu)))
  ...
  private[methods] def ibs0(ctx: ExecuteContext, g: M, mu: M): M = {
    val homalt =
      BlockMatrix
        .map2((g, mu) => if (java.lang.Double.isNaN(mu) || g != 2.0) 0.0 else 1.0)(g, mu)

    val homref =
      BlockMatrix
        .map2((g, mu) => if (java.lang.Double.isNaN(mu) || g != 0.0) 0.0 else 1.0)(g, mu)
