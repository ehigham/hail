...
object IBD {
  ...
  def computeIBDMatrix(
    ...
    val unnormalizedIbse = input.rvd
    ...
    val chunkedGenotypeMatrix = input.rvd
      .mapPartitions { (_, it) =>
        val view = HardCallView(rowPType)
        it.map { ptr =>
          view.set(ptr)
    ...
    val joined = ContextRDD.weaken(
      chunkedGenotypeMatrix
        .join(chunkedGenotypeMatrix)
  ...
  private val ibdPType =
    PCanonicalStruct(
      required = true,
      Array(("i", PCanonicalString()), ("j", PCanonicalString())) ++ ExtendedIBDInfo.pType.fields
