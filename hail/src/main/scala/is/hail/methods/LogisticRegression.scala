...
case class LogisticRegression(
  ...
  def execute(ctx: ExecuteContext, mv: MatrixValue): TableValue = {
    ...
    info(
      s"logistic_regression_rows: running $test on $n samples for response variable y,\n"
    ...
    val nullFits = (0 until yVecs.cols).map { col =>
      ...
      if (!nullFit.converged)
        if (logRegTest == LogisticFirthTest)
          ...
          fatal(
            "Failed to fit logistic regression null model (standard MLE with covariates only): " + (if (
                                                                                                      nullFit.exploded
                                                                                                    )
