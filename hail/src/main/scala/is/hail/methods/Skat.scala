...
case class Skat(
  ...
  def execute(ctx: ExecuteContext, mv: MatrixValue): TableValue = {
    ...
    if (logistic) {
      ...
      if (badVals.nonEmpty)
        fatal(
          s"For logistic SKAT, phenotype must be Boolean or numeric with value 0 or 1 for each complete " +
    ...
    def logisticSkat(): RDD[Row] = {
      val (sqrtV, res, cinvXtV) =
        if (k > 0) {
          val logRegM = new LogisticRegressionModel(cov, y)
            .fit(maxIter = logistic_max_iterations, tol = logistic_tolerance)
          if (!logRegM.converged)
            fatal(
              "Failed to fit logistic regression null model (MLE with covariates only): " + (if (
                                                                                               logRegM.exploded
                                                                                             )
