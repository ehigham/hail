...
object Nirvana {
  ...
  def annotate(ctx: ExecuteContext, tv: TableValue, config: String, blockSize: Int): TableValue = {
    ...
    val supplementaryAnnotationDirectoryOpt = Option(
      properties.getProperty("hail.nirvana.supplementaryAnnotationDirectory")
    )
    val supplementaryAnnotationDirectory =
      if (supplementaryAnnotationDirectoryOpt.isEmpty) List[String]()
    ...
    val cmd: List[String] = List[String](dotnet, s"$nirvanaLocation") ++
      ...
      List("--disable-recomposition", "-r", reference, "-i", "-", "-o", "-")
        ...
        it.map { ptr =>
          ...
          .flatMap { block =>
            ...
            val kt = jt.filter(_.startsWith("{\"chromosome")).map { s =>
              val a = JSONAnnotationImpex
                .importAnnotation(JsonMethods.parse(s), nirvanaSignature, warnContext = warnContext)
              val locus =
