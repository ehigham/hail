...
object VEP {
  ...
  private def getCSQHeaderDefinition(cmd: Array[String], confEnv: Map[String, String])
    ...
    val (jt, err, proc) = List((Locus("1", 13372), FastSeq("G", "C"))).iterator
                  ...
                  nonStarToOriginalVariant.get(vepv) match {
                    case Some((locus, alleles)) =>
                      ...
                      val a = x match {
                        ...
                        case None =>
                          warn(s"No CSQ INFO field for VEP output variant ${VariantMethods
                              .locusAllelesToString(vepLocus, vepAlleles)}.\nVEP output: $s.")
                          null
                      }
