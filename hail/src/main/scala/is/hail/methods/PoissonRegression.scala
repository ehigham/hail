...
case class PoissonRegression(
  ...
  def execute(ctx: ExecuteContext, mv: MatrixValue): TableValue = {
    ...
    info(
      s"poisson_regression_rows: running $test on $n samples for response variable y,\n"
    ...
    if (!nullFit.converged)
      fatal(
        "Failed to fit poisson regression null model (standard MLE with covariates only): " + (if (
                                                                                                 nullFit.exploded
                                                                                               )
