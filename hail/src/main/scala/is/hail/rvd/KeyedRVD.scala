...
class KeyedRVD(val rvd: RVD, val key: Int) {
  ...
  def orderedLeftIntervalJoin(
    ...
    rvd.intervalAlignAndZipPartitions(executeContext, right.rvd) { t: PStruct =>
  ...
  def orderedLeftJoinDistinct(
    ...
    rvd.alignAndZipPartitions(
      ...
      joiner(
        ctx,
        OrderedRVIterator(lTyp, leftIt, ctx, sm).leftJoinDistinct(
          OrderedRVIterator(rTyp, rightIt, ctx, sm)
        ),
      )
    }
  ...
  def orderedMerge(
    ...
    if (key == 0)
      return RVD.unkeyed(
        ...
        ContextRDD.union(rvd.sparkContext, Seq(this.rvd.crdd, right.rvd.crdd)),
      )
