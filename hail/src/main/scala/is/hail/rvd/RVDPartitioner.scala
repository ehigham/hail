...
class RVDPartitioner(
  ...
  def extendKeySamePartitions(newKType: TStruct): RVDPartitioner = {
    require(kType isPrefixOf newKType)
    new RVDPartitioner(sm, newKType, rangeBounds, allowedOverlap)
  }
...
object RVDPartitioner {
  ...
  def unkeyed(sm: HailStateManager, numPartitions: Int): RVDPartitioner = {
    ...
    new RVDPartitioner(sm, TStruct.empty, Array.fill(numPartitions)(unkeyedInterval), 0)
  }
  ...
  def fromKeySamples(
    ...
    val partitionEdges = Array
      .tabulate(nPartitions - 1) { i =>
  ...
  def isValid(
    ...
    rangeBounds.zip(rangeBounds.tail).forall { case (left: Interval, right: Interval) =>
      val r = PartitionBoundOrdering(sm, kType).intervalEndpointOrdering
        .lteqWithOverlap(allowedOverlap)(left.right, right.left)
      if (!r)
        log
          .info(s"invalid partitioner: !lteqWithOverlap($allowedOverlap)($left.right, $right.left)")
      r
    }
  }
