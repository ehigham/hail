  ...
  def readZippedLowered(
    ...
    newPartitioner match {
      case None =>
        ...
        val contextsValue: IndexedSeq[Any] = (leftParts, rightParts, leftParts.indices).zipped
      ...
      case Some(np) =>
        ...
        val contextsValues: IndexedSeq[Row] = partsAndIntervals.zipWithIndex.map {
          case ((partPath, interval), partIdx) =>
            Row(
              partIdx.toLong,
  ...
  def readTableStage(
    ...
    case None =>
      ...
      if (!part.kType.fieldNames.startsWith(requestedType.key))
        fatal(
          s"Error while reading table $path: legacy table written without key." +
      ...
      val contexts = ir.ToStream(
        ir.Literal(
          TArray(ctxType),
          absolutePartPaths(path).zipWithIndex.map { case (x, i) => Row(i.toLong, x) }.toFastSeq,
        )
      )
...
object RVDSpecMaker {
  def apply(
    ...
    JSONAnnotationImpex
      .exportAnnotation(partitioner.rangeBounds.toFastSeq, partitioner.rangeBoundsType),
    indexSpec,
    attrs,
  )
}
...
case class RVDSpecMaker(
  ...
  def apply(partFiles: Array[String]): AbstractRVDSpec =
    Option(indexSpec) match {
      case Some(ais) => IndexedRVDSpec2(key, codecSpec, ais, partFiles, bounds, attrs)
      case None =>
        OrderedRVDSpec2(
          key, codecSpec, partFiles, bounds, attrs,
        )
    }
}
...
case class IndexedRVDSpec2(
  ...
  def partitioner(sm: HailStateManager): RVDPartitioner = {
    ...
    new RVDPartitioner(
      ...
      keyType,
      JSONAnnotationImpex
        .importAnnotation(_jRangeBounds, rangeBoundsType, padNulls = false).asInstanceOf[IndexedSeq[
          Interval
        ]],
    )
  }
  ...
  override def readTableStage(
    ...
    case Some(np) =>
      ...
      { (globals: IR) =>
        val ts = TableStage(globals, tmpPartitioner, TableStageDependency.none, contexts, body)
