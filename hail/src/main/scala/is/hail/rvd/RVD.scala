...
class RVD(
  ...
  def stabilize(ctx: ExecuteContext, enc: AbstractTypedCodecSpec): RDD[Array[Byte]] = {
    ...
    crdd
      .mapPartitions(it => RegionValue.toBytes(theHailClassLoaderForSparkWorkers, makeEnc, it)).run
  }
            ...
            if (!partitionerBc.value.rangeBounds(i).contains(ord, kUR))
              fatal(
                s"""RVD error! Unexpected key in partition $i
                   ...
                   |  Range of partition IDs for key: [${partitionerBc.value
                    .lowerBound(kUR)}, ${partitionerBc.value.upperBound(kUR)})
  ...
  def truncateKey(newKey: IndexedSeq[String]): RVD = {
    ...
    if (typ.key == newKey)
      ...
      copy(typ = typ.copy(key = newKey), partitioner = partitioner.coarsen(newKey.length))
  }
  ...
  def repartition(
    ...
    if (shuffle) {
      ...
      val filtered: RVD =
        if (filter)
          filterWithContext[(UnsafeRow, SelectFieldsRow)](
            { case (_, _) =>
  ...
  def naiveCoalesce(
    ...
    if (newPartitioner == partitioner) {
      ...
      new RVD(typ, newPartitioner, crdd.coalesceWithEnds(newPartEnd))
    }
  }
  ...
  def coalesce(
    ...
    if (shuffle) {
      ...
      val newPartitioner =
        RVD.calculateKeyRanges(ctx, newType, keyInfo, shuffled.getNumPartitions, newType.key.length)

      if (newPartitioner.numPartitions < maxPartitions)
        warn(
          s"coalesced to ${newPartitioner.numPartitions} " +
      ...
      newPartEnd = newPartEnd.zipWithIndex
        .filter { case (end, i) => i == 0 || newPartEnd(i) != newPartEnd(i - 1) }
        .map(_._1)
  ...
  def distinctByKey(execCtx: ExecuteContext): RVD = {
    ...
    repartition(execCtx, partitioner.strictify())
      .mapPartitions(typ)((ctx, it) =>
        OrderedRVIterator(localType, it.toIteratorRV(ctx.r), ctx, sm).staircase
          .map(_.value.offset)
      )
  }
  ...
  def localSort(newKey: IndexedSeq[String]): RVD = {
    ...
    new RVD(newType, partitioner.copy(kType = newType.kType.virtualType), sortedRDD)
  }
  ...
  def mapPartitions(
    ...
    require(newTyp.kType isPrefixOf typ.kType)
    RVD(newTyp, partitioner.coarsen(newTyp.key.length), crdd.cmapPartitions(f))
  }
  ...
  def mapPartitionsWithIndex(
    ...
    require(newTyp.kType isPrefixOf typ.kType)
    RVD(newTyp, partitioner.coarsen(newTyp.key.length), crdd.cmapPartitionsWithIndex(f))
  }
  ...
  def tail(n: Long, partitionCounts: Option[IndexedSeq[Long]]): RVD = {
    ...
    val newRDD = crdd
      .cmapPartitionsAndContextWithIndex(
        { case (i, ctx, f) =>
          val it = f.next()(ctx)
          if (i == idxFirst) {
  ...
  def filterWithContext[C](makeContext: (Int, RVDContext) => C, f: (C, RVDContext, Long) => Boolean)
    : RVD = {
    val crdd: ContextRDD[Long] = this.crdd.cmapPartitionsWithContextAndIndex {
      (i, consumerCtx, iteratorToFilter) =>
  ...
  def filterToIntervals(intervals: RVDPartitioner): RVD = {
    ...
    val pred: (RVDContext, Long) => Boolean = (ctx: RVDContext, ptr: Long) => {
      ...
      val key = Row.fromSeq(kRowFieldIdx.map(i => ur.get(i)))
    ...
    val newPartitionIndices = Iterator
      .range(0, partitioner.numPartitions)
  ...
  def combine[U: ClassTag, T: ClassTag](
    ...
    if (tree) {
      ...
      val scale = math.max(math.ceil(math.pow(getNumPartitions, 1.0 / depth)).toInt, 2)
  ...
  def collect(execCtx: ExecuteContext): Array[Row] = {
    ...
    execCtx.r.pool.scopedRegion { region =>
      RegionValue
        .fromBytes(execCtx.theHailClassLoader, dec, region, encodedData.iterator)
        .map { ptr =>
  ...
  def orderedLeftJoinDistinctAndInsert(
    ...
    orderedLeftJoinDistinct(right, right.typ.key.length, joiner, typ.copy(rowType = newRowType))
  }
  ...
  def alignAndZipPartitions(
    ...
    RVD(
      ...
      crdd = left.crdd.toCRDDRegionValue
        .czipPartitions(
  ...
  def intervalAlignAndZipPartitions(
    ...
    require(
      that.rowType.field(that.typ.key(0)).typ.asInstanceOf[TInterval].pointType == rowType
        .field(typ.key(0)).typ
    )
...
object RVD {
  ...
  def unkeyed(rowType: PStruct, crdd: ContextRDD[Long]): RVD =
    new RVD(RVDType(rowType, FastSeq()), RVDPartitioner.unkeyed(null, crdd.getNumPartitions), crdd)
  ...
  def writeRowsSplitFiles(
    ...
    val partFilePartitionCounts = execCtx.timer.time("writeOriginUnionRDD")(
      new ContextRDD(new OriginUnionRDD(first.crdd.rdd.sparkContext, rvds.map(_.crdd.rdd), partF))
        .collect()
    )
