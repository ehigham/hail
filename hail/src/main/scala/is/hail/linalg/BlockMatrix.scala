...
object BlockMatrix {
  ...
  def apply(gp: GridPartitioner, piBlock: (GridPartitioner, Int) => ((Int, Int), BDM[Double]))
    ...
    new BlockMatrix(
      new RDD[((Int, Int), BDM[Double])](SparkBackend.sparkContext("BlockMatrix.apply"), Nil) {
        ...
        protected def getPartitions: Array[Partition] =
          Array.tabulate(gp.numPartitions)(pi => new Partition { def index: Int = pi })
  ...
  def fromBreezeMatrix(lm: BDM[Double], blockSize: Int): M = {
    ...
    val localBlocksBc = Array.tabulate(gp.numPartitions) { pi =>
      ...
      HailContext.backend.broadcast(
        lm(iOffset until iOffset + blockNRows, jOffset until jOffset + blockNCols).copy
      )
    }
  ...
  def exportBlockMatrices(
    ...
    val partitionCounts = collectMatrices(bms)
      .mapPartitionsWithIndex { case (i, it) =>
        ...
        using(
          new PrintWriter(new BufferedWriter(new OutputStreamWriter(fsBc.value.create(path))))
        ) { f =>
  ...
  def writeBlockMatrices(
    ...
    val partMap = ordd.partitions
      .map(part => part.asInstanceOf[OriginUnionPartition]).map(oup =>
...
class BlockMatrix(
  ...
  def write(
    ...
    info(
      s"wrote matrix with $nRows ${plural(nRows, "row")} " +
  ...
  def blockMap2(
    ...
    val newBlocks = blocks.zipPartitions(that.blocks, preservesPartitioning = true) {
      (thisIter, thatIter) =>
...
private class BlockMatrixFilterRDD(bm: BlockMatrix, keepRows: Array[Long], keepCols: Array[Long])
  ...
  private val blockParentMap = (0 until tempDenseGP.numPartitions)
    .map { blockId =>
  ...
  override def getDependencies: Seq[Dependency[_]] =
    Array[Dependency[_]](new NarrowDependency(bm.blocks) {
...
private class BlockMatrixFilterColsRDD(bm: BlockMatrix, keep: Array[Long])
  ...
  @transient private val blockParentMap = (0 until tempDenseGP.numPartitions)
    .map { blockId =>
...
private class BlockMatrixTransposeRDD(bm: BlockMatrix)
  ...
  def compute(split: Partition, context: TaskContext): Iterator[((Int, Int), BDM[Double])] =
    bm.blocks
      .iterator(split.asInstanceOf[BlockMatrixTransposeRDDPartition].prevPartition, context)
...
class WriteBlocksRDD(
  ...
  def compute(split: Partition, context: TaskContext): Iterator[(Int, String)] = {
    ...
    val (blockPartFiles, outPerBlockCol, paths) = Array
      .tabulate(gp.nBlockCols) { blockCol =>
    ...
    writeBlocksPart.range.zip(writeBlocksPart.parentPartitions).foreach { case (pi, pPart) =>
      using(RVDContext.default(SparkTaskContext.get().getRegionPool())) { ctx =>
        ...
        while (it.hasNext && i < nRowsInBlock) {
          ...
          var blockCol = 0
          var colIdx = 0
          val colIt = entryArrayType
            .elementIterator(entryArrayOffset, entryArrayType.loadLength(entryArrayOffset))
...
class BlockMatrixCachedPartFile(
  ...
  private[this] def fillCache(): Unit = {
    ...
    using(fs.open(filename)) { is =>
      ...
      val doublesToRead = math.min(cacheCapacity - startWritingAt, rows * cols - fileIndex)
