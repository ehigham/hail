...
package object utils
  ...
  def genDNAString: Gen[String] = Gen
    .stringOf(genBase)
    .resize(12)
    .filter(s => !s.isEmpty)
  ...
  def partitionPath(p: Partition): String = {
    p.getClass.getCanonicalName match {
      case "org.apache.spark.rdd.SqlNewHadoopPartition" =>
        val split = invokeMethod(invokeMethod(p, "serializableHadoopSplit"), "value")
          .asInstanceOf[NewFileSplit]
  ...
  def toMapIfUnique[K, K2, V](
    ...
    if (dupes.nonEmpty) {
      ...
      Right(grouped.map { case (k, m) => k -> m.map(_._2).head }.toMap)
    }
  }
...
object GenericIndexedSeqSerializer extends Serializer[IndexedSeq[_]] {
  ...
  override def serialize(implicit format: Formats) = { case seq: IndexedSeq[_] =>
    JArray(seq.map(Extraction.decompose).toList)
  }
