...
object Gen {
  ...
  val identifierLeadingChars = (0 to 127)
    .map(_.toChar)
    .filter(c => c == '_' || c.isLetter)

  val identifierChars = (0 to 127)
    .map(_.toChar)
    .filter(c => c == '_' || c.isLetterOrDigit)

  val plinkSafeStartOfIdentifierChars = (0 to 127)
    .map(_.toChar)
    .filter(c => c.isLetter)

  val plinkSafeChars = (0 to 127)
    .map(_.toChar)
  ...
  def subset[T](s: Set[T]): Gen[Set[T]] = Gen.parameterized { p =>
    Gen.choose(0.0, 1.0).map(cutoff => s.filter(_ => p.rng.getRandomGenerator.nextDouble <= cutoff))
  }
  ...
  def denseMatrix[T: ClassTag: Zero](n: Int, m: Int, g: Gen[T]): Gen[DenseMatrix[T]] = Gen {
    (p: Parameters) => DenseMatrix.fill[T](n, m)(g.resize(p.size / (n * m))(p))
  }
  ...
  def interestingPosInt: Gen[Int] =
    oneOfGen(oneOf(1, 2, Int.MaxValue - 1, Int.MaxValue), choose(1, 100), posInt)
