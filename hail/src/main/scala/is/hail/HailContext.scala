...
object HailContext {
  ...
  def apply(backend: Backend, branchingFactor: Int = 50, optimizerIterations: Int = 3)
...

    {
      ...
      implicitly[
        BinaryRegistry[DenseMatrix[Double], Vector[Double], OpMulMatrix.type, DenseVector[Double]]
      ].register(DenseMatrix.implOpMulMatrix_DMD_DVD_eq_DVD)
    }
...
class HailContext private (
  ...
  private[this] def fileAndLineCounts(
    ...
    val regexp = regex.r
    SparkBackend
      .sparkContext("fileAndLineCounts").textFilesLines(fs.globAll(files).map(_.getPath))
