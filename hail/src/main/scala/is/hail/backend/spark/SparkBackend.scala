...
object SparkBackend {
  ...
  def checkSparkConfiguration(sc: SparkContext): Unit = {
    ...
    if (
      !conf
        .getOption("spark.kryo.registrator").exists(
  ...
  def getOrCreate(
    ...
    if (theSparkBackend == null)
      return SparkBackend(sc, appName, master, local, logFile, quiet, append,
        skipLoggingConfiguration, minBlockSize, tmpdir, localTmpdir, gcsRequesterPaysProject,
    ...
    val initializedMinBlockSize =
      theSparkBackend.sc.getConf
        .getLong("spark.hadoop.mapreduce.input.fileinputformat.split.minsize", 0L) / 1024L / 1024L
...
class SparkBackend(
  ...
  private[this] val theHailClassLoader: HailClassLoader = new HailClassLoader(
    ...
    val rdd =
      new RDD[(Try[Array[Byte]], Int)](sc, sparkDeps) {
        ...
        override def compute(partition: Partition, context: TaskContext)
          ...
          val result = Try(
            f(sp.data, SparkTaskContext.get(), theHailClassLoaderForSparkWorkers, fs)
          )
  ...
  override def execute(
    ...
    withExecuteContext("SparkBackend.execute") { ctx =>
      ...
      ctx.timer.finish()
      val timings =
        if (timed) Serialization.write(Map("timings" -> ctx.timer.toMap))(new DefaultFormats {})
        else ""
  ...
  def pyReadMultipleMatrixTables(jsonQuery: String): java.util.List[MatrixIR] = {
    ...
    val paths =
      kvs("paths").asInstanceOf[json4s.JArray].arr.toArray.map { case json4s.JString(s) => s }

    val intervalPointType = parseType(kvs("intervalPointType").asInstanceOf[json4s.JString].s)
    val intervalObjects = JSONAnnotationImpex
      .importAnnotation(kvs("intervals"), TArray(TInterval(intervalPointType)))
  ...
  override def lowerDistributedSort(
    ...
    val rdd = rvd
      .keyedEncodedRDD(ctx, codec, sortFields.map(_.field)).sortBy(
        _._1,
