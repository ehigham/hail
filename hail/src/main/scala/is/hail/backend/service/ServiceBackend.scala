...
class ServiceBackend(
    ...
    val jobs = collection.zipWithIndex.map { case (_, i) =>
      ...
      JObject(
        ...
        "process" -> JObject(
          ...
          "command" -> JArray(
            List(JString(Main.WORKER), JString(root), JString(s"$i"), JString(s"$n"))
          ),
          "type" -> JString("jvm"),
  ...
  private[this] def execute(ctx: ExecuteContext, _x: IR, bufferSpecString: String): Array[Byte] = {
    TypeCheck(ctx, _x)
    Validate(_x)
    val x = LoweringPipeline
      .darrayLowerer(true)(DArrayLowering.All).apply(ctx, _x)
      .asInstanceOf[IR]
    if (x.typ == TVoid) {
  ...
  def withExecuteContext[T](methodName: String): (ExecuteContext => T) => T = { f =>
    ExecutionTimer.logTime(methodName) { timer =>
      ExecuteContext.scoped(
        tmpdir, "file:///tmp", this, fs, timer, null, theHailClassLoader, references, flags,
...
object ServiceBackendAPI {
  ...
  def main(argv: Array[String]): Unit = {
    ...
    val deployConfig =
      DeployConfig.fromConfigFile(s"$scratchDir/secrets/deploy-config/deploy-config.json")
