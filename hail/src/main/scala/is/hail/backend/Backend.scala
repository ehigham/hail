...
abstract class Backend {
  ...
  def fromFASTAFile(
    ...
    withExecuteContext("fromFASTAFile") { ctx =>
      val rg = ReferenceGenome.fromFASTAFile(ctx, name, fastaFile, indexFile, xContigs, yContigs,
        mtContigs, parInput)
  ...
  def importFam(path: String, isQuantPheno: Boolean, delimiter: String, missingValue: String)
    ...
    withExecuteContext("importFam") { ctx =>
      LoadPlink
        .importFamJSON(ctx.fs, path, isQuantPheno, delimiter, missingValue).getBytes(
