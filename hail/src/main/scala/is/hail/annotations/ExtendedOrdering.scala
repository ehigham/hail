...
object ExtendedOrdering {
  ...
  def setOrdering(ord: ExtendedOrdering, _missingEqual: Boolean = true): ExtendedOrdering =
    new ExtendedOrdering {
      ...
      override def ltNonnull(x: T, y: T): Boolean =
        saOrd.ltNonnull(x.asInstanceOf[Iterable[T]].toArray, y.asInstanceOf[Iterable[T]].toArray)

      override def lteqNonnull(x: T, y: T): Boolean =
        saOrd.lteqNonnull(x.asInstanceOf[Iterable[T]].toArray, y.asInstanceOf[Iterable[T]].toArray)

      override def equivNonnull(x: T, y: T): Boolean =
        saOrd.equivNonnull(x.asInstanceOf[Iterable[T]].toArray, y.asInstanceOf[Iterable[T]].toArray)
    }
  ...
  def mapOrdering(ord: ExtendedOrdering, _missingEqual: Boolean = true): ExtendedOrdering =
    new ExtendedOrdering {
      ...
      def compareNonnull(x: T, y: T): Int =
        saOrd.compareNonnull(toArrayOfT(x), toArrayOfT(y))

      override def ltNonnull(x: T, y: T): Boolean =
        saOrd.ltNonnull(toArrayOfT(x), toArrayOfT(y))

      override def lteqNonnull(x: T, y: T): Boolean =
        saOrd.lteqNonnull(toArrayOfT(x), toArrayOfT(y))

      override def equivNonnull(x: T, y: T): Boolean =
        saOrd.equivNonnull(toArrayOfT(x), toArrayOfT(y))
    }
