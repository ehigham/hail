...
class ReferenceGenomeSuite extends HailSuite {
  ...
  @Test def testAssertions() {
    ...
    TestUtils.interceptFatal("No lengths given for the following contigs:")(
      ReferenceGenome("test", Array("1", "2", "3"), Map("1" -> 5))
    )
    ...
    TestUtils.interceptFatal("The contig name for PAR interval")(
      ReferenceGenome(
        "test",
        Array("1", "2", "3"),
        Map("1" -> 5, "2" -> 5, "3" -> 5),
    ...
    TestUtils.interceptFatal("in both X and Y contigs.")(
      ReferenceGenome(
        "test",
        Array("1", "2", "3"),
        Map("1" -> 5, "2" -> 5, "3" -> 5),
  ...
  @Test def testWriteToFile() {
    ...
    assert(
      (rg.contigs sameElements gr2.contigs) &&
  ...
  @Test def testFasta() {
    ...
    assert(
      fr.lookup(
        Interval(Locus("a", 1), Locus("a", 5), includesStart = true, includesEnd = false)
      ) == "AGGT"
    )
    assert(
      fr.lookup(
        Interval(Locus("a", 20), Locus("b", 5), includesStart = false, includesEnd = false)
      ) == "ACGTATAAT"
    )
    assert(
      fr.lookup(
        Interval(Locus("a", 20), Locus("c", 5), includesStart = false, includesEnd = false)
  ...
  @Test def testSerializeWithLiftoverOnFB() {
    withExecuteContext() { ctx =>
      ...
      fb.emit(
        rgfield.invoke[String, Locus, Double, (Locus, Boolean)](
          "liftoverLocus",
      ...
      val f = fb.resultWithIndex()(theHailClassLoader, ctx.fs, ctx.taskContext, ctx.r)
      assert(
        f("GRCh38", Locus("20", 60001), 0.95) == grch37.liftoverLocus(
          "GRCh38",
          Locus("20", 60001),
          0.95,
        )
