...
class UnsafeSuite extends HailSuite {
  def subsetType(t: Type): Type = {
    t match {
      case t: TStruct =>
        TStruct(
          t.fields
            .filter(_ => Random.nextDouble() < 0.4)
  ...
  def codecs(): Array[Array[Any]] =
    (BufferSpec.specs ++ Array(
      TypedCodecSpec(PCanonicalStruct("x" -> PInt64()), BufferSpec.default)
    ))
      .map(x => Array[Any](x))
  ...
  @Test def testPacking() {
    ...
    val t4 = makeStruct(
      (0 until 256).flatMap(_ => Iterator(PInt32(), PInt32(), PFloat64(), PBoolean())): _*
    )
