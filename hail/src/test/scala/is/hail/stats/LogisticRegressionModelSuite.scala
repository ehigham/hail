...
class LogisticRegressionModelSuite extends HailSuite {
  ...
  @Test def covariatesVsInterceptOnlyTest() {
    ...
    val C = DenseMatrix((0.0, -1.0), (2.0, 3.0), (1.0, 5.0), (-2.0, 0.0), (-2.0, -4.0), (4.0, 3.0))
  ...
  @Test def firthSeparationTest() {
    ...
    assert(
      LogisticFirthTest.test(X, y, nullFit, "logistic", 25, 1e-5).stats.isDefined
    ) // firth penalized MLE still exists
  }
}
