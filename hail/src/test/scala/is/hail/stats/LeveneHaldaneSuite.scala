...
class LeveneHaldaneSuite extends TestNGSuite {
  ...
  def LH(n: Int, nA: Int)(nAB: Int): Double = {
    ...
    if (nAB < 0 || nAB > nA || (nA - nAB) % 2 != 0) 0
      ...
      val nBB = (nB - nAB) / 2
      math.exp(
        nAB * math.log(2) + factorialLog(n) - (factorialLog(nAA) + factorialLog(nAB) + factorialLog(
          nBB
        )) - (factorialLog(2 * n) - (factorialLog(nA) + factorialLog(nB)))
      )
    }
  }
          ...
          D_==(LH.leftMidP(nAB) + LH.rightMidP(nAB), 1.0)
            ...
            && D_==(
              LH.exactMidP(nAB), {
                ...
                (0 to nA)
                  .map(LH.probability).filter(D_<(_, p0, tolerance = 1.0e-12)).sum + 0.5 * (0 to nA)
                    .map(LH.probability).filter(D_==(_, p0, tolerance = 1.0e-12)).sum
              },
            )
        )
      )
