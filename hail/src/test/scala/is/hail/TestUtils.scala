...
object TestUtils {
  ...
  def assertMatrixEqualityDouble(
    ...
    assert(
      (0 until A.rows).forall(i => (0 until A.cols).forall(j => D_==(A(i, j), B(i, j), tolerance)))
    )
  }
  ...
  def loweredExecute(
    ...
    ExecutionTimer.logTime("TestUtils.loweredExecute") { timer =>
      HailContext
        .sparkBackend("TestUtils.loweredExecute")
  ...
  def eval(
    ...
    val (_, substEnv) = env.m.foldLeft((args.length, Env.empty[IR])) {
      case ((i, env), (name, (v, t))) =>
    ...
    agg match {
      case Some((aggElements, aggType)) =>
        ...
        val aggIR = StreamAgg(
          ...
          aggElementVar,
          MakeTuple.ordered(
            FastSeq(rewrite(Subst(x, BindingEnv(eval = substEnv, agg = Some(substAggEnv)))))
          ),
        )
