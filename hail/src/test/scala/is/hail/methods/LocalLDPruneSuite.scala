...
object LocalLDPruneSuite {
  ...
  def estimateMemoryRequirements(nVariants: Long, nSamples: Int, memoryPerCore: Long): Int = {
    val bytesPerVariant = math
      .ceil(8 * nSamples.toDouble / BitPackedVector.GENOTYPES_PER_PACK).toLong + variantByteOverhead
...
class LocalLDPruneSuite extends HailSuite {
  ...
  lazy val maxQueueSize =
    LocalLDPruneSuite.estimateMemoryRequirements(mt.rvd.count(), mt.nCols, memoryPerCoreBytes)
  ...
  def getLocallyPrunedRDDWithGT(
    ...
    val locallyPrunedVariants = locallyPrunedTable.rdd
      .mapPartitions(
        it => it.map(row => (row.get(locusIndex), row.get(allelesIndex))),
        preservesPartitioning = true,
      ).collectAsSet()

    unprunedMatrixTable.rvd.toRows
      .map { r =>
        (
  ...
  def isGloballyUncorrelated(
    ...
    val r2Matrix = LocalLDPruneSuite.correlationMatrixGT(
      locallyPrunedRDD.map { case (locus, alleles, gs) => gs }.collect()
    )
    val variantMap = locallyPrunedRDD.zipWithIndex
      .map { case ((locus, alleles, gs), i) => (i.toInt, locus) }.collectAsMap()
    ...
    val locallyUncorrelated = {
      locallyPrunedRDD.mapPartitions(
        it => {
          ...
          val computeCorrelationMatrix =
            (gts: Array[Iterable[Annotation]]) => LocalLDPruneSuite.correlationMatrixGT(gts)
  ...
  object Spec extends Properties("LDPrune") {
    ...
    property("bitPacked pack and unpack give same as orig") = forAll(vectorGen) {
      case (nSamples: Int, v1: Array[BoxedCall], _) =>
    ...
    property("R2 bitPacked same as BVector") = forAll(vectorGen) {
      case (nSamples: Int, v1: Array[BoxedCall], v2: Array[BoxedCall]) =>
