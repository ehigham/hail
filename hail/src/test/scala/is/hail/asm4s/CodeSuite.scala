...
class CodeSuite extends HailSuite {
  ...
  @Test def testIntervalSizeInBytes(): Unit = {
    ...
    assert(
      fb.result()(theHailClassLoader)(ctx.r) == 72L
  ...
  @Test def testHash() {
    ...
    assert(
      hashTestNumHelper(new SInt64Value(5000000000L)) == hashTestNumHelper(
    ...
    assert(
      hashTestNumHelper(new SFloat64Value(5000000000.89d)) == hashTestNumHelper(
        new SFloat64Value(5000000000.89d)
      )
    )
    assert(hashTestStringHelper("dog") == hashTestStringHelper("dog"))
    assert(
      hashTestArrayHelper(IndexedSeq(1, 2, 3, 4, 5, 6)) == hashTestArrayHelper(
        IndexedSeq(1, 2, 3, 4, 5, 6)
      )
    )
    assert(hashTestArrayHelper(IndexedSeq(1, 2)) != hashTestArrayHelper(IndexedSeq(3, 4, 5, 6, 7)))
    assert(
      hashTestStructHelper(Row("wolf", 8, .009f), fields) == hashTestStructHelper(
        Row("wolf", 8, .009f),
        fields,
      )
    )
