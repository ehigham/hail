...
class PContainerTest extends PhysicalTestUtils {
  ...
  @Test def arrayCopyTest() {
    ...
    def runTests(deepCopy: Boolean, interpret: Boolean) {
      ...
      val srcType = PCanonicalArray(
        PCanonicalStruct("a" -> PCanonicalArray(PInt32(true)), "b" -> PInt64())
      )
      val destType = PCanonicalArray(
        PCanonicalStruct("a" -> PCanonicalArray(PInt32()), "b" -> PInt64())
      )
