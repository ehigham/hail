...
class BGzipCodecSuite extends HailSuite {
  ...
  @Test def testGenericLinesRandom() {
    ...
    val compSplits = Array[Long](6566, 20290, 33438, 41165, 56691, 70278, 77419, 92522, 106310,
      112477, 112505, 124593, 136405, 144293, 157375, 169172, 175174, 186973, 195325)

    val g = for {
      n <- Gen.oneOfGen(Gen.choose(0, 10), Gen.choose(0, 100))
  ...
  @Test def test() {
    ...
    assert(
      sc.textFile(uncompPath).collectOrdered()
        .sameElements(lines)
    )
    ...
    using(fs.openNoCompression(uncompPath)) { uncompIS =>
      using(new BGzipInputStream(fs.openNoCompression(compPath))) { decompIS =>
        ...
        for {
          ...
          val extra =
            if (cOff == blockStarts.last._1 && e == maxBlockSize - fromEnd)
          ...
          val expectedVirtualOffset =
            if (extra == lastBlockLen - fromEnd)
        ...
        assert(
          decompIS.getVirtualOffset() == BlockCompressedFilePointerUtil.makeFilePointer(
            blockStarts(2)._2,
            0,
          )
        )
