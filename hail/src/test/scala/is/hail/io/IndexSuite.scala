...
class IndexSuite extends HailSuite {
  val strings = Array("bear", "cat", "deer", "dog", "lion", "mouse", "parrot", "quail", "rabbit",
    "raccoon", "rat", "raven", "skunk", "snail", "squirrel", "vole", "weasel", "whale", "yak",
    "zebra")

  val stringsWithDups = Array("bear", "bear", "cat", "cat", "cat", "cat", "cat", "cat", "cat",
    "dog", "mouse", "mouse", "skunk", "skunk", "skunk", "whale", "whale", "zebra", "zebra", "zebra")
  ...
  @Test def testQueryByKey() {
    for (branchingFactor <- 2 to 5) {
      ...
      assert(
        stringsInList.forall(s => index.queryByKey(s) sameElements leafsWithDups.filter(_.key == s))
      )
    }
  }
  ...
  @Test def testIntervalIterator() {
    for (branchingFactor <- 2 to 5) {
      ...
      intercept[IllegalArgumentException](
        index.queryByInterval("bear", "bear", includesStart = false, includesEnd = false).toFastSeq
      )
      intercept[IllegalArgumentException](
        index.queryByInterval("bear", "bear", includesStart = false, includesEnd = true).toFastSeq
      )
      intercept[IllegalArgumentException](
        index.queryByInterval("bear", "bear", includesStart = true, includesEnd = false).toFastSeq
      )
      intercept[IllegalArgumentException](
        index.queryByInterval("cat", "bear", includesStart = true, includesEnd = true).toFastSeq
      )

      val endPoints =
        (stringsWithDups.distinct ++ Array("aardvark", "boar", "elk", "oppossum", "snail", "zoo"))
          .combinations(2)
            ...
            if (Interval.isValid(ordering, bounds(0), bounds(1), includesStart, includesEnd)) {
              val lowerBoundIdx =
                if (includesStart) {
                  ...
                  stringsWithDups.lastIndexWhere(
                    bounds(0) >= _
                  ) + 1 // want last index at transition point where key is gteq and then want to exclude that point (add 1)

              val upperBoundIdx =
                if (includesEnd)
                  stringsWithDups.lastIndexWhere(
                    bounds(1) >= _
                  ) + 1 // want last index at transition point and then want to include that value so add 1
                else
                  stringsWithDups.lastIndexWhere(
                    bounds(1) > _
                  ) + 1 // want last index before transition point and then want to include that value so add 1

              assert(
                index.queryByInterval(bounds(0), bounds(1), includesStart, includesEnd).toFastSeq ==
              ...
              intercept[IllegalArgumentException](
                index.queryByInterval(bounds(0), bounds(1), includesStart, includesEnd)
              )
          }
        }
      }
  ...
  @Test def testIntervalIteratorWorksWithGeneralEndpoints() {
    for (branchingFactor <- 2 to 5) {
      ...
      assert(
        index.queryByInterval(Row(), Row(), includesStart = true, includesEnd = true).toFastSeq ==
          leafChildren
      )
      assert(
        index
  ...
  @Test def testIterateFromUntil() {
    for (branchingFactor <- 2 to 5) {
      ...
      uniqueStrings.foreach { s =>
        ...
        assert(
          index.iterateFrom(s).toFastSeq == leafsWithDups
            .slice(start, stringsWithDups.length).toFastSeq
        )
