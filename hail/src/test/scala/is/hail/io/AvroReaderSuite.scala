...
class AvroReaderSuite extends HailSuite {
  ...
  private val testSchema = SchemaBuilder
    .record("Root")
    .fields()
  ...
  def makeRecord(row: Row): GenericRecord = row match {
    case Row(int, long, float, double, string) =>
  ...
  def makeTestFile(): String = {
    ...
    using(fs.create(avroFile)) { os =>
      using(
        new DataFileWriter[GenericRecord](new GenericDatumWriter(testSchema)).create(testSchema, os)
      ) { dw =>
        for (row <- testValue)
