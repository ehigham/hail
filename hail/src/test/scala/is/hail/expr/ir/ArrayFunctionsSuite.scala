...
class ArrayFunctionsSuite extends HailSuite {
  ...
  def sum(a: IndexedSeq[Integer]) {
    assertEvalsTo(
      ...
      Option(a)
        .flatMap(
          _.foldLeft[Option[Int]](Some(0))((comb, x) => comb.flatMap(c => Option(x).map(_ + c)))
        ).orNull,
    )
  }
  ...
  def product(a: IndexedSeq[Integer]) {
    assertEvalsTo(
      ...
      Option(a)
        .flatMap(
          _.foldLeft[Option[Int]](Some(1))((comb, x) => comb.flatMap(c => Option(x).map(_ * c)))
        ).orNull,
    )
  }
  ...
  def lift(f: (Int, Int) => Int)
    : (IndexedSeq[Integer], IndexedSeq[Integer]) => IndexedSeq[Integer] = { case (a, b) =>
    Option(a)
      .zip(Option(b)).headOption.map { case (a0, b0) =>
    ...
    assertEvalsTo(
      ...
      Option(a)
        .zip(Option(b)).headOption.map { case (a0, b0) =>
          a0.zip(b0).map { case (i, j) =>
            Option(i)
              .zip(Option(j)).headOption.map[java.lang.Double] { case (m, n) =>
                m.toDouble / n
              }.orNull
          }
        }.orNull,
