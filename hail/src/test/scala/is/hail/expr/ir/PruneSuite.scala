...
class PruneSuite extends HailSuite {
  @Test def testUnionType() {
    val base = TStruct(
      "a" -> TStruct("aa" -> TInt32, "ab" -> TStruct("aaa" -> TString)),
      "b" -> TInt32,
      "c" -> TArray(TStruct("ca" -> TInt32)),
    )
    ...
    assert(
      PruneDeadFields.unify(base, TStruct("a" -> TStruct.empty), TStruct("b" -> TInt32)) == TStruct(
        "a" -> TStruct.empty,
        "b" -> TInt32,
      )
    )
    assert(
      PruneDeadFields.unify(base, TStruct("c" -> TArray(TStruct.empty))) == TStruct(
  ...
  def tableRefStruct(tt: TableType, fields: String*): IR =
    MakeStruct(
      tt.key.map(k => k -> GetField(Ref("row", tt.rowType), k)) ++ FastSeq(
  ...
  @Test def testAggLetMemo() {
    checkMemo(
      AggLet(
        ...
        ApplyAggOp(
          FastSeq(),
          FastSeq(SelectFields(Ref("foo", ref.typ), IndexedSeq("a"))),
  ...
  @Test def testStreamMergeMemo() {
    val st2 = st.deepCopy()
    checkMemo(
      StreamMultiMerge(IndexedSeq(st, st2), FastSeq("a")),
      TStream(justB),
  ...
  @Test def testTableUnionRebuildUnifiesRowTypes() {
    ...
    val tfilter = TableFilter(TableMapRows(tr, mapExpr), tableRefBoolean(tr.typ, "row.2"))
  ...
  @Test def testMatrixMapRowsRebuild() {
    val mmr = MatrixMapRows(MatrixKeyRowsBy(mr, IndexedSeq.empty), matrixRefStruct(mr.typ, "va.r2"))
    checkRebuild(
      mmr,
      ...
      (_: BaseIR, r: BaseIR) => {
        ...
        mmr.child
          .asInstanceOf[MatrixKeyRowsBy].child.asInstanceOf[MatrixRead].typ == subsetMatrixTable(
            mr.typ,
            "global.g1",
            "va.r2",
            "g.e1",
  ...
  @Test def testMatrixMapColsRebuild() {
    ...
    checkRebuild(
      ...
      (_: BaseIR, r: BaseIR) => {
        ...
        mmc.child
          .asInstanceOf[MatrixRead].typ == subsetMatrixTable(mr.typ, "global.g1", "sa.c2", "g.e1")
      },
    )
  }
  ...
  @Test def testMatrixMapEntriesRebuild() {
    ...
    checkRebuild(
      ...
      (_: BaseIR, r: BaseIR) => {
        ...
        mme.child
          .asInstanceOf[MatrixRead].typ == subsetMatrixTable(mr.typ, "global.g1", "sa.c2", "va.r2")
      },
    )
  }
  ...
  @Test def testMatrixMapGlobalsRebuild() {
    ...
    checkRebuild(
      ...
      (_: BaseIR, r: BaseIR) => {
        ...
        mmg.child
          .asInstanceOf[MatrixRead].typ == subsetMatrixTable(mr.typ, "global.g1", "va.r2", "g.e1")
      },
    )
  }
  ...
  @Test def testMatrixUnionRowsRebuild() {
    ...
    checkRebuild(
      MatrixUnionRows(
        FastSeq(mat, MatrixMapCols(mat2, Ref("sa", mat2.typ.colType), Some(FastSeq("ck"))))
      ),
  ...
  @Test def testMatrixUnionColsRebuild(): Unit = {
    ...
    val wrappedMat = MatrixMapCols(
      mat,
      MakeStruct(
        IndexedSeq(("ck", getColField("ck")), ("c2", getColField("c2")), ("c3", getColField("c3")))
      ),
      Some(FastSeq("ck")),
    )
    ...
    checkRebuild[MatrixUnionCols](
      ...
      (old, rebuilt) => childrenMatch(rebuilt),
    )
  ...
  @Test def testMatrixAnnotateRowsTableRebuild() {
    ...
    checkRebuild(
      ...
      (_: BaseIR, r: BaseIR) => r.isInstanceOf[MatrixLiteral],
    )
  }
  ...
  @Test def testCoalesceRebuild() {
    checkRebuild(
      ...
      subsetTS("b"),
      (_: BaseIR, r: BaseIR) => r.children.forall(_.typ == subsetTS("b")),
    )
  }
  ...
  @Test def testMakeStructRebuild() {
    checkRebuild(
      ...
      subsetTS("b"),
      (_: BaseIR, r: BaseIR) => r == MakeStruct(IndexedSeq("b" -> NA(TInt64))),
    )
  }
  ...
  @Test def testInsertFieldsRebuild() {
    ...
    checkRebuild[InsertFields](
      insertF,
      TStruct(("b", TInt32)),
      (old, rebuilt) => PruneDeadFields.isSupertype(rebuilt.typ, old.typ),
    )
  }
  ...
  @Test def testMakeTupleRebuild() {
    checkRebuild(
      ...
      (_: BaseIR, r: BaseIR) => r == MakeTuple(IndexedSeq(2 -> NA(TString))),
    )
  }
  ...
  @Test def testTableCollectRebuild() {
    ...
    checkRebuild(tc, TStruct.empty, (_: BaseIR, r: BaseIR) => r == MakeStruct(IndexedSeq()))
  }
  ...
  def supertypePairs: Array[Array[Type]] = Array(
    Array(TInt32, TInt32),
    Array(
      TStruct("a" -> TInt32, "b" -> TArray(TInt64)),
      TStruct("a" -> TInt32, "b" -> TArray(TInt64)),
    ),
