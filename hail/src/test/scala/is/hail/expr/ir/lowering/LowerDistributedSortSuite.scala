...
class LowerDistributedSortSuite extends HailSuite {
  ...
  @Test def testSamplePartition() {
    ...
    val data1 = ToStream(
      Literal(TArray(elementType), dataKeys.map { case (k1, k2) => Row(k1, k2, k1 * k1) })
    )
    ...
    val data2 = ToStream(
      Literal(TArray(elementType2), dataKeys2.map { case (k1, k2) => Row(k1, k2) })
    )
