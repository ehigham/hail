...
class SimplifySuite extends HailSuite {
  ...
  @Test def testRepartitionableMapUpdatesForUpstreamOptimizations() {
    ...
    val checksRepartitioningIR =
      TableFilter(TableOrderBy(range, FastSeq(SortField("idx", Ascending))), simplifiableIR)
  ...
  @Test def testInsertFieldsRewriteRules() {
    ...
    assert(
      Simplify(ctx, ir2) == InsertFields(base, Seq("3" -> I32(3)), Some(FastSeq("3", "1", "2")))
    )
  ...
  @Test def testFilterParallelize() {
    for (
      rowsAndGlobals <- Array(
        MakeStruct(
          FastSeq(("rows", In(0, TArray(TStruct("x" -> TInt32)))), ("global", In(1, TStruct.empty)))
        ),
  ...
  @Test def testNestedFilterIntervals() {
    ...
    assert(
      Simplify(ctx, tir).asInstanceOf[TableFilterIntervals].intervals == FastSeq(
    ...
    Array((Literal.coerce(TInt32, _)) -> TInt32, (Literal.coerce(TInt64, _)) -> TInt64).flatMap {
      case (pure, typ) =>
        Array.concat(
          ...
          // Shifts
          Array(LeftShift(), RightShift(), LogicalRightShift())
            .flatMap { shift =>
              Array(
  ...
  def binaryFloatingArithmetic: Array[Array[Any]] =
    Array((Literal.coerce(TFloat32, _)) -> TFloat32, (Literal.coerce(TFloat64, _)) -> TFloat64)
