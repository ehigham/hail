...
class AggregatorsSuite extends HailSuite {
  ...
  def arraySumFloat64OnSmallArray(): Unit =
    assertArraySumEvalsTo(
      TFloat64,
      FastSeq(FastSeq(1.0, 2.0), FastSeq(10.0, 20.0), null),
      FastSeq(11.0, 22.0),
    )
  ...
  def arraySumInt64OnSmallArray(): Unit =
    assertArraySumEvalsTo(
      TInt64,
      FastSeq(FastSeq(1L, 2L), FastSeq(10L, 20L), null),
      FastSeq(11L, 22L),
    )
  ...
  def arraySumInt64FirstElementMissing(): Unit =
    assertArraySumEvalsTo(
      TInt64,
      FastSeq(null, FastSeq(1L, 33L), FastSeq(42L, 3L)),
      FastSeq(43L, 36L),
    )
  ...
  def keyedKeyedCollect() {
    ...
    assertEvalsTo(
      AggGroupBy(
        ...
        AggGroupBy(
          Ref("k2", TBoolean),
          ApplyAggOp(FastSeq(), FastSeq(Ref("x", TInt32)), aggSig),
          false,
        ),
        false,
      ),
  ...
  def keyedKeyedCallStats() {
    ...
    assertEvalsTo(
      AggGroupBy(
        ...
        AggGroupBy(
          Ref("k2", TString),
          ApplyAggOp(FastSeq(I32(2)), FastSeq(Ref("g", TCall)), aggSig),
          false,
        ),
        false,
      ),
  ...
  def keyedKeyedTakeBy() {
    ...
    assertEvalsTo(
      AggGroupBy(
        ...
        AggGroupBy(
          Ref("k2", TString),
          ApplyAggOp(FastSeq(I32(2)), FastSeq(Ref("x", TFloat64), Ref("y", TInt32)), aggSig),
          false,
        ),
        false,
      ),
