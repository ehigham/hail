...
class OrderingSuite extends HailSuite {
  ...
  def getStagedOrderingFunction(
    ...
    fb.emitWithBuilder { cb =>
      ...
      fb.ecb
        .getOrderingFunction(cv1.st, cv2.st, op)
  ...
  @Test def testMissingNonequalComparisons() {
    def getStagedOrderingFunctionWithMissingness(
      ...
      fb.emitWithBuilder { cb =>
        ...
        fb.ecb
          .getOrderingFunction(ev1.st, ev2.st, op)
          .apply(cb, ev1, ev2)
      }
      fb.resultWithIndex()(theHailClassLoader, ctx.fs, ctx.taskContext, r)
    }
  ...
  def testDictGetOnRandomDict() {
    ...
    val compareGen = Gen.zip(Type.genArb, Type.genArb).flatMap { case (k, v) =>
      Gen.zip(Gen.const(TDict(k, v)), TDict(k, v).genNonmissingValue(sm), k.genNonmissingValue(sm))
    }
    val p = Prop.forAll(compareGen) {
      case (tdict: TDict, dict: Map[Any, Any] @unchecked, testKey1) =>
        assertEvalsTo(
    ...
    val p = Prop.forAll(compareGen.filter { case (tdict, a, key) =>
      ...
      pool.scopedRegion { region =>
        ...
        val bs = new BinarySearch(
          ...
          { (cb, elt) =>
            cb.memoize(elt.toI(cb).flatMap(cb) { case x: SBaseStructValue =>
              x.loadField(cb, 0)
            })
          },
        )
        ...
        if (closestI == asArray.length) {
          ...
          val closestIIsClosest =
            (pDict.keyType.virtualType
              .ordering(sm).compare(key, maybeEqual) <= 0 || closestI == dict.size - 1) &&
              (closestI == 0 || pDict.keyType.virtualType
                .ordering(sm).compare(key, getKey(closestI - 1)) > 0)
