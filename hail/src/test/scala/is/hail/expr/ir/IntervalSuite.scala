...
class IntervalSuite extends HailSuite {
  ...
  @Test def isEmpty() {
    for (setInterval <- testIntervals) {
      val interval = toIRInterval(setInterval)
      assert(
        eval(invoke("isEmpty", TBoolean, ErrorIDs.NO_ERROR, interval)) == setInterval
          .definitelyEmpty()
      )
    }
  }
  ...
  @Test def overlaps() {
    for {
      ...
      assert(
        eval(invoke("overlaps", TBoolean, interval1, interval2)) == setInterval1.probablyOverlaps(
          setInterval2
        )
      )
    }
  ...
  @Test def testIntervalSortAndReduce() {
    ...
    assert(
      Interval
        .union(Array(intInterval(0, 10)), ord)
  ...
  @Test def testIntervalIntersection() {
    ...
    assert(Interval.intersection(x1, x2, ord).toSeq == x1.toSeq)
    assert(
      Interval.intersection(x1, x3, ord).toSeq == FastSeq[Interval](
        intInterval(7, 10),
  ...
  @Test def testPointPartitionIntervalEndpointComparison() {
    def assertComp(
      ...
      val endpointIR = MakeTuple.ordered(
        FastSeq(
          MakeTuple.ordered(
            Array.tabulate(3)(i =>
      ...
      assertEvalsTo(invoke(function, TBoolean, pointIR, endpointIR, leansRightIR), expected)
    }
