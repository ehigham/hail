...
class EmitStreamSuite extends HailSuite {
  ...
  @Test def testStreamBufferedAggregator(): Unit = {
    ...
    val newKey = MakeStruct(
      IndexedSeq("count" -> SelectFields(Ref("foo", streamType.elementType), IndexedSeq("a", "b")))
    )
  ...
  @Test def testStreamBufferedAggregatorCombine(): Unit = {
    ...
    val seqOps = SeqOp(0, FastSeq(), countAggSig)
    val newKey = MakeStruct(
      IndexedSeq("count" -> SelectFields(Ref("foo", streamType.elementType), IndexedSeq("a")))
    )
    val streamBuffAggCount = StreamBufferedAggregate(
      countStructStream,
      initOps,
  ...
  @Test def testStreamBufferedAggregatorCollectAggregator(): Unit = {
    ...
    val newKey = MakeStruct(
      IndexedSeq("collect" -> SelectFields(Ref("foo", streamType.elementType), IndexedSeq("a")))
    )
  ...
  @Test def testStreamBufferedAggregatorMultipleAggregators(): Unit = {
    ...
    val collectStructStream = MakeStream(
      IndexedSeq(elemOne, elemTwo, elemThree, elemFour, elemFive, elemSix, elemSeven, elemEight,
        elemNine, elemTen, elemEleven, elemTwelve, elemThirteen, elemFourteen, elemFifteen,
        elemSixteen, elemSeventeen),
      streamType,
    )
    val collectAggSig =
      PhysicalAggSig(Collect(), CollectStateSig(VirtualTypeWithReq(PType.canonical(TInt64))))
  ...
  @Test def testEmitJoinRightDistinct() {
    ...
    def join(lstream: IR, rstream: IR, joinType: String): IR =
      StreamJoinRightDistinct(
        ...
        "r",
        MakeTuple.ordered(
          IndexedSeq(GetField(Ref("l", eltType), "v"), GetField(Ref("r", eltType), "v"))
        ),
        joinType,
      )
  ...
  @Test def testEmitJoinRightDistinctInterval() {
    ...
    def join(lstream: IR, rstream: IR, joinType: String): IR =
      StreamJoinRightDistinct(
        ...
        "r",
        MakeTuple.ordered(
          IndexedSeq(GetField(Ref("l", lEltType), "v"), GetField(Ref("r", rEltType), "v"))
        ),
        joinType,
      )
    ...
    def rElts(xs: ((Char, Any, Any, Char), String)*): IR =
      MakeStream(
        xs.toArray.map { case ((is, s, e, ie), v) =>
  ...
  @Test def testGrouped() {
    ...
    assertEvalsTo(
      ...
      IndexedSeq(IndexedSeq(0, 1, 2), IndexedSeq(3, 4, 5), IndexedSeq(6, 7, 8), IndexedSeq(9)),
    )
    ...
    assertEvalsTo(
      ...
      IndexedSeq(IndexedSeq(0, 1, 2, 3, 4), IndexedSeq(5, 6, 7, 8, 9)),
    )
  ...
  def filteredRangeStructs(size: IR): IR = mapIR(
    filterIR(
