...
class IRSuite extends HailSuite {
  ...
  @Test def testMakeArrayWithDifferentRequiredness(): Unit = {
    ...
    val pt2 = PCanonicalArray(
      PCanonicalStruct(true, "a" -> PInt32(true), "b" -> PCanonicalArray(PInt32(), true))
    )
  ...
  @Test def testLowerBoundOnOrderedCollectionSet() {
    ...
    val swna = ToSet(
      MakeStream(FastSeq(I32(0), I32(2), I32(2), I32(4), NA(TInt32)), TStream(TInt32))
    )
  ...
  @Test def testStreamLen(): Unit = {
    ...
    val filteredRange = StreamLen(
      StreamFilter(rangeIR(12), "x", irToPrimitiveIR(Ref("x", TInt32)) < 5)
    )
    assertEvalsTo(filteredRange, 5)

    val lenOfLet = StreamLen(
      bindIR(I32(5))(ref =>
        StreamGrouped(
  ...
  @Test def testArrayFlatMap() {
    ...
    val a = MakeStream(
      FastSeq(MakeArray(FastSeq(I32(7), NA(TInt32)), ta), NA(ta), MakeArray(FastSeq(I32(2)), ta)),
      tsa,
    )
    ...
    val b = MakeStream(
      FastSeq(MakeArray(FastSeq(I32(7), I32(0)), ta), NA(ta), MakeArray(FastSeq(I32(2)), ta)),
      tsa,
    )
    assertEvalsTo(
  ...
  @Test def testArrayScan() {
    ...
    def scan(array: IR, zero: IR, f: (IR, IR) => IR): IR =
      ToArray(
        StreamScan(array, zero, "_accum", "_elt", f(Ref("_accum", zero.typ), Ref("_elt", zero.typ)))
      )
  ...
  @Test def testNDArrayConcat() {
    ...
    val nd1 = (FastSeq(0, 1, 2, 3, 4, 5), 2L, 3L)

    val rowwise = (FastSeq(6, 7, 8, 9, 10, 11, 12, 13, 14), 3L, 3L)

    val colwise = (FastSeq(15, 16, 17, 18), 2L, 2L)
    ...
    val colwiseExpected = FastSeq(FastSeq(0, 1, 2, 15, 16), FastSeq(3, 4, 5, 17, 18))
    ...
    val topRow = NDArraySlice(
      ...
      MakeTuple.ordered(
        IndexedSeq(
          I64(0),
          MakeTuple.ordered(
            IndexedSeq(I64(0), GetTupleElement(NDArrayShape(matrixRowMajor), 1), I64(1))
          ),
        )
      ),
    )
  ...
  @Test def testNDArrayFilter() {
    ...
    assertNDEvals(
      NDArrayFilter(
        matrixRowMajor,
        FastSeq(MakeArray(FastSeq(I64(0)), TArray(TInt64)), NA(TArray(TInt64))),
      ),
    ...
    assertNDEvals(
      NDArrayFilter(
        matrixRowMajor,
        FastSeq(NA(TArray(TInt64)), MakeArray(FastSeq(I64(0)), TArray(TInt64))),
      ),
  ...
  @Test def testStreamZipJoin() {
    ...
    assertEvalsTo(zipJoin(FastSeq(Array[Integer](0, 1, null), null), 1), null)
    ...
    assertEvalsTo(zipJoin(FastSeq(Array[Integer](), Array[Integer]()), 1), FastSeq())

    assertEvalsTo(
      zipJoin(FastSeq(Array[Integer](0, 1)), 1),
      FastSeq(Row(0, FastSeq(Row(0, "x", 0))), Row(1, FastSeq(Row(1, "x", 1)))),
    )
  }
  ...
  @Test def testStreamMultiMerge() {
    ...
    assertEvalsTo(
      ...
      FastSeq(Row(0, "x", 0), Row(0, "x", 0), Row(1, "x", 1), Row(2, "x", 1)),
    )

    assertEvalsTo(merge(FastSeq(Array[Integer](), Array[Integer]()), 1), FastSeq())

    assertEvalsTo(merge(FastSeq(Array[Integer](0, 1)), 1), FastSeq(Row(0, "x", 0), Row(1, "x", 1)))
  }
  ...
  @Test def testJoinRightDistinct() {
    ...
    assertEvalsTo(
      join(
        ...
        MakeStream.unify(
          ctx,
          IndexedSeq(
            MakeStruct(FastSeq("b" -> I32(0), "k2" -> Str("x"), "k1" -> I32(3), "c" -> Str("foo")))
          ),
        ),
        FastSeq("k1", "k2"),
        FastSeq("k1", "k2"),
    ...
    assertEvalsTo(
      ...
      FastSeq(Row(0, "x", 0L, null, null), Row(null, "x", 1L, null, null)),
    )
    ...
    assertEvalsTo(
      ...
      FastSeq(Row(0, "x", 0L, null, null), Row(1, "x", 1L, 0, "foo"), Row(2, "x", 2L, null, null)),
    )

    assertEvalsTo(
      leftJoinRows(Array[Integer](0, 1, 2), Array[Integer](-1, 0, 0, 1, 1, 2, 2, 3)),
      FastSeq(Row(0, "x", 0L, 1, "foo"), Row(1, "x", 1L, 3, "foo"), Row(2, "x", 2L, 5, "foo")),
    )
  ...
  @Test def testStreamJoin() {
    ...
    assertEvalsTo(
      ...
      FastSeq(Row(1, 0, 2), Row(1, 0, 3), Row(1, 1, 2), Row(1, 1, 3)),
    )
  ...
  @Test def testStreamMerge() {
    ...
    assertEvalsTo(
      ...
      FastSeq(Row(1, 1, 0), Row(2, 1, 1), Row(null, 1, 2)),
    )
  ...
  @Test def testInsertFields() {
    ...
    assertEvalsTo(InsertFields(NA(s), IndexedSeq()), null)

    assertEvalsTo(InsertFields(emptyStruct, IndexedSeq("a" -> I64(5))), Row(5L, null))

    assertEvalsTo(InsertFields(emptyStruct, IndexedSeq("c" -> F64(3.2))), Row(null, null, 3.2))

    assertEvalsTo(InsertFields(emptyStruct, IndexedSeq("c" -> NA(TFloat64))), Row(null, null, null))

    assertEvalsTo(
      InsertFields(MakeStruct(IndexedSeq("a" -> NA(TInt64), "b" -> Str("abc"))), IndexedSeq()),
      Row(null, "abc"),
    )
  ...
  @Test def testSelectFields() {
    assertEvalsTo(
      SelectFields(NA(TStruct("foo" -> TInt32, "bar" -> TFloat64)), FastSeq("foo")),
      null,
    )

    assertEvalsTo(
      SelectFields(MakeStruct(FastSeq("foo" -> 6, "bar" -> 0.0)), FastSeq("foo")),
      Row(6),
    )

    assertEvalsTo(
      SelectFields(MakeStruct(FastSeq("a" -> 6, "b" -> 0.0, "c" -> 3L)), FastSeq("b", "a")),
      Row(0.0, 6),
    )
  }
  ...
  @Test def testGroupByKey() {
    ...
    def groupby(tuples: IR*): IR = GroupByKey(
      MakeStream(tuples.toArray[IR], TStream(TTuple(TString, TInt32)))
    )
  ...
  def valueIRs(ctx: ExecuteContext): Array[Array[Object]] = {
    ...
    val blockMatrix = BlockMatrixRead(
      BlockMatrixNativeReader(fs, "src/test/resources/blockmatrix_example/0")
    )
    ...
    val irs = Array[(IR, BindingEnv[Type] => BindingEnv[Type])](
      ...
      ApplyAggOp(FastSeq(I32(2)), FastSeq(call), callStatsSig) -> (_.createAgg
        .bindAgg(call.name, call.typ)),
      ...
      WriteMetadata(Begin(FastSeq()), RelationalWriter("path", overwrite = false, None)),
      ReadValue(
        Str("foo"),
        ETypeValueReader(
          TypedCodecSpec(
  ...
  def tableIRs(ctx: ExecuteContext): Array[Array[TableIR]] = {
    try {
      ...
      val xs: Array[TableIR] = Array(
        ...
        TableAggregateByKey(read, MakeStruct(FastSeq("a" -> I32(5)))),
        ...
        TableMapRows(
          ...
          MakeStruct(
            FastSeq("a" -> GetField(Ref("row", read.typ.rowType), "f32"), "b" -> F64(-2.11))
          ),
        ),
        TableMapPartitions(
          TableKeyBy(read, FastSeq()),
        ...
        TableMapGlobals(read, MakeStruct(FastSeq("foo" -> NA(TArray(TInt32))))),
        TableRange(100, 10),
        TableUnion(FastSeq(TableRange(100, 10), TableRange(50, 10))),
  ...
  def testBlockMatrixIRParserPersist() {
    ...
    val persist = BlockMatrixRead(
      BlockMatrixPersistReader("x", BlockMatrixType.fromBlockMatrix(bm))
    )
  ...
  @Test def testRelationalLetTable() {
    ...
    val ir = TableAggregate(
      RelationalLetTable(
        ...
        TableParallelize(
          MakeStruct(FastSeq("rows" -> RelationalRef("x", t), "global" -> MakeStruct(FastSeq())))
        ),
      ),
  ...
  def relationalFunctionsData(): Array[Array[Any]] = Array(
    ...
    Array(
      LinearRegressionRowsChained(FastSeq(FastSeq("foo")), "bar", Array("baz"), 1, Array("a", "b"))
    ),
    Array(
      LogisticRegression("firth", Array("a", "b"), "c", Array("d", "e"), Array("f", "g"), 25, 1e-6)
    ),
  ...
  def regressionTestUnifyBug(): Unit = {
    ...
    assert(
      ir.typ
        .ordering(ctx.stateManager).equiv(
          FastSeq(
            Interval(
  ...
  @Test def testSimpleTailLoop(): Unit = {
    ...
    val triangleSum: IR = TailLoop(
      ...
      If(
        ...
        Recur(
          "f",
          FastSeq(Ref("x", TInt32) - I32(1), Ref("accum", TInt32) + Ref("x", TInt32)),
          TInt32,
        ),
      ),
    )
      ...
      If(
        ...
        TailLoop(
          ...
          If(
            ...
            Recur(
              "f2",
              FastSeq(Ref("x2", TInt32) - I32(5), Ref("accum2", TInt32) + Ref("x2", TInt32)),
              TInt32,
            ),
          ),
        ),
  ...
  def nonNullTypesAndValues(): Array[Array[Any]] = Array(
    ...
    Array(
      PTypeReferenceSingleCodeType(
        PCanonicalTuple(false, PInt32(), PCanonicalString(), PCanonicalStruct())
      ),
      Row(3, "bar", Row()),
    ),
  )
  ...
  def runStreamDistTest(data: IndexedSeq[Int], splitters: IndexedSeq[Int]): Unit = {
    def makeRowStruct(i: Int) = MakeStruct(
      IndexedSeq(("rowIdx", I32(i)), ("extraInfo", I32(i * i)))
    )
