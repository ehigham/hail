...
//  }
  ...
  def valueIR(): Array[Array[Any]] = {
    ...
    // test coalesce
    nodes += Array(
      Coalesce(FastSeq(array(required, optional), array(optional, required))),
      parray(required, optional),
    )

    nodes += Array(
      Coalesce(FastSeq(array(optional, optional), array(optional, required))),
    ...
    val loop = TailLoop(
      "loop",
      FastSeq(param1.name -> array(required, required), param2.name -> int(required)),
      tnestedarray,
      If(
        False(), // required
        ...
        If(
          ...
          Recur("loop", FastSeq(array(required, optional), int(required)), tnestedarray),
          Recur("loop", FastSeq(array(optional, required), int(optional)), tnestedarray),
        ),
      ),
    )
    ...
    // ApplyIR
    nodes += Array(invoke("argmin", TInt32, array(required, required)), pint(optional))
    nodes += Array(invoke("argmin", TInt32, array(required, optional)), pint(optional))
    nodes += Array(invoke("argmin", TInt32, array(optional, required)), pint(optional))
    // Apply
    nodes += Array(
      invoke(
        "fisher_exact_test",
  ...
  def tableIR(): Array[Array[Any]] = {
    ...
    nodes += Array(
      ...
      rowType.insertFields(
        FastSeq("e1" -> PCanonicalStruct(required, "x" -> pint(required), "y" -> pint(required)))
      ),
      globalType,
    )
    ...
    val left = TableMapGlobals(
      TableKeyBy(
        TableMapRows(
          table.deepCopy(),
          makestruct("a" -> nestedarray(required, optional, required), "b" -> GetField(row, "b")),
        ),
        FastSeq("a"),
      ),
      selectIR(global, "x"),
    ...
    val right = TableMapGlobals(
      TableKeyBy(
        TableMapRows(
          table.deepCopy(),
          makestruct("a" -> nestedarray(required, required, optional), "c" -> GetField(row, "c")),
        ),
        FastSeq("a"),
      ),
      selectIR(global, "y", "z"),
