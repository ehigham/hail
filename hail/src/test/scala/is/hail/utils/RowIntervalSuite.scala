...
class RowIntervalSuite extends HailSuite {
  ...
  @Test def testAbovePosition() {
    ...
    assert(
      Interval(Row(0, 1, 2, 3), Row(1, 2, 3, 4), true, true).isAbovePosition(pord, Row(0, 1, 1, 4))
    )
    assert(
      !Interval(Row(0, 1, 2, 3), Row(1, 2, 3, 4), true, true).isAbovePosition(pord, Row(0, 1, 2, 2))
    )
  }
  ...
  @Test def testAbutts() {
    assert(
      Interval(Row(0, 1, 5), Row(1, 2, 4), true, true)
        .abutts(pord, Interval(Row(1, 2, 4), Row(1, 3, 4), false, true))
    )
    assert(
      !Interval(Row(0, 1, 5), Row(1, 2, 4), true, true)
        .abutts(pord, Interval(Row(1, 2, 4), Row(1, 3, 4), true, true))
    )

    assert(
      Interval(Row(0, 1), Row(1, 2), true, true)
        .abutts(pord, Interval(Row(1, 2), Row(1, 3), false, true))
    )
    assert(
      !Interval(Row(0, 1), Row(1, 2), true, true)
        .abutts(pord, Interval(Row(1, 2), Row(1, 3), true, true))
    )
  }
