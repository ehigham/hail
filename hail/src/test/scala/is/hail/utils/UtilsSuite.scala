...
class UtilsSuite extends HailSuite {
  ...
  @Test def testSortFileListEntry() {
    ...
    val partFileNames = fs
      .glob("src/test/resources/part-*")
  ...
  @Test def storageLevelStringTest() = {
    val sls = List("NONE", "DISK_ONLY", "DISK_ONLY_2", "MEMORY_ONLY", "MEMORY_ONLY_2",
  ...
  @Test def testCollectAsSet() {
    Prop
      .forAll(Gen.buildableOf[Array](Gen.choose(-1000, 1000)), Gen.choose(1, 10)) {
        case (values, parts) =>
  ...
  @Test def toMapUniqueSmallDupes() {
    assert(
      toMapIfUnique(Seq(1 -> 2, 6 -> 6, 10 -> 2))(x => x % 5) ==
        Left(Map(1 -> Seq(1, 6)))
    )
  }
