...
DROP PROCEDURE IF EXISTS is_job_cancelled $$
CREATE PROCEDURE is_job_cancelled(
  OUT cancelled BOOLEAN,
  IN batch_id BIGINT,
  IN job_id INT,
)
BEGIN
  SELECT (J.cancelled OR C.id IS NOT NULL) AND NOT J.always_run
  INTO cancelled
  FROM jobs AS J
  LEFT JOIN (
    SELECT C.id AS batch_id, C.job_group_id
    FROM job_groups_cancelled               AS C
    INNER JOIN job_group_self_and_ancestors AS D
       ON C.id           = D.batch_id
      AND C.job_group_id = D.ancestor_id
  ) AS C
     ON C.batch_id     = J.batch_id
    AND C.job_group_id = J.job_group_id
...
BEGIN
  ...
  CALL is_job_cancelled(@cur_job_cancelled, NEW.batch_id, NEW.job_id);
  SELECT ROW_COUNT() FOR SHARE;
...
BEGIN
  ...
  CALL is_job_cancelled(@cur_job_cancel, @in_batch_id, @in_job_id);
  SELECT ROW_COUNT() FOR SHARE;
